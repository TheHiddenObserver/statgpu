...
class PR79GPUValidator:
    ...
    def run_gate_c(self):
        ...
        exit_code, stdout, stderr = self.run_remote_script(script, timeout=300)
        self._log(stdout)
        if stderr.strip():
            self._log(f"stderr: {stderr.strip()}")
        success = exit_code == 0
        self._log(f"Gate C: {'PASSED' if success else 'FAILED'} (exit={exit_code})")
        return success
    ...
    def run_gate_d(self):
        ...
        result_dir = self.result_dir
        script = (
            "import json, os, sys, gc, numpy as np\n"
            "sys.path.insert(0, '.')\n"
        ...
        if stderr.strip():
            self._log(f"stderr: {stderr.strip()}")
        success = exit_code == 0
        self._log(f"Gate D: {'PASSED' if success else 'FAILED'} (exit={exit_code})")
        return success
    ...
    def run_gate_e(self):
        ...
        exit_code, stdout, stderr = self.run_remote_script(script, timeout=600)
        self._log(stdout)
        if stderr.strip():
            self._log(f"stderr: {stderr.strip()}")
        success = exit_code == 0
        self._log(f"Gate E: {'PASSED' if success else 'FAILED'} (exit={exit_code})")
        return success
    ...
    def run_gate_f(self):
        ...
        exit_code, stdout, stderr = self.run_remote_script(script, timeout=600)
        self._log(stdout[-2000:] if len(stdout) > 2000 else stdout)
        if stderr.strip():
            self._log(f"stderr: {stderr.strip()}")
        success = exit_code == 0
        self._log(f"Gate F: {'PASSED' if success else 'FAILED'}")
        return success
    ...
    def run_gate_g(self):
        ...
        if stderr.strip():
            self._log(f"stderr: {stderr.strip()}")
        success = exit_code == 0
        self._log(f"Gate G: {'PASSED' if success else 'FAILED'} (exit={exit_code})")
        return success
