...
replace_once(
    ...
    """- **Fama-MacBeth / shared panel numerical stability**: NumPy/CuPy/Torch now share the conservative Gram-certificate dispatch; non-finite Gram batches are masked before eigenspectrum evaluation, and non-finite Gram/RHS/solutions fall through to the maintained SVD rank policy. Shared SVD least-squares applies inverse singular values to $U^T$ before the raw-response reduction, uses a uniform safe working scale for collectively subnormal full-rank designs, and preserves the existing relative rank cutoff. Fama-MacBeth coefficient averages, fixed-effect group means, and parameter-R² scalar/group means now share a magnitude-tiered float64 reduction path for dynamic-range/cancellation risk while ordinary columns retain the existing fast scatter reduction. Generic grouped sums separate magnitude tiers before applying tier-local count scaling, so a large tier cannot erase a representable subnormal cancellation tail; mean callers keep their additional whole-mean count scaling outside that primitive. Coefficient-series covariance continues to use per-coordinate scales with symmetric large-scale-first restoration. Positive inference variances are no longer replaced by an absolute floor, while exact-zero variance maps zero coefficients to statistic 0 and nonzero coefficients to signed infinity instead of using a fake tiny denominator.""",
)
