"""Fast unsupervised benchmark: skip slow cases."""
import json, time, sys, warnings
sys.path.insert(0, '/root/statgpu')
import numpy as np

try:
    import torch
    Xw = torch.randn(100, 10, device='cuda')
    _ = torch.mm(Xw, Xw.t())
    torch.cuda.synchronize()
    print("GPU warmup done", flush=True)
except:
    pass

results = {}
N_REPEAT = 3

def make_clustering(n, p, k, seed=42):
    rng = np.random.RandomState(seed)
    c = rng.randn(k, p) * 3
    l = rng.randint(0, k, n)
    return c[l] + rng.randn(n, p) * 0.5

def make_reduction(n, p, seed=42):
    return np.random.RandomState(seed).randn(n, p)

def make_nmf(n, p, seed=42):
    return np.abs(np.random.RandomState(seed).randn(n, p)) + 0.1

def bench(cls, kw, X, device, n_rep=N_REPEAT):
    times = []
    for _ in range(n_rep):
        try:
            m = cls(**kw, device=device)
            t0 = time.perf_counter()
            m.fit(X)
            times.append(time.perf_counter() - t0)
        except Exception as e:
            return float('nan'), str(e)[:150]
    return float(np.median(times)), None

from statgpu.unsupervised import (
    PCA, KMeans, NMF, DBSCAN, GaussianMixture,
    TruncatedSVD, IncrementalPCA, MiniBatchKMeans, MiniBatchNMF,
    AgglomerativeClustering, UMAP, TSNE,
)
from sklearn.decomposition import PCA as SkPCA, NMF as SkNMF, TruncatedSVD as SkTSVD, IncrementalPCA as SkIPCA
from sklearn.cluster import KMeans as SkKMeans, DBSCAN as SkDBSCAN, AgglomerativeClustering as SkAgglo
from sklearn.mixture import GaussianMixture as SkGMM

SCALES = {"small": (1000, 20), "medium": (10000, 50), "large": (100000, 100)}

# ===========================================================================
# PCA
# ===========================================================================
print("=== PCA ===", flush=True)
for s, (n, p) in SCALES.items():
    X = make_reduction(n, min(p, 50)); nc = 10
    sk = SkPCA(n_components=nc); t0=time.perf_counter(); sk.fit(X); sk_t=time.perf_counter()-t0
    for be, dev in [("numpy","cpu"),("cupy","cuda"),("torch","cuda")]:
        sg_t, err = bench(PCA, {"n_components":nc}, X, dev)
        spd = sk_t/sg_t if sg_t>0 else 0
        results[f"pca_{s}_{be}"] = {"time":sg_t, "external":sk_t, "speedup":spd}
        print(f"  {s}/{be}: {sg_t:.4f}s spd={spd:.1f}x", flush=True)

# ===========================================================================
# KMeans (GPU needs torch tensor, not numpy)
# ===========================================================================
print("\n=== KMeans ===", flush=True)
for s, (n, p) in SCALES.items():
    X = make_clustering(n, min(p,50), 8)
    sk = SkKMeans(n_clusters=8, n_init=1, random_state=42); t0=time.perf_counter(); sk.fit(X); sk_t=time.perf_counter()-t0
    for be, dev in [("numpy","cpu"),("cupy","cuda"),("torch","cuda")]:
        try:
            if dev != "cpu":
                if be == "cupy":
                    import cupy; X_dev = cupy.asarray(X)
                else:
                    X_dev = torch.tensor(X, device='cuda', dtype=torch.float64)
                m = KMeans(n_clusters=8, n_init=1, random_state=42, device=dev)
                t0=time.perf_counter(); m.fit(X_dev); sg_t=time.perf_counter()-t0
            else:
                sg_t, err = bench(KMeans, {"n_clusters":8,"n_init":1,"random_state":42}, X, dev)
            spd = sk_t/sg_t if sg_t>0 else 0
            results[f"kmeans_{s}_{be}"] = {"time":sg_t, "external":sk_t, "speedup":spd}
            print(f"  {s}/{be}: {sg_t:.4f}s spd={spd:.1f}x", flush=True)
        except Exception as e:
            print(f"  {s}/{be}: FAIL - {e}", flush=True)

# ===========================================================================
# GMM
# ===========================================================================
print("\n=== GMM ===", flush=True)
for s, (n, p) in SCALES.items():
    X = make_clustering(n, min(p,50), 5)
    sk = SkGMM(n_components=5, covariance_type='diag', random_state=42); t0=time.perf_counter(); sk.fit(X); sk_t=time.perf_counter()-t0
    for be, dev in [("numpy","cpu"),("cupy","cuda"),("torch","cuda")]:
        sg_t, err = bench(GaussianMixture, {"n_components":5,"covariance_type":"diag","random_state":42}, X, dev)
        spd = sk_t/sg_t if sg_t>0 else 0
        results[f"gmm_{s}_{be}"] = {"time":sg_t, "external":sk_t, "speedup":spd}
        print(f"  {s}/{be}: {sg_t:.4f}s spd={spd:.1f}x", flush=True)

# ===========================================================================
# NMF
# ===========================================================================
print("\n=== NMF ===", flush=True)
for s, (n, p) in SCALES.items():
    X = make_nmf(n, min(p,50)); nc = 10
    sk = SkNMF(n_components=nc, max_iter=100, random_state=42); t0=time.perf_counter(); sk.fit(X); sk_t=time.perf_counter()-t0
    for be, dev in [("numpy","cpu"),("cupy","cuda"),("torch","cuda")]:
        sg_t, err = bench(NMF, {"n_components":nc,"max_iter":100,"random_state":42}, X, dev)
        spd = sk_t/sg_t if sg_t>0 else 0
        results[f"nmf_{s}_{be}"] = {"time":sg_t, "external":sk_t, "speedup":spd}
        print(f"  {s}/{be}: {sg_t:.4f}s spd={spd:.1f}x", flush=True)

# ===========================================================================
# TruncatedSVD (GPU may fail)
# ===========================================================================
print("\n=== TruncatedSVD ===", flush=True)
for s, (n, p) in SCALES.items():
    X = make_reduction(n, min(p,50)); nc = 10
    sk = SkTSVD(n_components=nc, random_state=42); t0=time.perf_counter(); sk.fit(X); sk_t=time.perf_counter()-t0
    for be, dev in [("numpy","cpu"),("cupy","cuda"),("torch","cuda")]:
        sg_t, err = bench(TruncatedSVD, {"n_components":nc,"random_state":42}, X, dev)
        spd = sk_t/sg_t if sg_t>0 else 0
        results[f"tsvd_{s}_{be}"] = {"time":sg_t, "external":sk_t, "speedup":spd}
        status = f"spd={spd:.1f}x" if not err else f"FAIL: {err}"
        print(f"  {s}/{be}: {sg_t:.4f}s {status}", flush=True)

# ===========================================================================
# IncrementalPCA (default batch_size=n_samples for fast single fit)
# ===========================================================================
print("\n=== IncrementalPCA ===", flush=True)
for s, (n, p) in SCALES.items():
    X = make_reduction(n, min(p,50)); nc = 10
    sk = SkIPCA(n_components=nc); t0=time.perf_counter(); sk.fit(X); sk_t=time.perf_counter()-t0
    for be, dev in [("numpy","cpu"),("cupy","cuda"),("torch","cuda")]:
        sg_t, err = bench(IncrementalPCA, {"n_components":nc}, X, dev)
        spd = sk_t/sg_t if sg_t>0 else 0
        results[f"ipca_{s}_{be}"] = {"time":sg_t, "external":sk_t, "speedup":spd}
        status = f"spd={spd:.1f}x" if not err else f"FAIL: {err}"
        print(f"  {s}/{be}: {sg_t:.4f}s {status}", flush=True)

# ===========================================================================
# AgglomerativeClustering (skip large)
# ===========================================================================
print("\n=== AgglomerativeClustering ===", flush=True)
for s, (n, p) in [("small",(1000,20)),("medium",(10000,50))]:
    X = make_clustering(n, p, 5)
    sk = SkAgglo(n_clusters=5); t0=time.perf_counter(); sk.fit(X); sk_t=time.perf_counter()-t0
    for be, dev in [("numpy","cpu"),("cupy","cuda"),("torch","cuda")]:
        sg_t, err = bench(AgglomerativeClustering, {"n_clusters":5}, X, dev)
        spd = sk_t/sg_t if sg_t>0 else 0
        results[f"agglo_{s}_{be}"] = {"time":sg_t, "external":sk_t, "speedup":spd}
        status = f"spd={spd:.1f}x" if not err else f"FAIL: {err}"
        print(f"  {s}/{be}: {sg_t:.4f}s {status}", flush=True)

# ===========================================================================
# UMAP (skip numpy medium/large - too slow)
# ===========================================================================
print("\n=== UMAP ===", flush=True)
for s, (n, p) in [("small",(1000,20)),("medium",(10000,50))]:
    X = make_reduction(n, p)
    for be, dev in [("numpy","cpu"),("cupy","cuda"),("torch","cuda")]:
        if be == "numpy" and s == "medium":
            print(f"  {s}/{be}: SKIP (too slow)", flush=True)
            continue
        sg_t, err = bench(UMAP, {"n_components":2}, X, dev)
        results[f"umap_{s}_{be}"] = {"time":sg_t}
        status = f"{sg_t:.4f}s" if not err else f"FAIL: {err}"
        print(f"  {s}/{be}: {status}", flush=True)

# ===========================================================================
# TSNE (skip numpy medium - too slow, skip GPU - backend bug)
# ===========================================================================
print("\n=== TSNE ===", flush=True)
for s, (n, p) in [("small",(1000,20))]:
    X = make_reduction(n, p)
    for be, dev in [("numpy","cpu")]:
        sg_t, err = bench(TSNE, {"n_components":2,"max_iter":250}, X, dev)
        results[f"tsne_{s}_{be}"] = {"time":sg_t}
        status = f"{sg_t:.4f}s" if not err else f"FAIL: {err}"
        print(f"  {s}/{be}: {status}", flush=True)

# ===========================================================================
# MiniBatchKMeans
# ===========================================================================
print("\n=== MiniBatchKMeans ===", flush=True)
for s, (n, p) in SCALES.items():
    X = make_clustering(n, min(p,50), 8)
    for be, dev in [("numpy","cpu"),("cupy","cuda"),("torch","cuda")]:
        try:
            if dev != "cpu":
                if be == "cupy":
                    import cupy; X_dev = cupy.asarray(X)
                else:
                    X_dev = torch.tensor(X, device='cuda', dtype=torch.float64)
                m = MiniBatchKMeans(n_clusters=8, random_state=42, device=dev)
                t0=time.perf_counter(); m.fit(X_dev); sg_t=time.perf_counter()-t0
            else:
                sg_t, err = bench(MiniBatchKMeans, {"n_clusters":8,"random_state":42}, X, dev)
            results[f"mbkmeans_{s}_{be}"] = {"time":sg_t}
            print(f"  {s}/{be}: {sg_t:.4f}s", flush=True)
        except Exception as e:
            print(f"  {s}/{be}: FAIL - {e}", flush=True)

# ===========================================================================
# MiniBatchNMF
# ===========================================================================
print("\n=== MiniBatchNMF ===", flush=True)
for s, (n, p) in SCALES.items():
    X = make_nmf(n, min(p,50)); nc = 10
    for be, dev in [("numpy","cpu"),("cupy","cuda"),("torch","cuda")]:
        sg_t, err = bench(MiniBatchNMF, {"n_components":nc,"random_state":42}, X, dev)
        results[f"mbnmf_{s}_{be}"] = {"time":sg_t}
        status = f"{sg_t:.4f}s" if not err else f"FAIL: {err}"
        print(f"  {s}/{be}: {status}", flush=True)

# ===========================================================================
# DBSCAN (skip large - OOM)
# ===========================================================================
print("\n=== DBSCAN ===", flush=True)
for s, (n, p) in [("small",(1000,20)),("medium",(10000,50))]:
    X = make_clustering(n, p, 5)
    sk = SkDBSCAN(eps=1.0, min_samples=5); t0=time.perf_counter(); sk.fit(X); sk_t=time.perf_counter()-t0
    for be, dev in [("numpy","cpu"),("cupy","cuda"),("torch","cuda")]:
        sg_t, err = bench(DBSCAN, {"eps":1.0,"min_samples":5}, X, dev)
        spd = sk_t/sg_t if sg_t>0 else 0
        results[f"dbscan_{s}_{be}"] = {"time":sg_t, "external":sk_t, "speedup":spd}
        status = f"spd={spd:.1f}x" if not err else f"FAIL: {err}"
        print(f"  {s}/{be}: {sg_t:.4f}s {status}", flush=True)

with open("/root/statgpu/results_unsupervised_full.json", "w") as f:
    json.dump(results, f, indent=2, default=str)

print(f"\nTotal: {len(results)} results")
print("DONE")
