...
"""Canonical v3 parsers for PR #126 post-rank-df P100 evidence.
...
def parse_panel_stage_c_rank_df_physical_validation(
    filepath: Path, env_id: str
) -> tuple[list[dict], list[dict], list[str]]:
    data = json.loads(filepath.read_text(encoding="utf-8"))
    warnings: list[str] = []
    if int(data.get("schema_version", -1)) != 1:
        raise ValueError("PR126 rank-df Stage-C validation requires schema_version=1")
    if data.get("git_sha") != _MEASUREMENT_SHA:
        raise ValueError("PR126 rank-df Stage-C validation measurement SHA drifted")
    if data.get("working_tree_clean") is not True:
        raise ValueError("PR126 rank-df Stage-C validation requires a clean measurement tree")
    if data.get("status") != "success":
        raise ValueError("PR126 rank-df Stage-C validation source did not succeed")
    environment = data.get("environment", {})
    if environment.get("gpu") != _EXPECTED_GPU:
        raise ValueError("PR126 rank-df Stage-C validation GPU identity drifted")
    ...
    if int(data.get("case_count_per_backend", -1)) != len(_EXPECTED_CASES):
        raise ValueError("PR126 rank-df Stage-C estimator case count drifted")
    if int(data.get("public_primitive_count_per_backend", -1)) != len(_EXPECTED_PRIMITIVES):
        raise ValueError("PR126 rank-df Stage-C public primitive count drifted")
    ...
    if set(backends) != {"cupy", "torch"}:
        raise ValueError("PR126 rank-df Stage-C validation requires exactly CuPy and Torch")
    ...
    for backend in ("cupy", "torch"):
        ...
        if set(cases) != _EXPECTED_CASES:
            raise ValueError(f"{backend}: PR126 rank-df estimator case identity drifted")
        if set(primitives) != _EXPECTED_PRIMITIVES:
            raise ValueError(f"{backend}: PR126 rank-df public primitive identity drifted")
        ...
        for case_name in sorted(_EXPECTED_CASES):
            ...
            runs.append(
                {
                    "run_id": "",
                    "benchmark_session_id": f"{env_id}-panel-stage-c-pr126-rank-df-validation",
                    "env_id": env_id,
                    "category_ids": ["panel"],
                    "model_id": model_id,
                    "case_id": _stable_id(
                        "case", "stage-c-rank-df", case_name, scale["scale_key"]
                    ),
                    "method_config_id": _stable_id(
                        "method", "stage-c-rank-df-physical", case_name
                    ),
                    "variant": f"rank-df-{case_name.replace('_', '-')}",
                    "penalty": None,
                    ...
                    "scale": dict(scale),
                    "parameters": {
                        "metric_scope": "rank_df_physical_validation",
            ...
            runs.append(
                {
                    ...
                    "case_id": _stable_id(
                        "case", "stage-c-rank-df-primitive", primitive_name, scale["scale_key"]
                    ),
                    "method_config_id": _stable_id(
                        "method", "stage-c-rank-df-public-primitive", primitive_name
                    ),
                    "variant": f"rank-df-public-{primitive_name.replace('_', '-')}",
                    "penalty": None,
                    "solver": "physical_validation",
                    "solver_display": "Physical validation",
                    "solver_kind": "internal",
                    ...
                    "scale": dict(scale),
                    "parameters": {
                        "metric_scope": "rank_df_public_primitive_physical_validation",
                        "measurement_git_sha": data.get("git_sha"),
                        "working_tree_clean": True,
    ...
    if len(runs) != expected_rows:
        raise ValueError(
            f"PR126 rank-df validation parser expected {expected_rows} rows, got {len(runs)}"
        )
    return runs, _models(model_ids), warnings


def parse_panel_stage_c_rank_df_performance(
    filepath: Path, env_id: str
) -> tuple[list[dict], list[dict], list[str]]:
    data = json.loads(filepath.read_text(encoding="utf-8"))
    if int(data.get("schema_version", -1)) != 2:
        raise ValueError("PR126 rank-df Stage-C performance requires schema_version=2")
    if data.get("git_sha") != _MEASUREMENT_SHA:
        raise ValueError("PR126 rank-df Stage-C performance measurement SHA drifted")
    if data.get("working_tree_clean") is not True:
        raise ValueError("PR126 rank-df Stage-C performance requires a clean measurement tree")
    if data.get("benchmark") != "panel_stage_c_covariance_fit_overhead":
        raise ValueError("PR126 rank-df Stage-C performance benchmark identity drifted")
    if data.get("timing_scope") != "synchronized end-to-end estimator fit":
        raise ValueError("PR126 rank-df Stage-C performance timing scope drifted")
    if data.get("high_t_scale") != "10000x2x200":
        raise ValueError("PR126 rank-df Stage-C high-T scale drifted")
    ...
    rows = data.get("rows", [])
    if len(rows) != 58:
        raise ValueError(f"PR126 rank-df Stage-C performance requires 58 rows, got {len(rows)}")
    if {row.get("backend") for row in rows} != {"cupy", "torch"}:
        raise ValueError("PR126 rank-df Stage-C performance requires CuPy and Torch rows")
    ...
    if len(actual_base) != len(expected_base) or set(actual_base) != expected_base:
        raise ValueError("PR126 rank-df Stage-C performance base matrix drifted")
    ...
    if len(actual_high_t) != len(expected_high_t) or set(actual_high_t) != expected_high_t:
        raise ValueError("PR126 rank-df Stage-C performance high-T QS matrix drifted")
    ...
    for row in rows:
        ...
        if scenario not in {"base", "high_t_qs"}:
            raise ValueError(f"unknown PR126 rank-df Stage-C scenario: {scenario!r}")
        ...
        if repeats <= 0 or not isinstance(samples, list) or len(samples) != repeats:
            raise ValueError("PR126 rank-df Stage-C timing samples/repeats contract failed")
        numeric_samples = [float(value) for value in samples]
        if any(not math.isfinite(value) or value <= 0.0 for value in numeric_samples):
            raise ValueError("PR126 rank-df Stage-C timing samples must be finite and positive")
        median = float(row.get("median_seconds"))
        if not math.isfinite(median) or median <= 0.0:
            raise ValueError("PR126 rank-df Stage-C timing median must be finite and positive")
        ...
        if not math.isclose(median, expected_median, rel_tol=1e-12, abs_tol=1e-15):
            raise ValueError("PR126 rank-df Stage-C reported median does not match raw samples")
        ...
        output.append(
            {
                "run_id": "",
                "benchmark_session_id": f"{env_id}-panel-stage-c-pr126-rank-df-performance",
                "env_id": env_id,
                "category_ids": ["panel"],
                "model_id": model_id,
                "case_id": _stable_id(
                    "case", "stage-c-rank-df-performance", case_name, scenario,
                    n_samples, n_features, n_times,
                ),
                "method_config_id": _stable_id(
                    "method", "stage-c-rank-df-performance", case_name
                ),
                "variant": (
                    f"rank-df-{case_name.replace('_', '-')}-{scenario.replace('_', '-')}"
                ),
                "penalty": None,
