...
ID namespaces so the fresh ``5f0cea92`` evidence can coexist with the
...
def parse_panel_stage_c_final_physical_validation(
    ...
    if int(data.get("schema_version", -1)) != 2:
        raise ValueError("PR126 final Stage-C validation requires schema_version=2")
    if data.get("git_sha") != _MEASUREMENT_SHA:
        raise ValueError("PR126 final Stage-C validation measurement SHA drifted")
    if data.get("working_tree_clean") is not True:
        raise ValueError("PR126 final Stage-C validation requires a clean measurement tree")
    if data.get("status") != "success":
        raise ValueError("PR126 final Stage-C validation source did not succeed")
    environment = data.get("environment", {})
    if environment.get("gpu") != _EXPECTED_GPU:
        raise ValueError("PR126 final Stage-C validation GPU identity drifted")
    packages = environment.get("packages", {})
    if packages.get("cupy") != _EXPECTED_CUPY_VERSION:
        raise ValueError("PR126 final Stage-C CuPy provenance drifted")
    if packages.get("torch") != _EXPECTED_TORCH_VERSION:
        raise ValueError("PR126 final Stage-C Torch provenance drifted")
    if int(data.get("case_count_per_backend", -1)) != len(_EXPECTED_CASES):
        raise ValueError("PR126 final Stage-C estimator case count drifted")
    if int(data.get("public_primitive_count_per_backend", -1)) != len(_EXPECTED_PRIMITIVES):
        raise ValueError("PR126 final Stage-C public primitive count drifted")
    ...
    if set(backends) != {"cupy", "torch"}:
        raise ValueError("PR126 final Stage-C validation requires exactly CuPy and Torch")
    ...
    for backend in ("cupy", "torch"):
        ...
        for case_name in sorted(_EXPECTED_CASES):
            ...
            runs.append(
                {
                    "run_id": "",
                    "benchmark_session_id": f"{env_id}-panel-stage-c-pr126-final-validation",
                    "env_id": env_id,
                    "category_ids": ["panel"],
                    "model_id": model_id,
                    "case_id": _stable_id(
                        "case", "stage-c-final", case_name, scale["scale_key"]
                    ),
                    "method_config_id": _stable_id(
                        "method", "stage-c-identifiability-physical", case_name
                    ),
                    "variant": f"final-{case_name.replace('_', '-')}",
                    "penalty": None,
            ...
            runs.append(
                {
                    ...
                    "case_id": _stable_id(
                        "case", "stage-c-final-primitive", primitive_name, scale["scale_key"]
                    ),
                    "method_config_id": _stable_id(
                        "method", "stage-c-final-public-primitive", primitive_name
                    ),
                    "variant": f"final-public-{primitive_name.replace('_', '-')}",
                    "penalty": None,
                    "solver": "physical_validation",
                    "solver_display": "Physical validation",
                    "solver_kind": "internal",
...
def parse_panel_stage_c_final_performance(
    ...
    if int(data.get("schema_version", -1)) != 3:
        raise ValueError("PR126 final Stage-C performance requires schema_version=3")
    if data.get("git_sha") != _MEASUREMENT_SHA:
        raise ValueError("PR126 final Stage-C performance measurement SHA drifted")
    if data.get("working_tree_clean") is not True:
        raise ValueError("PR126 final Stage-C performance requires a clean measurement tree")
    if data.get("benchmark") != "panel_stage_c_covariance_fit_overhead":
        raise ValueError("PR126 final Stage-C performance benchmark identity drifted")
    if data.get("timing_scope") != "synchronized end-to-end estimator fit":
        raise ValueError("PR126 final Stage-C performance timing scope drifted")
    if data.get("high_t_scale") != "10000x2x200":
        raise ValueError("PR126 final Stage-C high-T scale drifted")
    if data.get("two_way_unbalanced_scale") != "10000x2x20":
        raise ValueError("PR126 final Stage-C two-way scale drifted")
    ...
    if gpu_by_backend != {"cupy": _EXPECTED_GPU, "torch": _EXPECTED_GPU}:
        raise ValueError("PR126 final Stage-C performance GPU provenance drifted")
    performance_packages = performance_environment.get("packages", {})
    if performance_packages.get("cupy") != _EXPECTED_CUPY_VERSION:
        raise ValueError("PR126 final Stage-C performance CuPy provenance drifted")
    if performance_packages.get("torch") != _EXPECTED_TORCH_VERSION:
        raise ValueError("PR126 final Stage-C performance Torch provenance drifted")

    rows = data.get("rows", [])
    if len(rows) != 60:
        raise ValueError(f"PR126 final Stage-C performance requires 60 rows, got {len(rows)}")
    if {row.get("backend") for row in rows} != {"cupy", "torch"}:
        raise ValueError("PR126 final Stage-C performance requires CuPy and Torch rows")
    ...
    if len(actual_base) != len(expected_base) or set(actual_base) != expected_base:
        raise ValueError("PR126 final Stage-C performance base matrix drifted")
    ...
    if len(actual_high_t) != len(expected_high_t) or set(actual_high_t) != expected_high_t:
        raise ValueError("PR126 final Stage-C performance high-T QS matrix drifted")
    ...
    if len(actual_two_way) != len(expected_two_way) or set(actual_two_way) != expected_two_way:
        raise ValueError(
            "PR126 final Stage-C performance two-way matrix drifted"
        )
    ...
    for row in rows:
        ...
        output.append(
            {
                "run_id": "",
                "benchmark_session_id": f"{env_id}-panel-stage-c-pr126-final-performance",
                "env_id": env_id,
                "category_ids": ["panel"],
                "model_id": model_id,
                "case_id": _stable_id(
                    "case", "stage-c-final-performance", case_name, scenario,
                    n_samples, n_features, n_times,
                ),
                "method_config_id": _stable_id(
                    "method", "stage-c-final-performance", case_name
                ),
                "variant": (
                    f"final-{case_name.replace('_', '-')}-{scenario.replace('_', '-')}"
                ),
                "penalty": None,
