#!/usr/bin/env python3
"""Apply the final PR #79 review fixes to the checked-out branch.

This is a temporary, assertion-heavy patcher used because the execution
session cannot clone GitHub directly. It is deleted after the fix commit.
"""
...
def replace_once(path, old, new):
    ...
    if count != 1:
        raise RuntimeError(
            f"{path}: expected one literal match, found {count}: {old[:100]!r}"
        )
    p.write_text(text.replace(old, new, 1))


def patch_linear_regression():
    path = "statgpu/linear_model/wrappers/_linear.py"
    sub_once(
        path,
        r'''            # Handle CuPy/Torch inputs safely \(CuPy 13\+ forbids implicit asarray\)\n.*?            except TypeError:\n                X_arr = _to_numpy\(X\)\n''',
...
def patch_pooled_ols():
    path = "statgpu/panel/_pooled.py"
    sub_once(
        path,
        r'''def _panel_lstsq\(X, y, xp\):\n.*?\n\nclass PooledOLS''',
...
''',
    ...
    sub_once(
        path,
        r'''        # OLS: use rank-revealing solver for stability with near-singular designs\n.*?                                cluster=cluster\)\n''',
...
''',
    ...
    replace_once(path, "        df = n - k\n", "        df = df_resid\n")


def patch_orchestrator():
    path = "dev/validation/pr79_gpu_orchestrator.py"
...
'''
    if text.count(raw_old) != 1:
        raise RuntimeError(f"{path}: run_raw command block mismatch")
...
'''
    if text.count(remote_old) != 1:
        raise RuntimeError(f"{path}: run_remote command block mismatch")
    p.write_text(text.replace(remote_old, remote_new, 1))
...
''',
    ...
    replace_once(path, '        for wt in ["base", "head"]:\n', '        for wt in ["head"]:\n')
    sub_once(
        path,
        r'''        # Step 2: Use existing repo or clone\n.*?            timeout=120\n        \)\n''',
...
def write_tests():
    Path("dev/tests/test_pr79_final_review_fixes.py").write_text(
        r'''"""Regression tests for the final PR #79 review-fix cycle."""
