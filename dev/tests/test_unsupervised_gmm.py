...
def test_gmm_diag_basic_interface_and_sklearn_score_alignment():
    sk_mixture = pytest.importorskip("sklearn.mixture")
    X = _make_gmm_data()

    sg = GaussianMixture(n_components=3, random_state=7, max_iter=80, tol=1e-5, device="cpu").fit(X)
    ...
    assert sg.covariances_.shape == (3, 2)
    assert sg.precisions_cholesky_.shape == (3, 2)
    ...
    assert np.isclose(sg.score(X), sk.score(X), rtol=0.05, atol=0.1)
...
def test_gmm_validation_errors():
    ...
    with pytest.raises(NotImplementedError, match="covariance_type"):
        GaussianMixture(n_components=2, covariance_type="full", device="cpu").fit(X)
...
def test_gmm_cuda_torch_score_matches_cpu_when_available():
    X = _make_gmm_data(seed=17)
    cpu = GaussianMixture(n_components=3, random_state=11, max_iter=30, device="cpu").fit(X)
    ...
    gpu = GaussianMixture(n_components=3, random_state=11, max_iter=30, device="cuda").fit(X)
    assert np.isclose(gpu.score(X), cpu.score(X), rtol=1e-5, atol=1e-5)

    torch = pytest.importorskip("torch")
    if not torch.cuda.is_available():
        pytest.skip("Torch CUDA not available")
    tgpu = GaussianMixture(n_components=3, random_state=11, max_iter=30, device="torch").fit(X)
