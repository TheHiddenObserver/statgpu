...
def test_canonical_inventory_v2_publishes_audited_catalog_snapshot() -> None:
    ...
    assert len(registered) == inventory["registered_sources"] == 13
...
def test_serialized_inventory_omits_legacy_aliases() -> None:
    ...
    assert serialized["available_registered_sources"] == 13
    assert serialized["parsed_registered_sources"] == 13
    ...
    assert inventory["available_sources"] == 13
    assert inventory["parsed_sources"] == 13
