...
def test_post_selection_ols_refits_active_set_with_student_t_reporting():
    X, y = _data()
    model = Lasso(
        alpha=0.05,
        inference_method="post_selection_ols",
    ...
    assert model._inference_result.df == pytest.approx(float(df_resid))
    ...
    assert model._inference_result.metadata["active_set_tolerance"] == 1e-15
    assert model._inference_result.metadata["inactive_inference_placeholders"] is True
...
def test_post_selection_ols_intercept_only_uses_compatibility_placeholders():
...
def test_post_selection_active_tolerance_preserves_numeric_dust_as_unselected():
    ...
    np.testing.assert_array_equal(model._conf_int[0], 0.0)
...
def test_post_selection_torch_cpu_numerical_kernel_matches_numpy():
    torch = pytest.importorskip("torch")
...
def test_auto_auto_can_preserve_native_backend_but_explicit_device_wins(monkeypatch):
