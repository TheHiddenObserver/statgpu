...
def test_hessian(backend="numpy", seed=42, n=200, p=4, K=4, link_name="logit"):
...
from statgpu.linear_model._ordered_probit import OrderedProbitRegression
    ...
    if backend == "torch":
        import torch
        cls = OrderedLogitRegression if link_name == "logit" else OrderedProbitRegression
    m = cls(n_categories=K, compute_inference=False, max_iter=50, device="torch")
        m.fit(X, y)
        coef = m.coef_
        thresh = m._thresh_est
        link_cls = {"logit": LogitLink, "probit": ProbitLink}[link_name]
    fam = Binomial(link=link_cls())
        ...
        import cupy as cp
        cls = OrderedLogitRegression if link_name == "logit" else OrderedProbitRegression
    m = cls(n_categories=K, compute_inference=False, max_iter=50, device="cuda")
        m.fit(X, y)
        coef = m.coef_
        thresh = m._thresh_est
        link_cls = {"logit": LogitLink, "probit": ProbitLink}[link_name]
    fam = Binomial(link=link_cls())
        ...
        cls = OrderedLogitRegression if link_name == "logit" else OrderedProbitRegression
    m = cls(n_categories=K, compute_inference=False,
                                    max_iter=50)
        m.fit(X, y)
        coef = m.coef_
        thresh = m._thresh_est
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument("--backend", default="numpy", choices=["numpy","cupy","torch"])
    parser.add_argument("--link", default="logit", choices=["logit","probit"])
    args = parser.parse_args()
    passed, diff = test_hessian(args.backend, link_name=args.link)
