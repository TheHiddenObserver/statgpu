...
def test_quantile_cv_context_installer_is_idempotent_signature_safe_and_unset():
    before_fit = PenalizedGLM_CV._fit_standard
    ...
    assert PenalizedGLM_CV._fit_standard is before_fit
    ...
    assert contract._INTERNAL_CV_RESOLVED_SOLVER.get() is False
    assert getattr(before_fit, contract._CV_FIT_VALIDATE_MARKER, False) is True
