...
- Standardized Fama-MacBeth inference aliases/results without changing backend-native public arrays, reused rank-revealing SVD factors, replaced the retained full training design with a zero-length device anchor, and documented its small-period GPU performance caveat with synchronized evidence.
- Added maintained Python/R external-definition checks plus physical CuPy/Torch validation. Historical P100 artifacts remain audit evidence, but the latest numerical/lifecycle repairs reopen exact-head acceptance until the Stage-C, focused Fama-MacBeth, and Pooled-HAC chronology GPU runners all pass on one final clean source.
