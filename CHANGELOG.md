...
- Closed existing weighted smooth-GLM L-BFGS consumers in penalized fitting and `PenalizedGLM_CV`, including the family-specific CV L2 rows that already dispatch Gamma/Inverse-Gaussian/Negative-Binomial through L-BFGS. Hosted coverage includes all maintained ordinary family/link rows (including no-intercept rows except the reviewed inverse-Gamma limitation), warning-as-error solver characterization, statsmodels weighted Binomial/Poisson alignment, formula/refit/provenance contracts, and the frozen schema-v3 physical CUDA validator. Schema v3 preserves the frozen numerical thresholds while adding NumPy/CuPy/Torch physical parity, solver/backend/device provenance, and CV selected-alpha identity for Negative-Binomial/Gamma/Inverse-Gaussian penalized/CV L-BFGS consumers. Physical CuPy/Torch acceptance remains a required promotion gate and is not implied by hosted CI.
