...
- **Numerical hardening**: CuPy `maximum.at`/`scatter_max` return `inf` for float64 magnitudes around 1e7..1e308, so group min/max scatter now uses the sequential host scatter; Torch CUDA SVD uses the exact `gesvd` driver (the default `gesvdj` leaks ~1e-16 into structurally-zero `U` entries that huge responses amplify); failed panel fits retain the executed backend provenance; Student-t(1) p-values use the well-conditioned `2 atan(1/x)/pi` form so extreme statistics keep their representable tail; formula side-array alignment fails closed on over-long inputs.
