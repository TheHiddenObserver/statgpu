...
- Hardened fixed-effect convergence and level prediction, PanelOLS formula/refit/intercept semantics, rank-deficient inference, FirstDifference time validation, covariance numerical stability, and Fama-MacBeth chronology/formula/rank/inference/refit behavior.
- Standardized Fama-MacBeth inference aliases/results without changing backend-native public arrays, streamlined its rank-revealing period solve to reuse SVD factors without an unused covariance bread, and expanded maintained NumPy/Torch plus focused physical coverage to both Newey-West and nonrobust inference modes.
- Added maintained Python/R external-definition checks plus physical CuPy/Torch correctness and synchronized performance validation; historical Stage-C v5 acceptance passed 47/47 checks per backend and a 60-row performance matrix, and the later Fama-MacBeth correctness source `464b587e83b234d78b5449666488d7f2f8ad367c` passed both the 35+12 Stage-C matrix and focused CuPy/Torch checks. Subsequent Fama-MacBeth numerical/inference fixes reopen exact-head physical GPU acceptance until a fresh clean-head rerun; the focused runner now records full output/inference parity and same-workload NumPy/GPU timing ratios with explicit optimization notes rather than claiming universal GPU speedup.
