...
- Hardened Cox/CV cleanup, packed-target provenance, prepared/fast-path integrity, fold-level metadata reuse, strict hazard-ratio exponentiation, backend-consistent prediction shapes, stable fit parameters, public numerical errors, robust-inference unit/rank/variance gates, shared inference/backend paths, and one-sync concordance tiling; inactive legacy kernels remain test-only.
