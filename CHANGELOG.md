...
- Kept weighted non-Gaussian sparse GLM routes on their existing loss-specific sample-weight-aware solver paths; the Gaussian weighted-centering/sqrt-weight working transform is gated strictly to squared-error L1/ElasticNet fits.
- Added focused Python 3.9/3.12, Torch-CPU, full CPU, sklearn-maintenance, static/ruff, documentation, and browser regression coverage plus an exact-head physical CuPy/Torch CUDA validator. Physical CUDA acceptance is recorded against an immutable source SHA in PR evidence; hosted checks do not substitute for it. No GPU performance claim is made.
