...
- Reconciled ordinary `GeneralizedLinearModel` with the maintained analytic-weight solver contract: explicit `solver="newton"` and `solver="lbfgs"` now accept genuine non-uniform `sample_weight` on supported GLM rows without silently substituting IRLS/FISTA or changing an explicit CPU/CuPy/Torch device request.
- Extended shared L-BFGS only for losses that explicitly opt into non-uniform analytic weights. Maintained `GLMLoss` rows use one normalized `sum(w_i * loss_i) / sum(w_i)` objective for initial gradients, line-search objectives/candidates, and accepted-point gradients; global positive weight rescaling, uniform-weight identity, zero-weight-row equivalence, and integer row replication are covered by regression tests. Generic Huber/Quantile/Cox direct non-uniform weighted L-BFGS remains fail-closed.
- Preserved existing `solver="auto"`, IRLS/FISTA, explicit smooth-solver `C`, Ordered GLM, and standalone LogisticRegression contracts while adding fit-recorded ordinary-GLM solver/backend/device provenance. Genuine non-uniform weighted inverse-power Gamma is supported for explicit Newton/L-BFGS when an intercept supplies a maintained family-valid interior start; the corresponding genuine non-uniform weighted no-intercept row fails closed precisely, while uniform/effectively-uniform weights retain the historical unweighted path.
- Closed existing weighted smooth-GLM L-BFGS consumers in penalized fitting and `PenalizedGLM_CV`, including the family-specific CV L2 rows that dispatch Gamma/Inverse-Gaussian/Negative-Binomial through L-BFGS. Hosted coverage includes all maintained ordinary family/link rows (including no-intercept rows except the reviewed inverse-Gamma limitation), warning-as-error solver characterization, statsmodels weighted Binomial/Poisson alignment, formula/refit/provenance contracts, and the frozen schema-v3 physical CUDA validator.
- Exact-source physical CUDA validation **passed** at numerical/validator head `c6781cb6a2e1fe500f325e832d23cdc80a99b564` on Tesla P100-SXM2-16GB with CuPy 13.6.0, Torch 2.0.0+cu117, and NumPy 1.24.2. The run covered 48 ordinary NumPy/CuPy/Torch Newton/L-BFGS routes, 4 Torch↔CuPy cross-container routes, and 9 penalized/CV L-BFGS consumer routes; the maximum ordinary GPU-vs-NumPy absolute error was `4.44e-16`. The original raw validator JSON is retained at `dev/reviews/pr151_glm_weighted_explicit_solvers_gpu.json`; it is the same blob originally added in evidence commit `0b2a1478c9ad5d4c82d510f15143ba3078015618` and was moved out of `results/` without content changes so correctness evidence is not scanned as a benchmark source. Its `source_sha` remains `c6781cb6a2e1fe500f325e832d23cdc80a99b564`. The evidence placement does not change the numerical implementation, tests, or validator, so the P100 run remains implementation evidence for `c6781cb6` rather than a new exact-current-head physical run.

### PR #147 / Issue #145 — Backend-native Gaussian residual bootstrap (targeted for 0.2.6)
- Extended the existing unweighted Gaussian `residual_bootstrap` row from NumPy-only execution to the fit-recorded NumPy/CuPy/Torch backend and concrete device, with one deterministic backend-neutral residual-index schedule and fail-closed backend/device provenance checks.
- Child refits preserve the fitted penalty family, tuning, intercept, solver/stopping, Lipschitz, and SCAD/MCP LLA controls while keeping child inference disabled. `PenalizedGLM_CV` continues to run bootstrap only once on the selected full-data final refit and reports conditioning on the CV-selected penalty without selection adjustment.
- Weighted, robust/HC, HAC/block, non-Gaussian, and Cox bootstrap semantics remain fail-closed. Large design/response/residual/bootstrap-response arrays and child optimization stay on the recorded numerical backend/device; reporting remains a final NumPy boundary, with only small control/parameter snapshots allowed to cross it.
- Added deterministic hosted coverage for NumPy preservation, L1/ElasticNet/SCAD/MCP, formula/public-wrapper consumers, exact-device child contexts, failure transactions, installer idempotence, and unsupported rows. `dev/benchmarks/validate_gaussian_residual_bootstrap_gpu.py` schema v1 is the frozen exact-source CuPy/Torch CUDA gate; physical CUDA acceptance remains required before PR #147 can be marked Ready or merged.
...
- Routed normal/Student-t inference through the maintained reference-distribution layer, including stable df=1/df=2 extreme-tail handling, and made missing/invalid executed-backend provenance fail closed instead of silently falling back to NumPy.
