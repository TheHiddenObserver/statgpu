...
- Hardened Cox/CV cleanup, packed-target provenance, prepared-state integrity, fold-level metadata reuse, strict hazard-ratio exponentiation, public numerical errors, truthful summaries, shared inference/backend paths, and one-sync concordance tiling; inactive legacy kernels remain test-only.
