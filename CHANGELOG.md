...
- Preserved existing `solver="auto"`, IRLS/FISTA, explicit smooth-solver `C`, Ordered GLM, and standalone LogisticRegression contracts while adding fit-recorded ordinary-GLM solver/backend/device provenance. Weighted inverse-power Gamma is supported for explicit Newton/L-BFGS when an intercept supplies a maintained family-valid interior start; the new weighted no-intercept row fails closed precisely rather than relying on data-dependent initialization luck.
- Closed existing weighted smooth-GLM L-BFGS consumers in penalized fitting and `PenalizedGLM_CV`, including the family-specific CV L2 rows that already dispatch Gamma/Inverse-Gaussian/Negative-Binomial through L-BFGS. Hosted coverage includes all maintained ordinary family/link rows, warning-as-error solver characterization, statsmodels weighted Binomial/Poisson alignment, formula/refit/provenance contracts, and a frozen schema-v2 physical CUDA validator. Physical CuPy/Torch acceptance remains a required promotion gate and is not implied by hosted CI.
