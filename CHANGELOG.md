...
- Exact-source physical CUDA validation **passed** at numerical/validator head `c6781cb6a2e1fe500f325e832d23cdc80a99b564` on Tesla P100-SXM2-16GB with CuPy 13.6.0, Torch 2.0.0+cu117, and NumPy 1.24.2. The run covered 48 ordinary NumPy/CuPy/Torch Newton/L-BFGS routes, 4 Torch↔CuPy cross-container routes, and 9 penalized/CV L-BFGS consumer routes; the maximum ordinary GPU-vs-NumPy absolute error was `4.44e-16`. The raw validator JSON was generated on the validation host at `results/pr151_glm_weighted_explicit_solvers_gpu/pr151_glm_weighted_explicit_solvers_gpu.json`; it is not currently committed to this repository and is not a GitHub Actions artifact. Subsequent PR #151 commits are documentation-only, so this P100 run remains implementation evidence for `c6781cb6` rather than exact-current-head evidence.
