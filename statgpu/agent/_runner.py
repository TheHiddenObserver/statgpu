...
# ---------------------------------------------------------------------------
# Self-correcting runner (GeneAgent pattern)
# ---------------------------------------------------------------------------

class SelfCorrectingRunner:
    """GeneAgent-style self-correcting runner with pruning + safety net."""

    def __init__(self, config: AgentConfig):
        self.config = config
        self.pruner = MethodPruner()

    def run(self, prepared, task_type: str) -> Tuple[List["ModelResult"], List[dict]]:
        """Run models with pruning and self-correction loop."""
        from ._analysis import ModelResult

        # Stage 1: Proactive pruning
        available_methods = self.pruner.prune(prepared, task_type, self.config)

        # Stage 2: Fit + self-correction safety net
        trace: List[dict] = []
        models: List[ModelResult] = []

        for round_num in range(self.config.max_correction_rounds):
            models = self._fit_with_methods(prepared, task_type, available_methods)
            issues = self._diagnose(models, prepared, task_type)

            if not issues:
                break  # All good

            correction = self._plan_correction(issues, prepared, available_methods)
            trace.append({
                "round": round_num,
                "issues": issues,
            ...
            # Apply correction
            available_methods = correction.get("methods", available_methods)

        return models, trace

    def _fit_with_methods(self, prepared, task_type: str,
                          methods: List[str]) -> List["ModelResult"]:
        """Fit models using specified methods."""
        models = []
            ...
            if entry is not None:
                try:
                    ...
                    if task_type == "survival":
                        ...
                        # Determine score_args based on task
                        ...
                        result = fit_supervised_model(
                            ...
                            fit_args=fit_args, score_args=score_args,
                ...
                # Handle special methods not in registry
                if method_name == "PCA(diagnostic)":
    ...
    def _diagnose(self, models, prepared, task_type: str) -> List[str]:
        ...
        for model in models:
            if model.error:
                if "singular" in str(model.error).lower() or "rank" in str(model.error).lower():
                    issues.append("rank_deficient")
                elif "separation" in str(model.error).lower():
                    issues.append("separation")
                elif "convergence" in str(model.error).lower():
                    issues.append("non_convergence")
        return issues

    def _plan_correction(self, issues: List[str], prepared,
                         current_methods: List[str]) -> dict:
        """Plan correction based on diagnosed issues."""
        n, p = prepared.X.shape

        if "rank_deficient" in issues:
            # Switch to Ridge
            new_methods = [m for m in current_methods if m != "LinearRegression"]
            if "Ridge(alpha=1.0)" not in new_methods:
                new_methods.insert(0, "Ridge(alpha=1.0)")
            return {"action": "switch_to_ridge", "methods": new_methods}

        if "separation" in issues:
            # Already handled by LogisticRegression self-correction
            return {"action": "already_corrected", "methods": current_methods}

        return {"action": "unknown", "methods": current_methods}
