...
from statgpu.backends import _is_cupy_array, _is_torch_array, _to_float_scalar, get_backend, xp_asarray
...
from statgpu.survival._cox_fit_adapter import _is_native_backend_array, _normalize_boolean_control, _normalize_mutable_fit_controls, _PreencodedCoxLabels
from statgpu.survival._cox_errors import CoxFitNumericalError
from statgpu.survival._cox_counting import _make_prepared_counting_process_inputs, _score_test_statistic, prepare_right_censored_cox_fast_path
from statgpu.survival._cox_inference import _classify_covariance_spectrum, _invert_information_cupy, _invert_information_numpy, _invert_information_torch, _joint_wald_from_covariance, _standard_errors_from_covariance, _validate_robust_inference_units
from statgpu.survival._numeric import _normalize_prediction_matrix, _safe_exp_linear_predictor
...
def _align_cox_side_array(values, retained_rows, original_n, name='array'):
    ...
    if _is_cupy_array(values) or _is_torch_array(values):
        ...
        if n_values != original_n:
            raise ValueError(f'{name} length {n_values} does not match original data length {original_n}')
        is_torch = _is_torch_array(values)
        target_device = 'cpu' if is_torch and str(getattr(getattr(values, 'device', None), 'type', 'cpu')) == 'cpu' else 'cuda'
        backend = get_backend('torch' if is_torch else 'cupy', device=target_device)
        idx = xp_asarray(retained_rows, dtype=backend.int64, xp=backend.xp, ref_arr=values)
        return values[idx]
...
class CoxPH(BaseEstimator):
    ...
    _DEFERRED_SET_PARAMS = frozenset({'compute_inference', 'compute_cindex', 'gpu_memory_cleanup'})
    ...
    def __sklearn_tags__(self):
        ...
        return Tags(estimator_type='regressor', target_tags=TargetTags(required=True, one_d_labels=False, two_d_labels=True, multi_output=True, single_output=False), regressor_tags=RegressorTags())

    def __init__(self, ties: str='breslow', tol: float=1e-09, max_iter: int=100, device: Union[str, Device]=Device.AUTO, n_jobs: Optional[int]=None, compute_inference: bool=True, compute_cindex: bool=True, cov_type: str='nonrobust', gpu_memory_cleanup: bool=False, penalty: float=0.0, inference_mode: str='strict'):
        for name, value in (('compute_inference', compute_inference), ('compute_cindex', compute_cindex), ('gpu_memory_cleanup', gpu_memory_cleanup)):
        ...
        try:
            ...
            raise ValueError('penalty must be a finite non-negative number') from exc
        self.ties = ties
        self.tol = tol
        ...
        if isinstance(max_iter, (bool, np.bool_)) or not isinstance(max_iter, numbers.Integral) or int(max_iter) < 1:
    ...
    def _validate_optimization_controls(self):
        ...
        if isinstance(self._max_iter, (bool, np.bool_)) or not isinstance(self._max_iter, numbers.Integral) or int(self._max_iter) < 1:
            raise ValueError('max_iter must be a positive integer')
        try:
            tol = float(self._tol)
            penalty = float(self.penalty)
        except (TypeError, ValueError) as exc:
            raise ValueError('tol and penalty must be finite numeric values') from exc
        if not np.isfinite(tol) or tol <= 0:
            raise ValueError('tol must be a finite positive number')
        if not np.isfinite(penalty) or penalty < 0:
            raise ValueError('penalty must be a finite non-negative number')

    def fit(self, X=None, time=None, event=None, entry=None, cluster=None, init_coef=None, formula=None, data=None, *, start=None, strata=None, subject_id=None, _right_censored_prepared=None):
        ...
        try:
            ...
            if formula is None and event is None and (time is not None):
                ...
                if target.ndim != 2 or target.shape[1] not in (2, 3):
                    raise ValueError('When event is omitted, time must be a survival target with columns [time, event] or [start, stop, event]')
                if target.shape[1] == 2:
                    time, event = (target[:, 0], target[:, 1])
                else:
                    if entry is not None or start is not None:
                        raise ValueError('Do not pass entry/start separately when the target already has [start, stop, event] columns')
                    start, time, event = (target[:, 0], target[:, 1], target[:, 2])
            if _right_censored_prepared is not None:
                if formula is not None or not _right_censored_prepared.matches_sources(X, time, event, controls.ties):
                    raise ValueError('prepared right-censored metadata does not match the current matrix fit inputs')
                if _right_censored_prepared.requires_content_validation and (not _right_censored_prepared.matches_content(X, time, event, controls.ties)):
                    raise ValueError('prepared right-censored metadata does not match dataset contents')
            result = self._fit_impl(X=X, time=time, event=event, entry=entry, cluster=cluster, init_coef=init_coef, formula=formula, data=data, start=start, strata=strata, subject_id=subject_id, _right_censored_prepared=_right_censored_prepared)
            if not self._is_counting_process:
                self._entry = None
            coef = np.asarray(self.coef_, dtype=np.float64)
            if not np.all(np.isfinite(coef)) or not np.isfinite(self._log_likelihood):
                raise CoxFitNumericalError('CoxPH fit produced non-finite coefficients or log-likelihood')
            if controls.compute_inference and any((value is None or not np.all(np.isfinite(value)) for value in (self._bse, self._pvalues, self._conf_int))):
                raise FloatingPointError('CoxPH inference produced non-finite standard errors, p-values, or confidence intervals')
            return result
        except Exception:
            self._reset_fit_state()
            raise

    def _fit_impl(self, X=None, time=None, event=None, entry=None, cluster=None, init_coef=None, formula=None, data=None, *, start=None, strata=None, subject_id=None, _right_censored_prepared=None):
        """
        ...
        if formula is not None:
            ...
            y_patsy, X_patsy = patsy.dmatrices(formula, formula_data, eval_env=custom_env, return_type='dataframe')
            retained_rows = np.asarray(X_patsy.index, dtype=np.int64)
            n_original = len(data)
            entry = _align_cox_side_array(entry, retained_rows, n_original, 'entry/start')
            cluster = _align_cox_side_array(cluster, retained_rows, n_original, 'cluster')
            strata = _align_cox_side_array(strata, retained_rows, n_original, 'strata')
            subject_id = _align_cox_side_array(subject_id, retained_rows, n_original, 'subject_id')
            design_info = X_patsy.design_info
            y_arr = np.asarray(y_patsy)
            if y_arr.ndim == 1:
                raise ValueError("Formula response must be Surv(time, event), not a single variable. Use: formula='Surv(time, event) ~ x1 + x2'")
            if y_arr.shape[1] == 2:
                time = y_arr[:, 0]
                event = y_arr[:, 1]
            elif y_arr.shape[1] == 3:
                if formula_entry_was_explicit:
                    raise ValueError('Surv(start, stop, event) already defines entry times; do not also pass entry= or start=')
                entry = y_arr[:, 0]
                time = y_arr[:, 1]
                event = y_arr[:, 2]
            else:
                raise ValueError('Formula response must be Surv(time, event) or Surv(start, stop, event)')
            ...
            if 'Intercept' in self._feature_names:
                ...
                self._feature_names = [name for index, name in enumerate(self._feature_names) if index != intercept_index]
            self._design_info = design_info
            X = X_arr
        else:
            if X is None or time is None or event is None:
                raise ValueError('Either formula+data or X+time+event must be provided.')
            self._design_info = None
        _require_real_array(X, 'X')
        _require_real_array(time, 'time')
        _require_real_array(event, 'event')
        ...
        _require_real_array(init_coef, 'init_coef')
        self._fit_call = {'interface': 'formula' if formula is not None else 'matrix', 'formula': None if formula is None else str(formula), 'counting_process': entry is not None or subject_id is not None, 'stratified': strata is not None, 'subject_grouped': subject_id is not None, 'clustered': cluster is not None, 'ties': controls.ties}
        device = get_device() if controls.device == Device.AUTO else controls.device
        return self._fit_counting_process_dispatch(X, time, event, entry=entry, strata=strata, cluster=cluster, subject_id=subject_id, init_coef=init_coef, device=device, right_censored_prepared=_right_censored_prepared)
    ...
    @staticmethod
    def _encode_group_labels(values, n_samples, name, *, return_labels=True):
        ...
        if module.startswith('torch'):
            ...
            if getattr(values, 'ndim', None) != 1 or int(values.shape[0]) != n_samples:
                raise ValueError(f'{name} must have shape (n_samples,)')
            if (values.is_floating_point() or values.is_complex()) and bool(torch.any(~torch.isfinite(values)).item()):
                raise ValueError(f'{name} must contain only finite labels')
            labels, encoded = torch.unique(values, sorted=True, return_inverse=True)
            labels_host = labels.detach().cpu().numpy() if return_labels else None
        ...
        return (encoded.astype(np.int64, copy=False), labels if return_labels else None)

    def _fit_counting_process_dispatch(self, X, time, event, *, entry, strata, cluster, subject_id, init_coef, device, right_censored_prepared=None):
        ...
        from statgpu.survival._risk_sets import counting_process_concordance, prepare_counting_process_inputs
        controls = self._fit_controls
        if controls is None:
            raise RuntimeError('CoxPH fit controls were not initialized')
        ...
        input_full_host_transfer = any((_is_device_resident_array(value) for value in (X, time, event, entry, strata, cluster, subject_id))) and device == Device.CPU
        strata_encoded, strata_labels = self._encode_group_labels(strata, n_samples, 'strata')
        cluster_encoded, _ = self._encode_group_labels(cluster, n_samples, 'cluster', return_labels=False)
        subject_encoded, _ = self._encode_group_labels(subject_id, n_samples, 'subject_id', return_labels=False)
        if controls.ties == 'exact' and controls.compute_inference and (controls.cov_type != 'nonrobust'):
            raise NotImplementedError("robust covariance is not yet defined for ties='exact'; use cov_type='nonrobust'")
        backend_name = {Device.CPU: 'numpy', Device.CUDA: 'cupy', Device.TORCH: 'torch'}[device]
        compute_backend = self._get_backend(backend=backend_name)
        backend = compute_backend.name
        self._fitted_backend_name = backend
        self.effective_device_ = {'numpy': 'cpu', 'cupy': 'cuda', 'torch': 'torch'}[backend]
        ...
        startb = compute_backend.zeros(stopb.shape, dtype=compute_backend.float64) if entry is None else compute_backend.asarray(entry, dtype=compute_backend.float64)
        stratab = compute_backend.zeros((n_samples,), dtype=compute_backend.int64) if strata_encoded is None else compute_backend.asarray(strata_encoded, dtype=compute_backend.int64)
        clusterb = None if cluster_encoded is None else compute_backend.asarray(cluster_encoded, dtype=compute_backend.int64)
        subjectb = None if subject_encoded is None else compute_backend.asarray(subject_encoded, dtype=compute_backend.int64)
        if Xb.ndim == 1:
            Xb = Xb.reshape(-1, 1)
        if entry is None and bool(_to_float_scalar(xp.any(stopb <= 0))):
            raise ValueError('time must contain only positive values')
        Xb, stopb, eventb, startb, stratab = prepare_counting_process_inputs(Xb, stopb, eventb, start=startb, strata=stratab)
        right_censored_fast_path = entry is None and strata is None and (subject_id is None) and (controls.cov_type == 'nonrobust') and (controls.ties in {'breslow', 'efron'})
        if right_censored_prepared is not None and (not right_censored_fast_path):
            raise ValueError('prepared right-censored metadata is incompatible with this fit')
        if right_censored_fast_path and right_censored_prepared is None:
            right_censored_prepared = prepare_right_censored_cox_fast_path(Xb, stopb, eventb, ties=controls.ties)
        prepared_inputs = _make_prepared_counting_process_inputs(Xb, stopb, eventb, startb, stratab, right_censored=right_censored_prepared)
        result = fit_counting_process_cox(Xb, stopb, eventb, start=startb, strata=stratab, ties=controls.ties, penalty=controls.penalty, tol=controls.tol, max_iter=controls.max_iter, init_coef=init_coef, compute_baseline=controls.compute_inference, compute_score_residuals=controls.compute_inference and controls.cov_type != 'nonrobust', _prepared_inputs=prepared_inputs)
        to_numpy = compute_backend.to_numpy
        scalar = _to_float_scalar
        self.coef_ = to_numpy(result['coef']).astype(np.float64, copy=False)
        self.hazard_ratios_ = _safe_exp_linear_predictor(self.coef_, error_type=CoxFitNumericalError, name='fitted Cox coefficients')
        ...
        self._objective_history = np.asarray([scalar(value) for value in result['objective_history']], dtype=np.float64)
        self._nobs = n_samples
        self._nevents = int(scalar(eventb.sum()))
        self._entry = None if entry is None else to_numpy(startb)
        self._strata = None if strata is None else to_numpy(stratab).astype(np.int64, copy=False)
        ...
        if controls.penalty > 0:
            identity = compute_backend.eye(information.shape[0], dtype=information.dtype)
        ...
        if controls.compute_inference:
            ...
            if controls.cov_type != 'nonrobust':
                if controls.cov_type == 'cluster':
                    if clusterb is None:
                        raise ValueError("cluster ids are required when cov_type='cluster'")
                    unit_codes = clusterb
                else:
                    unit_codes = subjectb
                if unit_codes is None:
                    ...
                    unique_units, inverse = xp.unique(unit_codes, return_inverse=True)
                    n_units = int(unique_units.shape[0])
                correction = _validate_robust_inference_units(controls.cov_type, n_units, int(Xb.shape[1]))
            if backend == 'torch':
            ...
            if controls.cov_type == 'nonrobust':
                ...
                if unit_codes is None:
                    ...
                    unit_scores = xp.zeros((n_units, residuals.shape[1]), dtype=residuals.dtype, device=residuals.device)
                    unit_scores.index_add_(0, inverse, residuals)
                else:
                    unit_scores = xp.zeros((n_units, residuals.shape[1]), dtype=residuals.dtype)
            ...
            covariance_spectrum = _classify_covariance_spectrum(self._var_matrix)
            self._bse = _standard_errors_from_covariance(self._var_matrix, cov_type=controls.cov_type, spectrum=covariance_spectrum)
            self._zvalues = self.coef_ / (self._bse + 1e-30)
            self._pvalues = 2.0 * norm.sf(np.abs(self._zvalues))
            ci_quantile = float(norm.ppf(0.975))
            self._conf_int = np.column_stack([self.coef_ - ci_quantile * self._bse, self.coef_ + ci_quantile * self._bse])
            self._lr_test_stat = 2.0 * (self._log_likelihood - self._log_likelihood_null)
            self._lr_test_pvalue = chi2.sf(self._lr_test_stat, df=int(Xb.shape[1]))
            wald_stat, wald_failure = _joint_wald_from_covariance(self.coef_, self._var_matrix, cov_type=controls.cov_type, spectrum=covariance_spectrum)
            self._wald_test_stat = wald_stat
            self.wald_test_available_ = wald_failure is None
            self.wald_test_failure_reason_ = wald_failure
            self._wald_test_pvalue = chi2.sf(wald_stat, df=int(Xb.shape[1])) if self.wald_test_available_ else np.nan
            score0 = result['null_score']
            score_stat, score_failure = _score_test_statistic(score0, result['null_information'], backend, xp)
            ...
            self._score_test_pvalue = chi2.sf(self._score_test_stat, df=int(Xb.shape[1]))
        else:
            self._var_matrix = None
            self._bse = None
            self._zvalues = None
        ...
        if result['baseline'] is None:
            ...
            baseline_by_stratum = {int(key): {name: to_numpy(value).astype(np.float64, copy=False) for name, value in baseline.items()} for key, baseline in result['baseline'].items()}
            if len(baseline_by_stratum) == 1:
                ...
                self._baseline_log_cumulative_hazard = baseline.get('log_cumulative_hazard')
                self._baseline_log_cumulative_hazard_centered = baseline.get('log_cumulative_hazard_centered')
                self._baseline_x_reference = baseline.get('x_reference')
                self._baseline_by_stratum = None if strata is None and entry is None and (subject_id is None) else baseline_by_stratum
            else:
        ...
        if controls.compute_cindex:
            self._cindex = scalar(counting_process_concordance(result['coef'], Xb, stopb, eventb, start=startb, strata=stratab, subject_id=subjectb))
        else:
            self._cindex = None
        ...
        self._final_kkt_normalized = score_inf / (1.0 + raw_score_inf + 2.0 * controls.penalty * beta_inf)
        ...
        self.full_host_transfer_performed_ = bool(input_full_host_transfer or result.get('full_target_host_transfer_performed', False) or (backend != 'numpy' and any((value is not None for value in (entry, strata, subject_id)))))
        if controls.compute_inference:
        ...
        if not self._converged:
            import warnings
            warnings.warn(f'CoxPH did not converge after {self._iterations} iterations (stop_reason={self._stop_reason})', RuntimeWarning, stacklevel=2)
        if controls.penalty > 0:
            self._lr_test_stat = None
            self._lr_test_pvalue = None
            self._score_test_stat = None
            self._score_test_pvalue = None
            self.score_test_available_ = False
            self.score_test_failure_reason_ = 'classical score test is suppressed for penalized fit'
        self._fitted = True
    ...
    def _require_classical_information_criterion(self, name):
        self._check_is_fitted()
        fitted_penalty = self._fit_controls.penalty if self._fit_controls is not None else float(self.penalty)
        if fitted_penalty > 0:
            raise RuntimeError(f'{name} is only defined here for an unpenalized CoxPH fit; the penalized estimate is not the partial-likelihood MLE')
    ...
    def bic(self):
        ...
        return float(-2.0 * self._log_likelihood + np.log(max(int(self._nevents), 1)) * len(self.coef_))

    def _format_fit_call(self):
        """Return only fitted-call details that the estimator can guarantee."""
        call = self._fit_call or {'interface': 'matrix', 'formula': None, 'counting_process': bool(self._is_counting_process), 'stratified': self._strata is not None, 'subject_grouped': self._subject_id is not None, 'clustered': self._cov_type == 'cluster', 'ties': self._ties}
        parts = []
        if call['interface'] == 'formula':
            parts.append(f"formula={call['formula']!r}")
        else:
            parts.append("interface='matrix'")
        parts.extend([f"ties={call['ties']!r}", f"counting_process={bool(call['counting_process'])}", f"stratified={bool(call['stratified'])}", f"subject_grouped={bool(call['subject_grouped'])}", f"clustered={bool(call['clustered'])}"])
    ...
    def summary(self):
        ...
        controls = self._fit_controls
        fitted_cov_type = controls.cov_type if controls is not None else str(self._cov_type)
        fitted_compute_inference = controls.compute_inference if controls is not None else bool(self._compute_inference_enabled)
        fitted_penalty = controls.penalty if controls is not None else float(self.penalty)
        print('=' * 80)
        ...
        if fitted_compute_inference and self._bse is not None:
            ...
            for i, name in enumerate(self._feature_names):
                print(f'{name:<15} {self.coef_[i]:>10.4f} {self.hazard_ratios_[i]:>12.4f} {self._bse[i]:>10.4f} {self._zvalues[i]:>10.3f} {self._pvalues[i]:>10.4f}')
            print('-' * 80)
            print(f"{'':<15} {'exp(coef)':>12} {'exp(-coef)':>12} {'lower .95':>12} {'upper .95':>12}")
            print('-' * 80)
            for i, name in enumerate(self._feature_names):
                hr = self.hazard_ratios_[i]
                inverse_hr = _safe_exp_linear_predictor(np.asarray([-self.coef_[i]]), name='inverse Cox coefficient')[0]
                interval_hr = _safe_exp_linear_predictor(self._conf_int[i], name='Cox confidence interval')
                print(f'{name:<15} {hr:>12.4f} {inverse_hr:>12.4f} {interval_hr[0]:>12.4f} {interval_hr[1]:>12.4f}')
        else:
        ...
        if fitted_compute_inference and self._lr_test_stat is not None:
            print(f'Classical likelihood-ratio test: {self._lr_test_stat:.2f} on {len(self.coef_)} df, p={self._lr_test_pvalue:.4e}')
            wald_label = 'Robust Wald test' if fitted_cov_type in {'hc0', 'hc1', 'cluster'} else 'Classical Wald test'
            if self.wald_test_available_:
                print(f'{wald_label}: {self._wald_test_stat:.2f} on {len(self.coef_)} df, p={self._wald_test_pvalue:.4e}')
            else:
            ...
            if self.score_test_available_:
                print(f'Classical score (logrank) test: {self._score_test_stat:.2f} on {len(self.coef_)} df, p={self._score_test_pvalue:.4e}')
            else:
                print(f"Classical score (logrank) test unavailable: {self.score_test_failure_reason_ or 'null information is singular'}")
        elif fitted_compute_inference and fitted_penalty > 0:
            print('Penalized coefficient inference: fixed-penalty frequentist estimating-equation sandwich; CV selection and shrinkage bias are not included.')
            if self.wald_test_available_:
                print(f'Penalized estimating-equation Wald test: {self._wald_test_stat:.2f} on {len(self.coef_)} df, p={self._wald_test_pvalue:.4e}')
            else:
                print(f"Penalized estimating-equation Wald test unavailable: {self.wald_test_failure_reason_ or 'covariance is rank-deficient'}")
            print('Classical LR/Score/AIC/BIC diagnostics suppressed for the penalized fit.')
        else:
    ...
    def _prepare_prediction_X(self, X):
        ...
        backend = get_backend(backend=backend_name, device='cpu' if backend_name == 'numpy' else 'cuda')
        n_features = int(len(self.coef_))
        X_arr = _normalize_prediction_matrix(X, backend=backend, n_features=n_features)
        return (X_arr, backend, backend.asarray(self.coef_, dtype=backend.float64))

    def _encode_prediction_strata(self, strata, *, n_samples, backend, context, required=False, known_codes=None):
        """Validate and encode row-level strata at prediction/score boundaries."""
        if strata is None:
            if required:
                action = 'predicting from' if context == 'prediction' else context
                raise ValueError(f'strata is required when {action} a stratified CoxPH fit')
            return None
        if self._strata_labels is None:
            codes, _ = self._encode_group_labels(strata, n_samples, 'strata', return_labels=False)
            ...
            try:
                ...
                raise ValueError(f'unknown {context} stratum: {exc.args[0]!r}') from exc
    ...
    def predict_survival(self, X, times=None, strata=None):
        ...
        if baselines is None and self._unique_times is not None and (self._baseline_cumulative_hazard is not None):
            ordinary_baseline = {'time': self._unique_times, 'cumulative_hazard': self._baseline_cumulative_hazard}
            if self._baseline_log_cumulative_hazard_centered is not None and self._baseline_x_reference is not None:
                ordinary_baseline.update({'log_cumulative_hazard_centered': self._baseline_log_cumulative_hazard_centered, 'x_reference': self._baseline_x_reference})
        ...
        if not explicitly_stratified:
            ...
            codes = self._encode_prediction_strata(strata, n_samples=n_samples, backend=backend, context='prediction', required=True, known_codes=baselines)
        if times is None:
        ...
        for code, baseline in baselines.items():
            ...
            if 'log_cumulative_hazard_centered' in baseline and 'x_reference' in baseline:
                ...
                risk = xp.exp(backend.minimum(log_risk, float(np.log(np.finfo(np.float64).max))))
            else:
                positive = cumulative > 0
                safe_cumulative = xp.where(positive, cumulative, xp.ones_like(cumulative))
                log_base = xp.where(positive, xp.log(safe_cumulative), xp.full_like(cumulative, -float('inf')))
                log_risk = log_base[None, :] + (X_arr[rows] @ coef)[:, None]
                risk = xp.exp(backend.minimum(log_risk, float(np.log(np.finfo(np.float64).max))))
    ...
    def score(self, X, time, event=None, start=None, strata=None, subject_id=None):
        ...
        return _score_impl(self, X, time, event=event, start=start, strata=strata, subject_id=subject_id)
