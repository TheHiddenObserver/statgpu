...
from statgpu.survival._cox_counting import _is_singular_linalg_error, _solve as _solve_counting_information
from statgpu.survival._cox_inference import _invert_information_cupy, _invert_information_numpy, _invert_information_torch
...
def _estimate_breslow_tensor_bytes(n, p, n_groups, itemsize=8):
    ...
    elements = 2 * int(n) * int(p) * int(p) + int(n) * int(p) + 3 * int(n_groups) * int(p) * int(p) + 2 * int(n_groups) * int(p)
...
_USE_NUMBA = os.environ.get('STATGPU_USE_NUMBA', '0').strip().lower() in ('1', 'true', 'yes', 'on')
...
if _USE_NUMBA:
    try:
        ...
        @njit(cache=True)
        def _efron_backward_scan_numba(X, e_linpred, risk_sum, risk_X_sum, first_idx_uft, fail_ptr, fail_ind, nuft, n, p):
...
def _efron_backward_scan_python(X, e_linpred, risk_sum, risk_X_sum, first_idx_uft, uft_ix, nuft, n, p):
    ...
    for g in range(nuft - 1, -1, -1):
        ...
        if enter_end > enter_start:
            ...
            xp2 += X[enter_start:enter_end].T @ (X[enter_start:enter_end] * e_linpred[enter_start:enter_end, None])
        ix_ev = uft_ix[g]
        d = len(ix_ev)
        if d == 0:
            continue
...
def _efron_backward_scan_vectorized(X, e_linpred, risk_sum, risk_X_sum, first_idx_uft, uft_ix, nuft, n, p):
...
class _LegacyCoxReferenceMixin:
    ...
    def _fit_cpu(self, X, time, event, entry=None, cluster=None, init_coef=None):
        ...
        if entry is not None:
            self._fit_counting_process_dispatch(X, time, event, entry=np.asarray(entry, dtype=np.float64), strata=None, cluster=cluster, subject_id=None, init_coef=init_coef, device=Device.CPU)
            return
        ...
        if self.ties == 'efron':
            ...
            try:
                ...
                self._efron_all_singletons = bool(nuft > 0) and all((len(ix) == 1 for ix in uft_ix))
            except Exception:
                self._efron_all_singletons = False
        else:
            self._efron_all_singletons = False
            self._breslow_pre = self._breslow_unique_failure_groups(time_sorted, event_sorted)
            if entry_sorted is not None:
                ...
                self._entry_fail_groups_np = [event_idx_np[inv_np == g].astype(np.int64, copy=False) for g in range(len(uft_np))]
                ...
                self._entry_add_end_np = np.searchsorted(entry_sorted, uft_np, side='left').astype(np.int64, copy=False)
                self._entry_rem_end_np = np.searchsorted(time_sorted, uft_np, side='left').astype(np.int64, copy=False)
            else:
        ...
        self._log_likelihood_null = self._compute_log_likelihood(np.zeros(n_features), X_sorted, time_sorted, event_sorted, self._efron_pre, entry=entry_sorted)
        penalty = float(self.penalty)
        use_penalty = penalty > 0.0
        identity = np.eye(n_features, dtype=np.float64)
        kkt_tol = max(self.tol * 0.001, 1e-09)
        objective_tol = 1e-10
        self._termination_reason = 'max_iter'
        iteration = -1
        current_obj = self._compute_log_likelihood(beta, X_sorted, time_sorted, event_sorted, self._efron_pre) - penalty * float(beta @ beta)
        self._objective_history = [float(current_obj)]
        for iteration in range(self.max_iter):
            grad_data, hess_data = self._compute_gradient_hessian(beta, X_sorted, time_sorted, event_sorted, self._efron_pre)
            penalized_grad = grad_data - 2.0 * penalty * beta
            kkt_inf = float(np.linalg.norm(penalized_grad, ord=np.inf))
            kkt_norm = kkt_inf / (1.0 + float(np.linalg.norm(grad_data, ord=np.inf)) + 2.0 * penalty * float(np.linalg.norm(beta, ord=np.inf)))
            if kkt_norm <= kkt_tol:
            ...
            for direction in (1.0, -1.0):
                ...
                for _ in range(21):
                    ...
                    trial_obj = self._compute_log_likelihood(trial_beta, X_sorted, time_sorted, event_sorted, self._efron_pre) - penalty * float(trial_beta @ trial_beta)
            ...
            if update_norm < max(self.tol * (1.0 + float(np.linalg.norm(beta))), 1e-08):
                trial_grad, _ = self._compute_gradient_hessian(beta, X_sorted, time_sorted, event_sorted, self._efron_pre)
                trial_pen_grad = trial_grad - 2.0 * penalty * beta
                trial_kkt_inf = float(np.linalg.norm(trial_pen_grad, ord=np.inf))
                trial_kkt_norm = trial_kkt_inf / (1.0 + float(np.linalg.norm(trial_grad, ord=np.inf)) + 2.0 * penalty * float(np.linalg.norm(beta, ord=np.inf)))
                ...
                break
        final_grad, final_hess = self._compute_gradient_hessian(beta, X_sorted, time_sorted, event_sorted, self._efron_pre)
        final_pen_grad = final_grad - 2.0 * penalty * beta
        self._final_kkt_inf = float(np.linalg.norm(final_pen_grad, ord=np.inf))
        self._final_kkt_normalized = self._final_kkt_inf / (1.0 + float(np.linalg.norm(final_grad, ord=np.inf)) + 2.0 * penalty * float(np.linalg.norm(beta, ord=np.inf)))
    ...
    def _fit_gpu(self, X, time, event, entry=None, cluster=None, init_coef=None):
        ...
        self._event_X_sum_gpu = cp.sum(X_sorted[event_idx_sorted], axis=0) if int(event_idx_sorted.size) > 0 else cp.zeros(n_features, dtype=cp.float64)
        efron_pre = None
        self._breslow_pre = None
        self._breslow_pre_gpu = None
        if self.ties == 'efron':
            if entry_sorted is None:
                efron_pre = self._efron_unique_failure_indices(cp.asnumpy(time_sorted), cp.asnumpy(event_sorted))
                ...
                try:
                    ...
                    enter_ptr, enter_ind, exit_ptr, exit_ind, fail_ptr, fail_ind = efron_indices_to_csr(uft_ix, risk_enter, risk_exit, nuft)
                    self._efron_pre_csr = (enter_ptr, enter_ind, exit_ptr, exit_ind, fail_ptr, fail_ind, first_idx_uft, nuft)
                    self._efron_pre_csr_gpu = (cp.asarray(enter_ptr, dtype=cp.int32), cp.asarray(enter_ind, dtype=cp.int32), cp.asarray(exit_ptr, dtype=cp.int32), cp.asarray(exit_ind, dtype=cp.int32), cp.asarray(fail_ptr, dtype=cp.int32), cp.asarray(fail_ind, dtype=cp.int32), cp.asarray(first_idx_uft, dtype=cp.int32), int(nuft))
                except Exception:
            ...
            self._efron_pre_csr = None
            self._efron_pre_csr_gpu = None
            first_idx_uft, counts_uft = self._breslow_unique_failure_groups(cp.asnumpy(time_sorted), cp.asnumpy(event_sorted))
            self._breslow_pre = (first_idx_uft, counts_uft)
            self._breslow_pre_gpu = (cp.asarray(first_idx_uft, dtype=cp.int32), cp.asarray(counts_uft, dtype=cp.int32))
        ...
        if entry_sorted is not None:
            ...
            entry_ctx_gpu = (_ctx[0], _ctx[1], _ctx[2], _ctx[3], cp.ascontiguousarray(X_sorted[_ctx[0]]), cp.ascontiguousarray(X_sorted), event_idx_ctx, cp.sum(X_sorted[event_idx_ctx], axis=0), _ctx[6])
        loglik_null_gpu = self._compute_log_likelihood_gpu(cp.zeros(n_features, dtype=cp.float64), X_sorted, time_sorted, event_sorted, efron_pre, entry=entry_sorted, entry_ctx=entry_ctx_gpu)
        ...
        eye_cache = cp.eye(n_features, dtype=cp.float64) if self.compute_inference or use_penalty else None
        loglik_gpu = None
        current_obj = None
        iteration = -1
        kkt_tol = max(self.tol * 0.001, 1e-09)
        ...
        for iteration in range(self.max_iter):
            grad, hess, aux_stats = self._compute_gradient_hessian_gpu(beta, X_sorted, time_sorted, event_sorted, efron_pre, return_aux=True, entry=entry_sorted, entry_ctx=entry_ctx_gpu)
            if use_penalty:
            ...
            if current_obj is None:
                current_obj = self._compute_log_likelihood_gpu_from_stats(aux_stats[0], aux_stats[1], aux_stats[2], time_sorted, event_sorted, efron_pre, entry=entry_sorted, entry_ctx=entry_ctx_gpu)
                if use_penalty:
            ...
            for direction in (-1.0, 1.0):
                ...
                for _ in range(21):
                    trial_beta = beta + direction * step * delta
                    trial_obj = self._compute_log_likelihood_gpu(trial_beta, X_sorted, time_sorted, event_sorted, efron_pre, entry=entry_sorted, entry_ctx=entry_ctx_gpu)
                    if use_penalty:
            ...
            if step_norm < max(self.tol * (1.0 + float(cp.linalg.norm(beta).item())), 1e-08):
                grad_check, hess_check, _aux_check = self._compute_gradient_hessian_gpu(beta, X_sorted, time_sorted, event_sorted, efron_pre, return_aux=True, entry=entry_sorted, entry_ctx=entry_ctx_gpu)
                if use_penalty:
                    pg = grad_check - 2 * penalty * beta
                else:
                    pg = grad_check
                kkt_check = float(cp.linalg.norm(pg, ord=cp.inf).item())
                kkt_n_check = kkt_check / (1.0 + float(cp.linalg.norm(grad_check, ord=cp.inf).item()) + 2.0 * penalty * float(cp.linalg.norm(beta, ord=cp.inf).item()))
        ...
        if self._final_kkt_inf is None:
            grad_final, hess_final, _aux_final = self._compute_gradient_hessian_gpu(beta, X_sorted, time_sorted, event_sorted, efron_pre, return_aux=True, entry=entry_sorted, entry_ctx=entry_ctx_gpu)
            if use_penalty:
            ...
            self._final_kkt_normalized = self._final_kkt_inf / (1.0 + float(cp.linalg.norm(grad_final, ord=cp.inf).item()) + 2.0 * penalty * float(cp.linalg.norm(beta, ord=cp.inf).item()))
        if self._final_kkt_normalized is not None and self._final_kkt_normalized > kkt_tol:
            if self._converged:
                self._termination_reason = 'stalled_with_large_kkt'
            self._converged = False
        final_hess = None
        if self.compute_inference:
            _, final_hess, final_aux = self._compute_gradient_hessian_gpu(beta, X_sorted, time_sorted, event_sorted, efron_pre, return_aux=True, entry=entry_sorted, entry_ctx=entry_ctx_gpu)
            if use_penalty:
                final_hess[diag_idx, diag_idx] -= 2.0 * penalty
            loglik_gpu = self._compute_log_likelihood_gpu_from_stats(final_aux[0], final_aux[1], final_aux[2], time_sorted, event_sorted, efron_pre, entry=entry_sorted)
        else:
            loglik_gpu = self._compute_log_likelihood_gpu(beta, X_sorted, time_sorted, event_sorted, efron_pre, entry=entry_sorted, entry_ctx=entry_ctx_gpu)
        self._iterations = iteration + 1
        ...
        self._penalized_objective = self._log_likelihood - penalty * float(np.dot(self.coef_, self.coef_))
        ...
        if self.compute_cindex:
            ...
            self._cindex = None
        if self.compute_inference:
            _, inference_hess = self._compute_gradient_hessian_gpu(beta, X_sorted, time_sorted, event_sorted, efron_pre, entry=entry_sorted, entry_ctx=entry_ctx_gpu)
            if use_penalty:
            ...
            if self.cov_type == 'nonrobust':
                ...
                self.inference_method_ = 'penalized_observed_information' if self.penalty > 0 else 'observed_information'
                self.inference_backend_ = 'cupy'
                self.inference_approximate_ = False
                self._var_matrix = 0.5 * (self._var_matrix + self._var_matrix.T)
    ...
    def _fit_torch(self, X, time, event, entry=None, cluster=None, torch_device='cuda', init_coef=None):
        ...
        if self.ties == 'efron':
            if entry_sorted is None:
                efron_pre = self._efron_unique_failure_indices(time_sorted.cpu().numpy(), event_sorted.cpu().numpy())
                self._efron_pre = efron_pre
                try:
                    _, uft_ix, _, _, nuft, _ = _unpack_efron_pre6(efron_pre)
                    self._efron_all_singletons = bool(nuft > 0) and all((len(ix) == 1 for ix in uft_ix))
            ...
            self._efron_pre_csr = None
            self._efron_pre_csr_gpu = None
            first_idx_uft, counts_uft = self._breslow_unique_failure_groups(time_sorted.cpu().numpy(), event_sorted.cpu().numpy())
            self._breslow_pre = (first_idx_uft, counts_uft)
            self._breslow_pre_torch = (torch.tensor(first_idx_uft, dtype=torch.int32, device=torch_device), torch.tensor(counts_uft, dtype=torch.int32, device=torch_device))
            if entry_sorted is not None:
        ...
        if entry_sorted is not None:
            ...
            event_idx_ctx = _ctx[5]
            entry_ctx_torch = (_ctx[0], _ctx[1], _ctx[2], _ctx[3], X_sorted.index_select(0, _ctx[0]).contiguous(), X_sorted.contiguous(), event_idx_ctx, torch.sum(X_sorted.index_select(0, event_idx_ctx), dim=0), _ctx[6])
        loglik_null_torch = self._compute_log_likelihood_torch(torch.zeros(n_features, dtype=torch.float64, device=torch_device), X_sorted, time_sorted, event_sorted, efron_pre, entry=entry_sorted, entry_ctx=entry_ctx_torch)
        penalty = float(self.penalty) if hasattr(self, 'penalty') else 0.0
        use_penalty = penalty > 0.0
        ...
        self._final_kkt_normalized = None
        for iteration in range(self.max_iter):
            grad, hess, aux_stats = self._compute_gradient_hessian_torch(beta, X_sorted, time_sorted, event_sorted, efron_pre, return_aux=True, entry=entry_sorted, entry_ctx=entry_ctx_torch)
            if use_penalty:
                pen_grad = grad - 2 * penalty * beta
            else:
                pen_grad = grad
            ...
            if current_obj is None:
                current_obj = self._compute_log_likelihood_torch_from_stats(aux_stats[0], aux_stats[1], aux_stats[2], time_sorted, event_sorted, efron_pre, entry=entry_sorted, entry_ctx=entry_ctx_torch)
                if use_penalty:
            ...
            for direction in (-1.0, 1.0):
                ...
                for _ in range(21):
                    trial_beta = beta + direction * step * delta
                    trial_obj = self._compute_log_likelihood_torch(trial_beta, X_sorted, time_sorted, event_sorted, efron_pre, entry=entry_sorted, entry_ctx=entry_ctx_torch)
                    if use_penalty:
            ...
            step_norm = delta_norm * accepted_step_size
            if step_norm < max(self.tol * (1.0 + float(torch.linalg.norm(beta).item())), 1e-08):
                grad_check, hess_check, _aux_check = self._compute_gradient_hessian_torch(beta, X_sorted, time_sorted, event_sorted, efron_pre, return_aux=True, entry=entry_sorted, entry_ctx=entry_ctx_torch)
                if use_penalty:
                    pg = grad_check - 2 * penalty * beta
                else:
                    pg = grad_check
                kkt_check = float(torch.linalg.norm(pg, ord=float('inf')).item())
                kkt_n_check = kkt_check / (1.0 + float(torch.linalg.norm(grad_check, ord=float('inf')).item()) + 2.0 * penalty * float(torch.linalg.norm(beta, ord=float('inf')).item()))
                if kkt_n_check <= kkt_tol:
                    self._converged = True
                    self._termination_reason = 'kkt_converged'
                    self._final_kkt_inf = kkt_check
                ...
                break
        if self._final_kkt_inf is None:
            grad_final, hess_final, _aux_final = self._compute_gradient_hessian_torch(beta, X_sorted, time_sorted, event_sorted, efron_pre, return_aux=True, entry=entry_sorted, entry_ctx=entry_ctx_torch)
            if use_penalty:
                pen_grad_final = grad_final - 2 * penalty * beta
            else:
                pen_grad_final = grad_final
            self._final_kkt_inf = float(torch.linalg.norm(pen_grad_final, ord=float('inf')).item())
            self._final_kkt_normalized = self._final_kkt_inf / (1.0 + float(torch.linalg.norm(grad_final, ord=float('inf')).item()) + 2.0 * penalty * float(torch.linalg.norm(beta, ord=float('inf')).item()))
        if self._final_kkt_normalized is not None and self._final_kkt_normalized > kkt_tol:
            if self._converged:
                self._termination_reason = 'stalled_with_large_kkt'
            self._converged = False
        final_hess = None
        if self.compute_inference:
            _, final_hess, final_aux = self._compute_gradient_hessian_torch(beta, X_sorted, time_sorted, event_sorted, efron_pre, return_aux=True, entry=entry_sorted, entry_ctx=entry_ctx_torch)
            if use_penalty:
                final_hess[diag_idx, diag_idx] -= 2.0 * penalty
            loglik_torch = self._compute_log_likelihood_torch_from_stats(final_aux[0], final_aux[1], final_aux[2], time_sorted, event_sorted, efron_pre, entry=entry_sorted)
        else:
            loglik_torch = self._compute_log_likelihood_torch(beta, X_sorted, time_sorted, event_sorted, efron_pre, entry=entry_sorted, entry_ctx=entry_ctx_torch)
        self._iterations = iteration + 1
        ...
        if self.compute_inference:
            hess = final_hess
            if self.cov_type == 'nonrobust':
                _, inference_hess = self._compute_gradient_hessian_torch(beta, X_sorted, time_sorted, event_sorted, efron_pre, entry=entry_sorted, entry_ctx=entry_ctx_torch)
                if use_penalty:
                    inference_hess[diag_idx, diag_idx] -= 2 * penalty
                ...
                self._compute_inference_cpu(X_sorted.cpu().numpy(), time_sorted.cpu().numpy(), event_sorted.cpu().numpy(), cluster_sorted.cpu().numpy() if cluster_sorted is not None else None)
            if self.compute_inference:
    ...
    def _compute_log_likelihood(self, beta, X, time, event, efron_pre=None, entry=None):
        ...
        if self.ties == 'breslow':
            if entry is not None:
                ...
                if fail_groups is None or add_end_np is None or rem_end_np is None or (order_np is None):
                    event_idx = np.flatnonzero(event_mask)
                    event_times = time[event_idx]
                    uft_np, inv_np = np.unique(event_times, return_inverse=True)
                    fail_groups = [event_idx[inv_np == g].astype(np.int64, copy=False) for g in range(len(uft_np))]
                    order_np = np.argsort(np.asarray(entry, dtype=np.float64)).astype(np.int64, copy=False)
                    add_end_np = np.searchsorted(np.asarray(entry, dtype=np.float64)[order_np], uft_np, side='left').astype(np.int64, copy=False)
            ...
            if breslow_pre is not None and len(breslow_pre) == 2 and (breslow_pre[0].size > 0):
        ...
        if efron_pre is not None:
            ...
            for g in range(nuft):
                ...
                if d == 0:
                    continue
                first_idx = int(first_idx_uft[g]) if first_idx_uft is not None else int(np.searchsorted(time, uft[g], side='left'))
    ...
    def _compute_log_likelihood_gpu(self, beta, X, time, event, efron_pre=None, entry=None, entry_ctx=None):
        ...
        return self._compute_log_likelihood_gpu_from_stats(eta, exp_eta, risk_sum, time, event, efron_pre, entry=entry, entry_ctx=entry_ctx)
    ...
    def _build_entry_ctx_gpu(self, time, event, entry, cp):
        ...
        if evt_t.size == 0:
            return (cp.zeros((0,), dtype=cp.int64), np.zeros((0,), dtype=np.float64), np.zeros((0,), dtype=np.int64), np.zeros((0,), dtype=np.int64), cp.zeros((0,), dtype=cp.int64), cp.zeros((0,), dtype=cp.int64), np.zeros((1,), dtype=np.int64))
    ...
    def _compute_log_likelihood_gpu_from_stats(self, eta, exp_eta, risk_sum, time, event, efron_pre=None, entry=None, entry_ctx=None):
        ...
        if entry is not None:
            if entry_ctx is None:
                entry_order, d_counts, add_end_np, rem_end_np, _rem_order, event_idx, fail_ptr = self._build_entry_ctx_gpu(time, event, entry, cp)
            else:
        ...
        if self.ties == 'breslow':
            ...
            if breslow_pre_gpu is not None and len(breslow_pre_gpu) == 2 and (int(breslow_pre_gpu[0].size) > 0):
            ...
            return cp.sum(eta[event_mask]) - cp.sum(counts_uft.astype(cp.float64) * cp.log(risk_at))
        ...
        if efron_pre is not None:
            try:
                ...
                if csr_gpu is not None:
                    ...
                    return compute_efron_loglik_raw_csr(eta, exp_eta, risk_sum, fail_ptr, fail_ind, first_idx_uft, nuft, cupy_module=cp)
            except Exception:
                pass
            from ._cox_efron_cuda import compute_efron_loglik_raw
            return compute_efron_loglik_raw(eta, exp_eta, risk_sum, time, efron_pre, cupy_module=cp)
    ...
    def _compute_gradient_hessian(self, beta, X, time, event, efron_pre=None, entry=None):
        ...
        if self.ties == 'breslow':
            ...
            hess = self._compute_hessian_breslow_fast(X, time, event, risk_sum, risk_X_sum, exp_eta, first_idx, counts)
        else:
            ...
            if HAS_CYTHON_EFRON and efron_pre is not None:
                try:
                    ...
                    grad, hess = _efron_grad_hess_cython(eta_efron, X, risk_enter, risk_exit, uft_ix, nuft)
                    hess = -hess
                    ...
                    uft, uft_ix, risk_enter, risk_exit, nuft, _ = _unpack_efron_pre6(efron_pre)
                    grad, hess = efron_grad_hess_python(eta_efron, X, risk_enter, risk_exit, uft_ix, nuft)
                    hess = -hess
                    if not (np.isfinite(grad).all() and np.isfinite(hess).all()):
                        grad, hess = self._compute_gradient_hessian_efron_backward(beta, X, time, event, efron_pre)
            else:
                grad, hess = self._compute_gradient_hessian_efron_backward(beta, X, time, event, efron_pre)
        return (grad, hess)

    def _compute_hessian_breslow_fast(self, X, time, event, risk_sum, risk_X_sum, exp_eta, first_idx=None, counts=None):
        ...
        estimated_bytes = _estimate_breslow_tensor_bytes(int(X.shape[0]), p, n_groups, int(X.dtype.itemsize))
        ...
        if p <= 24 and n_groups <= 512 and (estimated_bytes <= max_bytes):
            ...
            return self._compute_hessian_breslow_tensor_grouped(X, risk_sum, risk_X_sum, exp_eta, first_idx, counts)
        self._last_breslow_hessian_strategy_ = 'incremental'
        return self._compute_hessian_breslow_incremental_grouped(X, risk_sum, risk_X_sum, exp_eta, first_idx, counts)

    def _compute_hessian_breslow_tensor_grouped(self, X, risk_sum, risk_X_sum, exp_eta, first_idx, counts):
    ...
    def _compute_hessian_breslow_incremental_grouped(self, X, risk_sum, risk_X_sum, exp_eta, first_idx, counts):
    ...
    def _compute_hessian_breslow_incremental_grouped_cupy(self, X, risk_sum, risk_X_sum, exp_eta, first_idx, counts):
        ...
        estimated_bytes = _estimate_breslow_tensor_bytes(n, p, nuft, int(X.dtype.itemsize))
        max_bytes = _breslow_hessian_max_bytes()
        self._last_breslow_hessian_workspace_estimate_ = estimated_bytes
        self._last_breslow_hessian_workspace_limit_ = max_bytes
        if estimated_bytes > max_bytes:
            self._last_breslow_hessian_strategy_ = 'cupy_streaming'
            return self._compute_hessian_breslow_streaming_grouped_cupy(X, risk_sum, risk_X_sum, exp_eta, first_idx, counts)
        ...
        sc = counts / risk_at
        flat = (X_exp[:, :, None] * X[:, None, :]).reshape(n, p * p)
        prefix_flat = cp.cumsum(flat, axis=0)
    ...
    def _compute_hessian_breslow_streaming_grouped_cupy(self, X, risk_sum, risk_X_sum, exp_eta, first_idx, counts):
    ...
    def _compute_hessian_breslow(self, beta, X, time, event, risk_sum, risk_X_sum, exp_eta):
        ...
        hess = np.zeros((n_features, n_features), dtype=np.float64)
        X_exp = X * exp_eta[:, np.newaxis]
        risk_X2_sum = X_exp.T @ X
        event_positions = np.where(event)[0]
        prev_pos = 0
        for ev_i in event_positions:
            if ev_i > prev_pos:
                blk = slice(prev_pos, ev_i)
                risk_X2_sum -= X_exp[blk].T @ X[blk]
            prev_pos = ev_i
            E_X = risk_X_sum[ev_i] / risk_sum[ev_i]
            E_XX = risk_X2_sum / risk_sum[ev_i]
    ...
    def _efron_unique_failure_indices(self, time: np.ndarray, event: np.ndarray):
        ...
        group_ids = np.searchsorted(uft, ft, side='left').astype(np.int32)
        ...
        risk_enter = [idx_enter_sorted[ptr_en[i]:ptr_en[i + 1]].tolist() for i in range(nuft)]
    ...
    def _compute_gradient_hessian_gpu(self, beta, X, time, event, efron_pre=None, return_aux=False, entry=None, entry_ctx=None):
        """Compute gradient and Hessian on GPU."""
        import cupy as cp
        import time as _time
        n_samples, n_features = X.shape
        profile_breslow = os.environ.get('STATGPU_PROFILE_BRESLOW_CUDA', '0').strip().lower() in ('1', 'true', 'yes', 'on')
        ...
        if self.ties == 'efron' and entry is None:
            if getattr(self, '_efron_all_singletons', False):
                ...
                grad = grad_pre.copy() if grad_pre is not None and int(grad_pre.shape[0]) == int(n_features) else cp.sum(X[event_mask], axis=0)
                E_X = risk_X_sum[first_idx_uft] / risk_sum[first_idx_uft][:, cp.newaxis]
                grad = grad - cp.sum(E_X * counts_f[:, cp.newaxis], axis=0)
                use_fused_breslow = os.environ.get('STATGPU_BRESLOW_FUSED_CUPY', '0').strip().lower() in ('1', 'true', 'yes', 'on')
                hess = None
                if use_fused_breslow:
                    hess = self._compute_hessian_breslow_fused_cupy(X, first_idx_uft, counts_f, exp_eta)
                if hess is None:
                    hess = self._compute_hessian_breslow_incremental_grouped_cupy(X, risk_sum, risk_X_sum, exp_eta, first_idx_uft, counts_f)
                if return_aux:
                    return (grad, hess, (eta, exp_eta, risk_sum))
                return (grad, hess)
            if efron_pre is None:
                efron_pre = self._efron_unique_failure_indices(cp.asnumpy(time), cp.asnumpy(event))
            out = self._compute_gradient_hessian_efron_backward_gpu(beta, X, efron_pre)
            if return_aux:
        ...
        if entry is not None:
            if entry_ctx is None:
                entry_order, d_counts, add_end_np, rem_end_np, rem_order, event_idx, fail_ptr = self._build_entry_ctx_gpu(time, event, entry, cp)
            ...
            if s2_block_size <= 0:
                s2_block_size = 10 ** 18
            use_s2_fused = os.environ.get('STATGPU_ENTRY_S2_FUSED_CUPY', '0').strip().lower() in ('1', 'true', 'yes', 'on')
            ...
            for g in range(n_groups):
                ...
                if add_end > add_ptr:
                    ...
                    if use_s2_fused and n_add >= s2_fused_min_rows:
                        ...
                        s2 = self._s2_weighted_update_cupy_blocked(s2, x_add, w_add, s2_block_size, sign=1.0)
                    add_ptr = add_end
                rem_end = int(rem_end_np[g])
                if rem_end > rem_ptr:
                    ...
                    if use_s2_fused and n_rem >= s2_fused_min_rows:
                        ...
                        s2 = self._s2_weighted_update_cupy_blocked(s2, x_rem, w_rem, s2_block_size, sign=-1.0)
                    rem_ptr = rem_end
        ...
        if profile_breslow:
            ...
            print(f'[CUDA Breslow profile] pre={_t_pre - _t0_all:.4f}s grad={_t_grad - _t_pre:.4f}s hess={_t_hess - _t_grad:.4f}s total={_t_hess - _t0_all:.4f}s')
        if return_aux:
            return (grad, hess, (eta, exp_eta, risk_sum))
        return (grad, hess)
    ...
    def _compute_gradient_hessian_efron_backward_gpu(self, beta, X, efron_pre):
        ...
        n_features = X.shape[1]
        if nuft == 0:
            return (cp.zeros(n_features, dtype=cp.float64), cp.zeros((n_features, n_features), dtype=cp.float64))
        n_samples = int(X.shape[0])
        avg_tie = float(n_samples) / max(1.0, float(nuft))
        use_grouped_gemm = os.environ.get('STATGPU_EFRON_GROUPED_GEMM', '1').strip().lower() in ('1', 'true', 'yes', 'on')
        if use_grouped_gemm and n_features <= 192 and (avg_tie >= 24.0):
            return self._compute_gradient_hessian_efron_grouped_gemm_cupy(beta, X, efron_pre)
        try:
            from ._cox_efron_cuda import compute_efron_grad_hess_raw
            csr_gpu = getattr(self, '_efron_pre_csr_gpu', None)
            if csr_gpu is not None:
                out = compute_efron_grad_hess_raw(X, beta, efron_pre, efron_csr=csr_gpu, cupy_module=cp)
            else:
        ...
        for i in range(nuft)[::-1]:
            ...
            if len(ixf) > 0:
                ...
                hess_inner = hess_inner - (sum_aa * cp.outer(xp1, xp1) + sum_bb * cp.outer(xp1f, xp1f) - sum_ab * (cp.outer(xp1, xp1f) + cp.outer(xp1f, xp1)))
            ix = risk_exit[i]
            if len(ix) > 0:
                ix = cp.array(ix, dtype=cp.int32)
                elx = e_linpred[ix]
    ...
    def _efron_cumulative_workspace_fits(efron_pre, n_samples, n_features, itemsize, *, include_second_moments):
        ...
        if nuft == 0 or first_idx_uft is None or float(n_samples) / float(max(nuft, 1)) < 24.0:
            return False
        ...
        max_bytes = max(0, int(os.environ.get('STATGPU_EFRON_CUMULATIVE_MAX_BYTES', 512 * 1024 * 1024)))
    ...
    def _compute_gradient_hessian_efron_grouped_gemm_cupy(self, beta, X, efron_pre):
        ...
        if not self._efron_cumulative_workspace_fits(efron_pre, n_samples, n_features, int(X.dtype.itemsize), include_second_moments=True):
            return self._compute_gradient_hessian_efron_grouped_gemm_loop_cupy(beta, X, efron_pre)
        csr_gpu = getattr(self, '_efron_pre_csr_gpu', None)
        if csr_gpu is not None:
            ...
            fail_ind_np = np.asarray([row for group in uft_ix for row in group], dtype=np.int64)
        ...
        fail2 = segment_sum(fail_weighted_X[:, :, None] * fail_X[:, None, :])
        ...
        denominator = cp.maximum(risk0.reshape(-1, 1) - frac * fail0.reshape(-1, 1), 1e-300)
        ...
        grad = cp.sum(fail_X_sum - risk1 * sum_inv[:, None] + fail1 * sum_frac_inv[:, None], axis=0)
        risk_outer = risk1[:, :, None] * risk1[:, None, :]
        fail_outer = fail1[:, :, None] * fail1[:, None, :]
        cross_outer = risk1[:, :, None] * fail1[:, None, :] + fail1[:, :, None] * risk1[:, None, :]
        hess_inner = cp.sum(risk2 * sum_inv[:, None, None] - fail2 * sum_frac_inv[:, None, None] - risk_outer * sum_inv2[:, None, None] - fail_outer * sum_frac_inv2[:, None, None] + cross_outer * sum_cross[:, None, None], axis=0)
    ...
    def _efron_cumulative_indices_torch(self, efron_pre, device):
        ...
        cache = (efron_pre, device, first_idx, fail_ptr, fail_ind, counts, max_tie)
    ...
    def _compute_gradient_hessian_efron_grouped_gemm_torch(self, beta, X, efron_pre):
        ...
        import torch
        n_samples, n_features = (int(X.shape[0]), int(X.shape[1]))
        if not self._efron_cumulative_workspace_fits(efron_pre, n_samples, n_features, X.element_size(), include_second_moments=True):
            return self._compute_gradient_hessian_efron_grouped_gemm_loop_torch(beta, X, efron_pre)
        first_idx, fail_ptr, fail_ind, counts_int, max_tie = self._efron_cumulative_indices_torch(efron_pre, beta.device)
        linpred = X @ beta
        linpred = linpred - torch.max(linpred)
        weights = torch.exp(linpred)
        risk0_all = torch.flip(torch.cumsum(torch.flip(weights, (0,)), dim=0), (0,))
        weighted_X = weights[:, None] * X
        risk1_all = torch.flip(torch.cumsum(torch.flip(weighted_X, (0,)), dim=0), (0,))
        row_second = weighted_X[:, :, None] * X[:, None, :]
        risk2_all = torch.flip(torch.cumsum(torch.flip(row_second, (0,)), dim=0), (0,))
        risk0 = risk0_all[first_idx]
        risk1 = risk1_all[first_idx]
        ...
        def segment_sum(values):
            zero = torch.zeros((1,) + tuple(values.shape[1:]), dtype=values.dtype, device=values.device)
        ...
        active = steps < counts.reshape(-1, 1)
        frac = steps / counts.reshape(-1, 1)
        denominator = torch.clamp(risk0.reshape(-1, 1) - frac * fail0.reshape(-1, 1), min=1e-300)
        ...
        grad = torch.sum(fail_X_sum - risk1 * sum_inv[:, None] + fail1 * sum_frac_inv[:, None], dim=0)
        risk_outer = risk1[:, :, None] * risk1[:, None, :]
        fail_outer = fail1[:, :, None] * fail1[:, None, :]
        cross_outer = risk1[:, :, None] * fail1[:, None, :] + fail1[:, :, None] * risk1[:, None, :]
        hess_inner = torch.sum(risk2 * sum_inv[:, None, None] - fail2 * sum_frac_inv[:, None, None] - risk_outer * sum_inv2[:, None, None] - fail_outer * sum_frac_inv2[:, None, None] + cross_outer * sum_cross[:, None, None], dim=0)
        return (grad, -hess_inner)
    ...
    def _compute_gradient_hessian_efron_grouped_gemm_loop_torch(self, beta, X, efron_pre):
        ...
        for i in range(nuft - 1, -1, -1):
            ...
            if len(ixf) > 0:
                ...
                hess_inner = hess_inner + xp2 * sum_inv_c0
                hess_inner = hess_inner - xp2f * sum_J_c0
                hess_inner = hess_inner - (sum_aa * torch.outer(xp1, xp1) + sum_bb * torch.outer(xp1f, xp1f) - sum_ab * (torch.outer(xp1, xp1f) + torch.outer(xp1f, xp1)))
            ix = risk_exit[i]
            if len(ix) > 0:
                idx = torch.as_tensor(ix, dtype=torch.long, device=beta.device)
                v = X[idx]
    ...
    def _compute_log_likelihood_torch(self, beta, X, time, event, efron_pre=None, entry=None, entry_ctx=None):
        ...
        return self._compute_log_likelihood_torch_from_stats(eta, exp_eta, risk_sum, time, event, efron_pre, entry=entry, entry_ctx=entry_ctx)
    ...
    def _build_entry_ctx_torch(self, time, event, entry, device):
        ...
        if evt_t.size == 0:
            return (torch.zeros((0,), dtype=torch.long, device=device), np.zeros((0,), dtype=np.float64), np.zeros((0,), dtype=np.int64), np.zeros((0,), dtype=np.int64), torch.zeros((0,), dtype=torch.long, device=device), torch.zeros((0,), dtype=torch.long, device=device), np.zeros((1,), dtype=np.int64))
        uft_np, d_counts = np.unique(evt_t, return_counts=True)
        d_counts = d_counts.astype(np.float64, copy=False)
    ...
    def _compute_log_likelihood_torch_from_stats(self, eta, exp_eta, risk_sum, time, event, efron_pre=None, entry=None, entry_ctx=None):
        ...
        if entry is not None:
            if entry_ctx is None:
                entry_order, d_counts, add_end_np, rem_end_np, _rem_order, event_idx, fail_ptr = self._build_entry_ctx_torch(time, event, entry, eta.device)
            else:
                entry_order, d_counts, add_end_np, rem_end_np = entry_ctx[:4]
        ...
        if self.ties == 'breslow':
            ...
            if breslow_pre_torch is not None and len(breslow_pre_torch) == 2 and (int(breslow_pre_torch[0].numel()) > 0):
            ...
            risk_at = risk_sum[first_idx_uft]
            return torch.sum(eta[event_mask]) - torch.sum(counts_uft.to(torch.float64) * torch.log(risk_at))
        if efron_pre is not None:
        ...
        if efron_pre is not None and needs_exact_ties and self._efron_cumulative_workspace_fits(efron_pre, int(eta.shape[0]), 0, eta.element_size(), include_second_moments=False):
            first_idx, fail_ptr, fail_ind, counts_int, max_tie = self._efron_cumulative_indices_torch(efron_pre, eta.device)
            risk_at = risk_sum[first_idx]
            ...
            steps = torch.arange(int(max_tie), dtype=eta.dtype, device=eta.device).reshape(1, -1)
            active = steps < counts.reshape(-1, 1)
            frac = steps / counts.reshape(-1, 1)
            denominator = torch.clamp(risk_at.reshape(-1, 1) - frac * fail_sum.reshape(-1, 1), min=1e-300)
            log_terms = torch.where(active, torch.log(denominator), torch.zeros_like(denominator))
    ...
    def _compute_gradient_hessian_torch(self, beta, X, time, event, efron_pre=None, return_aux=False, entry=None, entry_ctx=None):
        ...
        if self.ties == 'efron' and efron_pre is not None and (entry is None):
            needs_exact_ties = not getattr(self, '_efron_all_singletons', False)
            if needs_exact_ties:
                if os.environ.get('STATGPU_EFRON_TRITON', '0').strip().lower() in ('1', 'true', 'yes', 'on') and beta.is_cuda:
                    ...
                    if triton_out is not None:
                        ...
                        if return_aux:
                            return (grad, hess, (eta, exp_eta, risk_sum))
                        return (grad, hess)
                out = self._compute_gradient_hessian_efron_grouped_gemm_torch(beta, X, efron_pre)
                if return_aux:
                    return (out[0], out[1], (eta, exp_eta, risk_sum))
                return out
            if os.environ.get('STATGPU_EFRON_TRITON', '0').strip().lower() in ('1', 'true', 'yes', 'on') and beta.is_cuda:
        ...
        event_mask = event == 1
        if not torch.any(event_mask):
            out = (torch.zeros(n_features, dtype=torch.float64, device=beta.device), torch.zeros((n_features, n_features), dtype=torch.float64, device=beta.device))
            if return_aux:
                return (out[0], out[1], (eta, exp_eta, risk_sum))
            return out
        if entry is not None:
            if entry_ctx is None:
                entry_order, d_counts, add_end_np, rem_end_np, rem_order, event_idx, fail_ptr = self._build_entry_ctx_torch(time, event, entry, beta.device)
            ...
            for g in range(n_groups):
                ...
                if add_end > add_ptr:
                    ...
                    if n_add <= s2_block_size:
                        ...
                        s2 = self._s2_weighted_update_torch_blocked(s2, x_add, w_add, s2_block_size, sign=1.0)
                    add_ptr = add_end
                rem_end = int(rem_end_np[g])
                if rem_end > rem_ptr:
                    x_rem = X_rem[rem_ptr:rem_end]
                    ...
                    if n_rem <= s2_block_size:
                        ...
                        s2 = self._s2_weighted_update_torch_blocked(s2, x_rem, w_rem, s2_block_size, sign=-1.0)
                    rem_ptr = rem_end
                d_t_f = float(d_counts[g])
                if d_t_f <= 0:
                    continue
        ...
        if self.ties == 'efron':
            ...
            weights = counts
        if self.ties != 'efron' and os.environ.get('STATGPU_BRESLOW_TRITON', '0').strip().lower() in ('1', 'true', 'yes', 'on') and beta.is_cuda:
            ...
            if triton_out is not None:
                ...
                return (grad, hess)
        total = risk_X2
        hess = self._compute_hessian_grouped_streaming_torch(X, X_exp, total, risk_at_uft, risk_X_sum, first_idx, weights)
        if return_aux:
            return (grad, hess, (eta, exp_eta, risk_sum))
        return (grad, hess)

    def _compute_hessian_grouped_streaming_torch(self, X, X_exp, total, risk_at, risk_X_sum, first_idx, weights):
    ...
    def _get_entry_s2_torch_fn(self):
        ...
        use_compile = os.environ.get('STATGPU_ENTRY_S2_COMPILE_TORCH', '0').strip().lower() in ('1', 'true', 'yes', 'on')
    ...
    def _compute_cindex_torch(self, X, time, event, beta):
        ...
        chunk_size = max(1, min(n_events, int(128000000.0 / max(n, 1))))
    ...
    def _compute_inference_cpu(self, X, time, event, cluster=None):
        ...
        n_features = X.shape[1]
        _, hess = self._compute_gradient_hessian(self.coef_, X, time, event, getattr(self, '_efron_pre', None), entry=getattr(self, '_entry', None))
        ...
        try:
            grad_0, hess_0 = self._compute_gradient_hessian(np.zeros(n_features), X, time, event, ep, entry=getattr(self, '_entry', None))
            ...
            self.score_test_failure_reason_ = f'numpy null information is singular: {exc}'
    ...
    def _score_residuals_via_statsmodels_if_available(self, X, time, event):
        ...
        try:
            ...
            return np.nan_to_num(np.asarray(residuals, dtype=np.float64), nan=0.0, posinf=0.0, neginf=0.0)
        except Exception:
            return None
    ...
    def _compute_robust_score_residuals(self, X, time, event):
        ...
        result = cox_counting_process_objective(self.coef_, X, time, event, start=getattr(self, '_entry', None), strata=getattr(self, '_strata', None), ties=self.ties, score_residuals=True)
    ...
    def _compute_robust_score_residuals_gpu(self, X, time, event):
        ...
        if self.inference_mode == 'approx':
            ...
            return residuals
        from statgpu.survival._risk_sets import cox_counting_process_objective
        result = cox_counting_process_objective(cp.asarray(self.coef_, dtype=cp.float64), X, time, event, ties=self.ties, score_residuals=True)
        self.inference_method_ = 'counting_process_score_sandwich'
        self.inference_backend_ = 'cupy'
        self.inference_approximate_ = False
        self.inference_fallback_reason_ = None
    ...
    def _compute_baseline_hazard(self, X, time, event, entry=None):
        ...
        if entry is None:
            ...
            remove_sum = np.where(remove_end > 0, time_prefix[np.maximum(remove_end - 1, 0)], 0.0)
    ...
    def _compute_baseline_hazard_gpu(self, X, time, event, beta, entry=None):
        ...
        if entry is None:
            ...
            add_sum = cp.where(add_end > 0, entry_prefix[cp.maximum(add_end - 1, 0)], 0.0)
            remove_sum = cp.where(remove_end > 0, time_prefix[cp.maximum(remove_end - 1, 0)], 0.0)
            risk_at = add_sum - remove_sum
    ...
    def _compute_baseline_hazard_torch(self, X, time, event, beta, entry=None):
        ...
        if not torch.any(event_mask):
            ...
            self._baseline_cumulative_hazard = np.array([], dtype=np.float64)
            return
        unique_times, event_counts = torch.unique(time[event_mask], sorted=True, return_counts=True)
        self._unique_times = unique_times
        eta = X @ beta
        exp_eta = torch.exp(eta)
        if entry is None:
            ...
            add_sum = torch.where(add_end > 0, entry_prefix[torch.clamp(add_end - 1, min=0)], torch.zeros_like(unique_times))
            remove_sum = torch.where(remove_end > 0, time_prefix[torch.clamp(remove_end - 1, min=0)], torch.zeros_like(unique_times))
            risk_at = add_sum - remove_sum
        hazard = event_counts.to(torch.float64) / torch.clamp(risk_at, min=1e-300)
...
class _LegacyCoxReference(_LegacyCoxReferenceMixin):
    ...
    def __setattr__(self, name, value):
        if name == '_estimator' or name in self._legacy_local_state or any((name in cls.__dict__ for cls in type(self).__mro__)):
...
__all__ = ['_LegacyCoxReference', '_LegacyCoxReferenceMixin', '_estimate_breslow_tensor_bytes']
