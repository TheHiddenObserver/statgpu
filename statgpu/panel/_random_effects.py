...
class RandomEffects(BaseEstimator):
    ...
    def __init__(self, alpha: float=0.05, device: Union[str, Device]=Device.AUTO, n_jobs: Optional[int]=None):
    ...
    def fit(self, X=None, y=None, entity_ids=None, time_ids=None, formula=None, data=None):
        ...
        if formula is not None:
            ...
            y_raw, X_raw, self._design_info, self._feature_names, self._formula_has_intercept, fe_entity_ids, fe_time_ids, _fe_entity, _fe_time = _prepare_formula_fit(formula, data, X, y, model_has_intercept=False, support_pipe=True)
        ...
        entity_arr, _entity_labels = factorize_panel_labels(entity_ids, xp, ref_arr=X_arr, name='entity_ids')
        n, k = X_arr.shape
        self.nobs = n
        if y_arr.shape[0] != n:
            raise ValueError(f'y has {y_arr.shape[0]} observations but X has {n} rows')
        if entity_arr.shape[0] != n:
            raise ValueError(f'entity_ids has {entity_arr.shape[0]} observations but X has {n} rows')
        ...
        if df_within <= 0:
            raise ValueError(f'Not enough observations for within df: n={n}, k={k}, n_entities={n_entities}, df_within={df_within}')
        sigma2_e = rss_within / df_within
        df_between = n_entities - k
        if df_between <= 0:
            warnings.warn(f'Between estimator under-identified: n_entities={n_entities} <= k={k}. Variance component sigma2_a may be unreliable.', UserWarning, stacklevel=2)
            df_between = max(df_between, 1)
        s_b_sq = rss_between / df_between
        sigma2_a_raw = (s_b_sq - sigma2_e) / T_bar
        sigma2_a = max(0.0, sigma2_a_raw)
        self.variance_components_ = {'sigma2_e': sigma2_e, 'sigma2_a': sigma2_a}
        ...
        self.theta_ = sum((theta_map[Ti] * entity_counts[Ti] / total_entities for Ti in T_i_unique))
    ...
    def _compute_inference_on_device(self, xp, X, coef, resid):
        ...
        self.conf_int_ = np.column_stack([coef_np - t_crit * bse_np, coef_np + t_crit * bse_np])
    ...
    def summary(self):
        ...
        s = PanelSummary(model_type='RandomEffects', nobs=self.nobs, df_resid=self.df_resid, coef=self._params, bse=self.bse_, tvalues=self.tvalues_, pvalues=self.pvalues_, conf_int=self.conf_int_, feature_names=feat_names, variance_components=self.variance_components_, theta=self.theta_, alpha=self.alpha)
        print(s)
        return s
