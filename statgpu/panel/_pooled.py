...
class PooledOLS(BaseEstimator):
    ...
    def __init__(self, cov_type: str='nonrobust', alpha: float=0.05, bandwidth: Optional[int]=None, kernel: str='bartlett', device: Union[str, Device]=Device.AUTO, n_jobs: Optional[int]=None):
    ...
    def fit(self, X=None, y=None, cluster=None, time_index=None, formula=None, data=None):
        ...
        y_arr, X_arr, self._design_info, self._feature_names, self._formula_has_intercept, _fe_eids, _fe_tids, _fe_entity, _fe_time = _prepare_formula_fit(formula, data, X, y, model_has_intercept=True)
        ...
        df_resid = n - rank
        if df_resid <= 0:
            raise ValueError(f'positive residual degrees of freedom required; n={n}, rank={rank}')
        resid = y_arr - X_arr @ params
        scale = _to_float_scalar(xp.sum(resid * resid)) / df_resid
        self._compute_inference(X_arr, resid, params, scale, n, k, df_resid, xp, backend.name, cluster=cluster)
    ...
    def predict(self, X):
        ...
        X_arr = _formula_predict(X, getattr(self, '_design_info', None), getattr(self, '_formula_has_intercept', None), model_has_intercept=True)
        backend = self._get_backend(backend='auto')
        xp = backend.xp
        X_arr = xp_asarray(X_arr, dtype=xp.float64, xp=xp)
        if X_arr.ndim == 1:
    ...
    def summary(self):
        ...
        feature_names = _get_feature_names(getattr(self, '_feature_names', None), len(self.coef_), prefix='x')
        return PanelSummary(model_type='PooledOLS', cov_type=self._cov_type, coef=np.asarray(self.coef_), bse=np.asarray(self.bse_), tvalues=np.asarray(self.tvalues_), pvalues=np.asarray(self.pvalues_), conf_int=np.asarray(self.conf_int_), nobs=self.nobs, df_resid=self.df_resid, alpha=self.alpha, feature_names=feature_names)

    def _compute_inference(self, X, resid, params, scale, n, k, df_resid, xp, backend_name, cluster=None):
        ...
        if self._cov_type == 'nonrobust':
            ...
            cluster_arr, _ = factorize_panel_labels(cluster, xp, ref_arr=X, name='cluster', expected_n=n)
            cov_params = clustered_covariance(X, resid, cluster_arr, xp)
        elif self._cov_type == 'hac':
            cov_params = hac_covariance(X, resid, bandwidth=self.bandwidth, kernel=self.kernel, xp=xp)
