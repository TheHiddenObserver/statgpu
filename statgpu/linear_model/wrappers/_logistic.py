...
from statgpu.metrics import binary_average_precision_score, binary_precision_recall_curve, binary_roc_auc_score, binary_roc_curve, evaluate_binary_classification
...
def _require_cupy(context: str):
    ...
    try:
        ...
        raise ImportError(f'{context} requires CuPy for GPU computation, but CuPy is not installed. Install CuPy matching your CUDA version, e.g.: `pip install cupy-cuda12x` (CUDA 12.x) or `pip install cupy-cuda11x` (CUDA 11.x).') from exc
...
class LogisticRegression(BaseEstimator):
    ...
    def __init__(self, fit_intercept: bool=True, C: float=1.0, max_iter: int=100, tol: float=0.0001, device: Union[str, Device]=Device.AUTO, n_jobs: Optional[int]=None, compute_inference: bool=True, cov_type: str='nonrobust', gpu_memory_cleanup: bool=False, hac_maxlags: Optional[int]=None):
    ...
    def _fit_gpu(self, X, y, sample_weight=None):
        ...
        if self._compute_inference_enabled:
            W_inf = p * (1 - p)
            W_inf = cp.clip(W_inf, 1e-08, 1 - 1e-08)
            ...
            conf_int_gpu = cp.stack([params - z_crit * bse_gpu, params + z_crit * bse_gpu], axis=1)
        ...
        self._loglik_null = float(cp.asnumpy(cp.sum(y * cp.log(y_mean) + (1 - y) * cp.log(1 - y_mean))))
        try:
            del X_design
        except Exception:
            pass
    ...
    def _fit_torch(self, X, y, sample_weight=None):
        ...
        if self._compute_inference_enabled:
            W_inf = p * (1 - p)
            W_inf = torch.clamp(W_inf, 1e-08, 1 - 1e-08)
            ...
            conf_int_gpu = torch.stack([params - z_crit * bse_gpu, params + z_crit * bse_gpu], dim=1)
    ...
    def _train_classification_table(self):
        ...
        if device == Device.CUDA:
            ...
            self._train_eval_cache = evaluate_binary_classification(y_true, y_score, threshold=0.5, include_curves=False, backend='cupy')
        ...
        if device == Device.TORCH:
            ...
            self._train_eval_cache = evaluate_binary_classification(y_true, y_score, threshold=0.5, include_curves=False, backend='torch')
            return self._train_eval_cache['classification_table']
        y_score = self._to_numpy(self.predict_proba(X_train))[:, 1]
        self._train_eval_cache = evaluate_binary_classification(self._y, y_score, threshold=0.5, include_curves=False, backend='numpy')
        return self._train_eval_cache['classification_table']
    ...
    def predict(self, X):
        ...
        if hasattr(proba, 'is_floating_point'):
    ...
    def confusion_matrix(self, X, y, threshold: float=0.5) -> np.ndarray:
        ...
        if self._get_compute_device() == Device.CUDA:
            ...
            out = evaluate_binary_classification(y_true, y_score, threshold=threshold, include_curves=False, backend='cupy')
        ...
        if self._get_compute_device() == Device.TORCH:
            ...
            out = evaluate_binary_classification(y_true, y_score, threshold=threshold, include_curves=False, backend='torch')
            return out['confusion_matrix']
        y_true = self._to_numpy(y)
        y_score = self._to_numpy(self.predict_proba(X))[:, 1]
        out = evaluate_binary_classification(y_true, y_score, threshold=threshold, include_curves=False, backend='numpy')
        return out['confusion_matrix']
    ...
    def evaluate_classification(self, X, y, threshold: float=0.5, include_curves: bool=True) -> Dict[str, Any]:
        """
        ...
        if self._get_compute_device() == Device.CUDA:
            ...
            y_true = cp.asarray(self._to_array(y, Device.CUDA)).reshape(-1)
            y_score = cp.asarray(self.predict_proba(X))[:, 1]
            return evaluate_binary_classification(y_true, y_score, threshold=threshold, include_curves=include_curves, backend='cupy')
        if self._get_compute_device() == Device.TORCH:
            y_true = self._to_array(y, Device.TORCH, backend='torch').reshape(-1)
            y_score = self.predict_proba(X)[:, 1]
            return evaluate_binary_classification(y_true, y_score, threshold=threshold, include_curves=include_curves, backend='torch')
        y_true = self._to_numpy(y).reshape(-1)
        y_score = self._to_numpy(self.predict_proba(X))[:, 1]
        return evaluate_binary_classification(y_true, y_score, threshold=threshold, include_curves=include_curves, backend='numpy')
    ...
    def plot_roc_curve(self, X, y, ax=None, label: Optional[str]=None):
        ...
        try:
            ...
            raise ImportError('matplotlib is required for plot_roc_curve(). Install it with: pip install matplotlib') from exc
    ...
    def plot_precision_recall_curve(self, X, y, ax=None, label: Optional[str]=None):
        ...
        try:
            ...
            raise ImportError('matplotlib is required for plot_precision_recall_curve(). Install it with: pip install matplotlib') from exc
    ...
    def summary(self):
        ...
        for i, name in enumerate(feature_names):
            print(f'{name:<15} {self._params[i]:>12.4f} {self._bse[i]:>12.4f} {self._zvalues[i]:>10.3f} {self._pvalues[i]:>10.4f} {self._conf_int[i, 0]:>12.4f} {self._conf_int[i, 1]:>12.4f}')
        print('=' * 80)
