...
from statgpu.linear_model._gaussian_inference import GaussianFitState, build_gaussian_fit_state, compute_gaussian_inference
if TYPE_CHECKING:
...
class _PenalizedInferenceMixin:
    ...
    def _gaussian_fit_state(self, X, y, sample_weight=None):
        ...
        if sample_weight is None:
            return build_gaussian_fit_state(X_np, y_np, self.coef_, self.intercept_, self._effective_intercept)
        ...
        return GaussianFitState(X_design=X_design, y=y_weighted, resid=resid, scale=scale, nobs=nobs, df_resid=df_resid, params=params)
    ...
    def _compute_post_fit_gaussian_inference(self, X, y, sample_weight=None):
        ...
        if penalty_name in ('scad', 'mcp'):
            ...
            if im == 'oracle':
                ...
                self._compute_post_fit_bootstrap_inference(X, y)
                return
            raise NotImplementedError(f"SCAD/MCP inference requires inference_method='oracle' or 'bootstrap', got '{im}'. Set compute_inference=False or choose a supported method.")
        if penalty_name in ('l1', 'elasticnet', 'en'):
            ...
            if 'debiased' in inference_method:
                ...
                raise NotImplementedError(f"L1/ElasticNet inference requires inference_method='debiased', 'cpu_ols', 'gpu_ols', or 'bootstrap', got '{inference_method}'. Set compute_inference=False or choose a supported method.")
            return
        if penalty_name != 'l2':
            raise NotImplementedError(f"Inference not supported for penalty='{penalty_name}' with loss='{self.loss}'. Set compute_inference=False or use a supported penalty.")
        ...
        ridge_normalization = float(state.nobs) if sample_weight is None else float(np.sum(np.asarray(_to_numpy(sample_weight), dtype=float)))
        ridge_alpha = ridge_normalization * self._ridge_alpha_for_exact()
        result = compute_gaussian_inference(self._X_design, self._params, self._resid, self._scale, self._df_resid, self._cov_type, hac_maxlags=self._hac_maxlags, ridge_alpha=ridge_alpha, ridge_penalize_intercept=False if self._effective_intercept else True)
        if result is None:
    ...
    @staticmethod
    def _debiased_stats_from_M(M, Sigma_hat, sigma2, coef, X, y, intercept, fit_intercept, n, xp, arr_norm):
    ...
    def _compute_post_fit_debiased_inference(self, X, y, sample_weight=None):
        ...
        if backend in ('cupy', 'torch'):
            raise NotImplementedError(f"Debiased Lasso inference is not yet supported on device={backend!r}. Use device='cpu' for inference, or set inference_method='cpu_ols' or 'bootstrap'.")
        ...
        from statgpu.linear_model.wrappers._lasso import _debiased_m_cache_get, _debiased_m_cache_put, _debiased_m_key_from_numpy_design
        sigma_hat = np.sqrt(sigma2)
        lam_nw = np.sqrt(2.0 * np.log(max(p, 2)) / n) * sigma_hat
        m_cache_key = _debiased_m_key_from_numpy_design(X_np, n=n, p=p, lam_nw=lam_nw, tol=float(self._tol))
        ...
        if M_cached is not None:
            ...
            for j in range(p):
                ...
                nw = PenalizedLinearRegression(penalty='l1', alpha=lam_nw, fit_intercept=False, max_iter=500, tol=1e-05, device='cpu', cpu_solver='fista', compute_inference=False, inference_method='none')
        ...
        theta_db, se, z_stats, _, se_intercept, z_intercept = self._debiased_stats_from_M(M, Sigma_hat, sigma2, coef, X_np, y_np, self.intercept_, self._effective_intercept, n, np, np.linalg.norm)
    ...
    def _compute_post_fit_cpu_ols_inference(self, X, y):
        ...
        backend = _resolve_backend('auto', X)
        if backend in ('cupy', 'torch'):
            raise NotImplementedError(f"CPU-OLS inference is not yet supported on device={backend!r}. Use device='cpu' for inference, or set inference_method='debiased'.")
        from statgpu.inference._distributions_backend import get_distribution
        ...
        ci_sel = np.column_stack([params_sel - t_crit * bse_sel, params_sel + t_crit * bse_sel])
        self._bse = np.zeros(n_params)
        self._tvalues = np.zeros(n_params)
        self._pvalues = np.ones(n_params)
        self._conf_int = np.zeros((n_params, 2))
        ...
        self._inference_result = ParameterInferenceResult(method='post_selection_ols', params=self._params.copy(), bse=self._bse.copy(), statistic=self._tvalues.copy(), statistic_name='t', pvalues=self._pvalues.copy(), conf_int=self._conf_int.copy(), distribution='t', df=float(df_resid), metadata={'heuristic_post_selection': True, 'backend_path': 'cpu_ols', 'n_selected': n_selected})
        self._inference_result.apply_to(self)
    ...
    def _compute_post_fit_bootstrap_inference(self, X, y):
        ...
        for b in range(B):
            ...
            from statgpu.linear_model.penalized._penalized_linear import PenalizedLinearRegression
            refit = PenalizedLinearRegression(penalty='l1', alpha=float(self.alpha), fit_intercept=self._effective_intercept, max_iter=self._max_iter, tol=self._tol, device='cpu', cpu_solver='fista', compute_inference=False, inference_method='none')
            if self._effective_intercept:
        ...
        from statgpu.inference._results import ParameterInferenceResult
        self._inference_result = ParameterInferenceResult(method='residual_bootstrap', params=self._params.copy(), bse=self._bse.copy(), statistic=self._tvalues.copy(), statistic_name='z', pvalues=self._pvalues.copy(), conf_int=self._conf_int.copy(), distribution='bootstrap_percentile', metadata={'n_bootstrap': B, 'random_state': getattr(self, 'bootstrap_random_state', None)})
        self._inference_result.apply_to(self)
    ...
    def _compute_inference_debiased_gpu(self, X_gpu, y_gpu, coef_gpu):
        ...
        n, p = X_gpu.shape
        if p <= 1:
            raise NotImplementedError('Debiased Lasso inference requires at least 2 features (p >= 2). For p=1, use post_selection_ols or bootstrap.')
        ...
        from statgpu.linear_model.wrappers._lasso import _debiased_m_cache_get, _debiased_m_cache_put, _LASSO_DEBIASED_M_GPU_HASH_ROW_CHUNK, _solve_lasso_path_gpu_fista_multi_fold_from_gram
        sigma_hat = np.sqrt(sigma2)
        ...
        if M_cached is not None:
            ...
            for j0 in range(0, p, chunk_size):
                ...
                coefs_batch_desc, _ = _solve_lasso_path_gpu_fista_multi_fold_from_gram(XtX_batch, Xty_batch, n_samples_vec=np.full((bsz,), float(n), dtype=np.float64), alphas_desc=alpha_nw, max_iter=500, tol=1e-05, stopping='coef_delta', lipschitz_L=L_global, check_every=8)
        ...
        theta_db, se, z_stats, _, se_intercept, z_intercept = self._debiased_stats_from_M(M, Sigma_hat, sigma2, coef_gpu, X_gpu, y_gpu, intercept_val, self._effective_intercept, n, cp, cp.linalg.norm)
        ...
        if self._effective_intercept:
            ...
            p_intercept = cp.minimum(1.0, 2.0 * _gpu_norm.sf(cp.abs(cp.asarray(z_intercept)).reshape(1)))
            ci_intercept = cp.stack([intercept_gpu - z_crit * cp.asarray(se_intercept), intercept_gpu + z_crit * cp.asarray(se_intercept)]).reshape(1, 2)
            self._bse = cp.asnumpy(cp.concatenate([cp.asarray(se_intercept).reshape(1), se]))
            self._tvalues = cp.asnumpy(cp.concatenate([cp.asarray(z_intercept).reshape(1), z_stats]))
    ...
    def _compute_inference_debiased_torch(self, X_torch, y_torch, coef_torch):
        ...
        from statgpu.linear_model.wrappers._lasso import _debiased_m_cache_get, _debiased_m_cache_put, _debiased_m_key_from_sample, _solve_lasso_path_gpu_fista_multi_fold_from_gram_torch
        sigma_hat = np.sqrt(sigma2)
        lam_nw = float(np.sqrt(2.0 * np.log(max(p, 2)) / n) * sigma_hat)
        alpha_nw = np.asarray([lam_nw], dtype=np.float64)
        X_sample = X_torch[:min(24, n), :min(24, p)].cpu().numpy()
        m_cache_key = _debiased_m_key_from_sample(n=n, p=p, dtype_name=str(dtype), sample_block=X_sample, lam_nw=lam_nw, tol=float(self._tol))
        M_cached = _debiased_m_cache_get(m_cache_key)
        if M_cached is not None:
            ...
            for j0 in range(0, p, chunk_size):
                ...
                Xty_batch = XtX_full[cols_batch, j_batch.reshape(-1, 1)].reshape(bsz, p - 1)
                coefs_batch_desc, _ = _solve_lasso_path_gpu_fista_multi_fold_from_gram_torch(XtX_batch, Xty_batch, n_samples_vec=torch.full((bsz,), float(n), dtype=torch.float64, device=device), alphas_desc=alpha_nw, max_iter=500, tol=1e-05, stopping='coef_delta', lipschitz_L=L_global, check_every=8)
                ...
                inv_c = torch.where(small_c, torch.tensor(0.0, dtype=dtype, device=device), torch.tensor(1.0, dtype=dtype, device=device) / C_batch)
        ...
        intercept_val = float(self.intercept_) if self._effective_intercept else 0.0
        theta_db, se, z_stats, _, se_intercept, z_intercept = self._debiased_stats_from_M(M, Sigma_hat, sigma2, coef_torch, X_torch, y_torch, intercept_val, self._effective_intercept, n, torch, torch.linalg.norm)
        pvalues = torch.minimum(torch.tensor(1.0, dtype=dtype, device=device), 2.0 * _gpu_norm.sf(torch.abs(z_stats)))
        z_crit = _gpu_norm.ppf(0.975)
        ...
        if self._effective_intercept:
            ...
            p_intercept = torch.minimum(torch.tensor(1.0, dtype=dtype, device=device), 2.0 * _gpu_norm.sf(torch.abs(torch.tensor(z_intercept, dtype=dtype, device=device)).reshape(1)))
            ci_intercept = torch.stack([intercept_t - z_crit * torch.tensor(se_intercept, dtype=dtype, device=device), intercept_t + z_crit * torch.tensor(se_intercept, dtype=dtype, device=device)]).reshape(1, 2)
        ...
        self._nobs = n
        if self._effective_intercept:
            self._X_design = np.column_stack([np.ones(n), X_torch.cpu().numpy() if hasattr(X_torch, 'cpu') else np.asarray(X_torch)])
        else:
    ...
    def _compute_penalized_sandwich_inference(self, X, y, sample_weight=None):
        ...
        if self._effective_intercept:
            ...
            params = xp.concatenate([xp.asarray([self.intercept_], dtype=xp.float64), xp_asarray(self.coef_, dtype=xp.float64, xp=xp)])
            intercept_idx = 0
        else:
            X_design = X_arr
        ...
        if self._penalty is not None:
            ...
            if pen_name in ('l2',):
                curv_feat = xp_asarray(self._penalty.curvature_diag(self.coef_), dtype=xp.float64, xp=xp)
        ...
        result = m_estimation_inference(self._loss, X_design, y_arr, params, cov_type=self._cov_type, penalty_curvature_diag=curv if has_curv else None, sample_weight=sw_arr)
    ...
    def _compute_oracle_inference(self, X, y, sample_weight=None):
        ...
        if n_active == 0:
            ...
            self._inference_result = ParameterInferenceResult(method='oracle', params=self._params.copy(), bse=self._bse.copy(), statistic=self._zvalues.copy(), statistic_name='z', pvalues=self._pvalues.copy(), conf_int=self._conf_int.copy(), distribution='normal', metadata={'n_active': 0, 'active_set': []})
            self._inference_result.apply_to(self)
            return
        ...
        _MODEL_MAP = {'squared_error': LinearRegression, 'poisson': PoissonRegression, 'logistic': None, 'gamma': GammaRegression, 'inverse_gaussian': InverseGaussianRegression, 'negative_binomial': NegativeBinomialRegression, 'tweedie': TweedieRegression}
        ...
        result = m_estimation_inference(loss_obj, X_design, y_cpu, params_active, cov_type=self._cov_type, sample_weight=sw_cpu)
        full_p = p + (1 if self._effective_intercept else 0)
        ...
        if self._effective_intercept:
            ...
            self._bse[0] = np.asarray(result['bse'])[0]
            self._zvalues[0] = np.asarray(result['statistic'])[0]
            self._pvalues[0] = np.asarray(result['pvalues'])[0]
            self._conf_int[0] = np.asarray(result['conf_int'])[0]
        self._inference_result = ParameterInferenceResult(method='oracle', params=self._params.copy(), bse=self._bse.copy(), statistic=self._zvalues.copy(), statistic_name='z', pvalues=self._pvalues.copy(), conf_int=self._conf_int.copy(), distribution='normal', metadata={'n_active': n_active, 'active_set': active.tolist() if hasattr(active, 'tolist') else list(active), 'covariance_convention': _infer_covariance_convention(self._cov_type, False)})
        self._inference_result.apply_to(self)
    ...
    def _compute_simultaneous_ci_maxz_bootstrap(self):
        ...
        include_intercept = getattr(self, 'simultaneous_include_intercept', getattr(self, '_simultaneous_include_intercept', False))
        ...
        alpha_sim = float(getattr(self, 'simultaneous_alpha', getattr(self, '_simultaneous_alpha', 0.05)))
        B = int(getattr(self, 'simultaneous_n_bootstrap', getattr(self, '_simultaneous_n_bootstrap', 1000)))
        rng = np.random.default_rng(getattr(self, 'simultaneous_random_state', getattr(self, '_simultaneous_random_state', None)))
        chunk = min(256, B)
    ...
    def _precompute_exact_l2_inference_cupy(self, X, y, XtX_centered, X_mean, coef_full, n_samples, sample_weight=None, normalization=None):
        ...
        if df_resid <= 0:
            ...
            self._precomputed_gaussian_state = {'params': coef_full.get(), 'X_design': X_design_gpu.get(), 'y': y_state.get(), 'resid': resid.get(), 'scale': np.nan, 'nobs': int(n_samples), 'df_resid': int(df_resid)}
            return
        if self._cov_type == 'nonrobust':
            ...
            cov_params = robust_covariance_gpu(X_design_gpu, resid, bread_inv, self._cov_type, cp, hac_maxlags=self._hac_maxlags)
        ...
        result = GaussianInferenceResult(params=coef_full.get(), bse=bse.get(), statistic=tvalues.get(), pvalues=pvalues.get(), conf_int=conf_int.get(), cov_type=self._cov_type, distribution=distribution, df=df_resid, method=method, metadata={'ridge_alpha': ridge_alpha, 'alpha': 0.05})
        result.apply_to(self)
        self._inference_precomputed = True
        self._precomputed_gaussian_state = {'params': coef_full.get(), 'X_design': X_design_gpu.get(), 'y': y_state.get(), 'resid': resid.get(), 'scale': float(scale.get()), 'nobs': int(n_samples), 'df_resid': int(df_resid)}

    def _precompute_exact_l2_inference_torch(self, X, y, XtX_centered, X_mean, coef_full, n_samples, sample_weight=None, normalization=None):
        ...
        sw = None if sample_weight is None else torch.as_tensor(sample_weight, dtype=X.dtype, device=X.device).reshape(-1)
        ...
        if df_resid <= 0:
            self._inference_precomputed = True
            self._precomputed_gaussian_state = {'params': coef_full.detach().cpu().numpy(), 'X_design': X_design_gpu.detach().cpu().numpy(), 'y': y_state.detach().cpu().numpy(), 'resid': resid.detach().cpu().numpy(), 'scale': np.nan, 'nobs': int(n_samples), 'df_resid': int(df_resid)}
            return
        if self._cov_type == 'nonrobust':
            cov_params = scale * (bread_inv @ xtx_full @ bread_inv)
            distribution, method = ('t', 'classical')
            ...
            from statgpu.linear_model._gaussian_inference import robust_covariance_gpu
            cov_params = robust_covariance_gpu(X_design_gpu, resid, bread_inv, self._cov_type, torch, hac_maxlags=self._hac_maxlags)
            distribution, method = ('normal', 'sandwich')
        ...
        from statgpu.inference._results import GaussianInferenceResult
        result = GaussianInferenceResult(params=coef_full.detach().cpu().numpy(), bse=bse.detach().cpu().numpy(), statistic=tvalues.detach().cpu().numpy(), pvalues=pvalues.detach().cpu().numpy(), conf_int=conf_int.detach().cpu().numpy(), cov_type=self._cov_type, distribution=distribution, df=df_resid, method=method, metadata={'ridge_alpha': ridge_alpha, 'alpha': 0.05})
        result.apply_to(self)
        self._inference_precomputed = True
        self._precomputed_gaussian_state = {'params': coef_full.detach().cpu().numpy(), 'X_design': X_design_gpu.detach().cpu().numpy(), 'y': y_state.detach().cpu().numpy(), 'resid': resid.detach().cpu().numpy(), 'scale': float(scale.detach().cpu().numpy()), 'nobs': int(n_samples), 'df_resid': int(df_resid)}
