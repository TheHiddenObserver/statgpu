...
from statgpu.backends._utils import _is_complex_array, _require_real_array, _to_float_scalar, _to_numpy
...
class PenalizedCoxPHModel(PenalizedGeneralizedLinearModel):
    _SUPPORTED_PENALTY_NAMES = frozenset({'', 'none', 'null', 'l1', 'l2', 'l2_squared', 'ridge', 'elasticnet', 'en', 'scad', 'mcp'})
    ...
    def __init__(self, penalty='l2', alpha=1.0, *, ties='breslow', solver='auto', max_iter=1000, tol=0.0001, fit_intercept=False, l1_ratio=0.5, penalty_kwargs=None, device='auto', n_jobs=None, cpu_solver='fista', lipschitz_L=None, gpu_memory_cleanup=False, loss_kwargs=None, compute_inference=False, inference_method='debiased', cov_type='nonrobust', hac_maxlags=None, stopping='coef_delta', lla=True, max_lla_iters=50, lla_tol=1e-06):
        for name, value in (('fit_intercept', fit_intercept), ('gpu_memory_cleanup', gpu_memory_cleanup), ('compute_inference', compute_inference), ('lla', lla)):
            _normalize_boolean_control(value, name)
        if bool(fit_intercept):
            raise ValueError('PenalizedCoxPHModel does not fit an intercept because the Cox partial likelihood cannot identify one; set fit_intercept=False.')
        ...
        if loss_kwargs is not None and 'ties' in loss_kwargs:
            ...
            if loss_ties != ties_normalized:
                raise ValueError("ties and loss_kwargs['ties'] specify different tie methods")
        super().__init__(loss='cox_ph', penalty=penalty, alpha=alpha, solver=solver, max_iter=max_iter, tol=tol, fit_intercept=False, l1_ratio=l1_ratio, penalty_kwargs=penalty_kwargs, device=device, n_jobs=n_jobs, cpu_solver=cpu_solver, lipschitz_L=lipschitz_L, gpu_memory_cleanup=gpu_memory_cleanup, loss_kwargs=loss_kwargs, compute_inference=compute_inference, inference_method=inference_method, cov_type=cov_type, hac_maxlags=hac_maxlags, stopping=stopping, lla=lla, max_lla_iters=max_lla_iters, lla_tol=lla_tol)
    ...
    def _validate_inference_request(self):
        ...
        if self._compute_inference_enabled:
            raise NotImplementedError('PenalizedCoxPHModel is currently estimation-only: compute_inference=True is not supported for penalized Cox models. Set compute_inference=False, or use statgpu.survival.CoxPH for unpenalized Cox inference.')

    def set_params(self, **params):
        """Set estimator parameters while preserving the no-intercept contract."""
        for name in ('fit_intercept', 'gpu_memory_cleanup', 'compute_inference', 'lla'):
            if name in params:
        ...
        if 'max_lla_iters' in params:
            self._validate_positive_integer(params['max_lla_iters'], 'max_lla_iters')
        ...
        if params.get('lipschitz_L', self.lipschitz_L) is not None:
            self._validate_finite_positive(params.get('lipschitz_L', self.lipschitz_L), 'lipschitz_L')
        ...
        if prospective_loss_kwargs is not None and 'ties' in prospective_loss_kwargs and (str(prospective_loss_kwargs['ties']).lower() != prospective_ties):
            raise ValueError("ties and loss_kwargs['ties'] specify different tie methods")
    ...
    def _cleanup_selected_backend_memory(self):
        self._cleanup_backend_memory(getattr(self, '_selected_backend_name', None))

    @staticmethod
    def _validate_positive_integer(value, name):
        if isinstance(value, (bool, np.bool_)) or not isinstance(value, numbers.Integral) or int(value) < 1:
    ...
    def _validate_supported_penalty(cls, penalty):
        from statgpu.penalties import ElasticNetPenalty, L1Penalty, L2Penalty, MCPPenalty, Penalty, SCADPenalty
        name = str(getattr(penalty, 'name', penalty)).lower().strip()
        if name not in cls._SUPPORTED_PENALTY_NAMES:
            raise ValueError(f'PenalizedCoxPHModel supports only L1, L2/Ridge, ElasticNet, SCAD, MCP, or no penalty; got penalty={name!r}')
        if not isinstance(penalty, Penalty):
            return
        supported_types = (L1Penalty, L2Penalty, ElasticNetPenalty, SCADPenalty, MCPPenalty)
        if not isinstance(penalty, supported_types):
            raise ValueError('PenalizedCoxPHModel accepts only built-in validated penalty objects for L1, L2, ElasticNet, SCAD, or MCP')
        try:
            alpha = float(penalty.alpha)
        except (AttributeError, TypeError, ValueError) as exc:
            raise ValueError('penalty object alpha must be finite') from exc
        minimum = 0.0 if isinstance(penalty, (L1Penalty, L2Penalty, ElasticNetPenalty)) else np.nextafter(0.0, 1.0)
    ...
    def _parse_survival_formula(formula, data):
        ...
        try:
            ...
            raise ImportError('pandas and patsy are required for the penalized Cox formula interface') from exc
        ...
        y_patsy, X_patsy = patsy.dmatrices(formula, formula_data, eval_env=EvalEnvironment([make_surv_env()]), return_type='dataframe')
        ...
        if y_array.shape[1] == 3:
            raise NotImplementedError('PenalizedCoxPHModel currently supports right-censored Surv(time, event) formulas only; use statgpu.survival.CoxPH for start-stop data.')
    ...
    def _validate_event_target(y):
        ...
        if isinstance(y, dict):
            ...
            target_xp = _get_xp(y)
            target = y if target_xp.__name__ == 'torch' else target_xp.asarray(y)
        ...
        invalid = xp.any(~xp.isfinite(event) | (event != 0) & (event != 1))
        has_event = xp.any(event == 1)
        status = xp.stack((invalid, has_event))
        invalid_host, has_event_host = np.asarray(_to_numpy(status), dtype=bool)
    ...
    def fit(self, X=None, y=None, sample_weight=None, formula=None, data=None):
        ...
        try:
            ...
            if formula is not None:
                ...
                X, y, design_info, has_intercept, feature_names = self._parse_survival_formula(formula, data)
                formula_state = (design_info, has_intercept, feature_names)
                formula = None
                data = None
            ...
            result = super().fit(X=X, y=y, sample_weight=None, formula=formula, data=data)
            if formula_state is not None:
                self._design_info, self._formula_has_intercept, self._feature_names = formula_state
    ...
    def _prepare_penalized_cox_prediction(self, X):
        ...
        Xb = _normalize_prediction_matrix(X, backend=backend, n_features=int(len(self.coef_)))
        return (backend, Xb)
    ...
    def _prepare_penalized_cox_target(y, backend):
        ...
        if isinstance(y, dict):
            ...
            time = backend.asarray(y['time'], dtype=backend.float64).reshape(-1)
            event = backend.asarray(y['event'], dtype=backend.float64).reshape(-1)
            return (time, event)
    ...
    def _score_impl(self, X, y, sample_weight=None):
        ...
        if sample_weight is not None:
            import warnings
            warnings.warn('sample_weight is not supported for C-index (ranking metric), ignoring.', UserWarning, stacklevel=2)
        if self.coef_ is None:
            raise RuntimeError('Model has not been fitted yet.')
        ...
        coef = backend.asarray(self.coef_, dtype=backend.float64)
        if int(time.shape[0]) != int(event.shape[0]) or int(Xb.shape[0]) != int(time.shape[0]):
            raise ValueError('X, time, and event must contain the same number of rows')
        return _to_float_scalar(counting_process_concordance(coef, Xb, time, event))
