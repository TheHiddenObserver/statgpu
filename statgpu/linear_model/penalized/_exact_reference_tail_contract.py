"""Stable low-degree Student-t tails for exact/precomputed Gaussian inference.

The shared Gaussian inference path already routes Student-t df=1/2 through
``two_sided_reference_inference`` because the generic incomplete-beta formula
squares the statistic and can overflow before a representable tail underflows.
Exact CuPy/Torch L2 inference historically called ``TDistributionBase``
directly.  Patch only its df=1/2 public two-sided helpers so exact/precomputed
and ordinary Gaussian inference share the same stable numerical contract.
"""
...
from statgpu.inference._distributions_backend import TDistributionBase
...
def _install_exact_reference_tail_contract() -> None:
    ...
    if getattr(current_pvalue, "_statgpu_exact_reference_tail", False):
        return
    ...
    _two_sided_pvalue_stable._statgpu_exact_reference_tail = True
    _two_sided_pvalue_stable._statgpu_original = current_pvalue
    _two_sided_critical_value_stable._statgpu_exact_reference_tail = True
...
_install_exact_reference_tail_contract()
