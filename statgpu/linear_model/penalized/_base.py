...
class PenalizedGeneralizedLinearModel(_PenalizedFitMixin, _PenalizedInferenceMixin, _PenalizedPredictMixin, BaseEstimator):
    """
    ...
    def __init__(self, loss: str='squared_error', penalty: Union[str, 'Penalty']='l1', alpha: float=1.0, l1_ratio: float=0.5, penalty_kwargs: Optional[Dict]=None, fit_intercept: bool=True, max_iter: int=1000, tol: float=0.0001, device: Union[str, Device]=Device.AUTO, n_jobs: Optional[int]=None, cpu_solver: str='fista', solver: str='auto', lipschitz_L: Optional[float]=None, gpu_memory_cleanup: bool=False, compute_inference: bool=False, inference_method: str='debiased', cov_type: str='nonrobust', hac_maxlags: Optional[int]=None, stopping: str='coef_delta', lla: bool=True, max_lla_iters: int=50, lla_tol: float=1e-06, loss_kwargs: Optional[Dict]=None):
        super().__init__(device=device, n_jobs=n_jobs)
        self.loss = loss
        self.penalty = penalty
        self.alpha = alpha
        self.l1_ratio = l1_ratio
        self.penalty_kwargs = penalty_kwargs if penalty_kwargs is not None else {}
    ...
    def _validate_solver_penalty(self):
        ...
        if self._solver == 'exact':
            if self.loss != 'squared_error' or penalty_name != 'l2':
                raise ValueError("solver='exact' is only supported for squared-error L2/Ridge models.")
            return
        if solver_name == 'irls' and penalty_name not in ('l2', 'none', 'null', ''):
            raise ValueError("solver='irls' only supports smooth L2 or no-penalty objectives.")
        if solver_name == 'irls' and (not getattr(self._loss, '_supports_irls', False)):
            raise ValueError(f"solver='irls' requires a loss with IRLS support, got loss='{self.loss}'. Use solver='newton' or 'fista'.")
        if solver_name in ('newton', 'lbfgs') and penalty_name in non_smooth:
            raise ValueError(f"solver='{solver_name}' only supports smooth objectives; use solver='fista' for penalty='{penalty_name}'.")
        if solver_name in ('newton', 'lbfgs', 'exact') and self.loss == 'quantile':
            raise ValueError(f"solver='{solver_name}' requires Hessian, but quantile loss has none. Use solver='fista', 'irls', or 'auto' for quantile regression.")
    ...
    def _validate_inference_request(self):
        ...
        if self.loss == 'squared_error' and penalty_name in ('l1', 'elasticnet', 'en') and (inference_method == 'sandwich'):
        ...
        if self.loss == 'squared_error':
            ...
            if penalty_name in ('scad', 'mcp') and inference_method in ('oracle', 'bootstrap'):
                return
            raise NotImplementedError(f"squared_error + '{penalty_name}' inference not supported with inference_method='{inference_method}'. Use inference_method='oracle' or 'bootstrap'.")
        ...
        if penalty_name in ('l1',):
            raise NotImplementedError(f"loss='{self.loss}' + penalty='l1' does not support inference_method='{inference_method}'. Use inference_method='bootstrap' or set compute_inference=False.")
        raise NotImplementedError(f"Inference not supported for loss='{self.loss}' × penalty='{penalty_name}'. Use inference_method='bootstrap' or set compute_inference=False.")
    ...
    def _family_for_loss(self):
        ...
        if cached is not None:
            return cached
        from statgpu.glm_core._family import Binomial, Gaussian, Poisson, Gamma, InverseGaussian, NegativeBinomial, Tweedie
        if self.loss == 'logistic':
            ...
            alpha = getattr(getattr(self, '_loss', None), 'alpha', getattr(self, 'loss_kwargs', {}).get('alpha', 1.0))
            fam = NegativeBinomial(alpha=alpha)
        elif self.loss == 'tweedie':
            power = getattr(getattr(self, '_loss', None), 'power', getattr(self, 'loss_kwargs', {}).get('power', 1.5))
