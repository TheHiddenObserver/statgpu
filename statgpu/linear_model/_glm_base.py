...
from statgpu.glm_core._family import Gaussian, Binomial, Poisson, Gamma, InverseGaussian, NegativeBinomial, Tweedie
...
class GeneralizedLinearModel(BaseEstimator):
    ...
    def __init__(self, family: str='gaussian', fit_intercept: bool=True, max_iter: int=100, tol: float=0.0001, C: float=1.0, device: Union[str, Device]=Device.AUTO, n_jobs: Optional[int]=None, solver: str='auto', gpu_memory_cleanup: bool=False, compute_inference: bool=False, cov_type: str='nonrobust'):
    ...
    def _get_family(self):
        ...
        family_map = {'gaussian': Gaussian, 'binomial': Binomial, 'poisson': Poisson, 'gamma': Gamma, 'inverse_gaussian': InverseGaussian, 'negative_binomial': NegativeBinomial, 'tweedie': Tweedie}
        if self.family not in family_map:
            raise ValueError(f"Unknown family '{self.family}'. Supported families: {list(family_map.keys())}")
    ...
    def family_to_loss(self):
        ...
        _map = {'gaussian': 'squared_error', 'binomial': 'logistic', 'poisson': 'poisson', 'gamma': 'gamma', 'inverse_gaussian': 'inverse_gaussian', 'negative_binomial': 'negative_binomial', 'tweedie': 'tweedie'}
        ...
        if self._effective_intercept:
            ...
            if is_gpu:
                if backend == 'torch':
                    ...
                    params_inf = torch.cat([torch.tensor([self.intercept_], dtype=dt, device=dev), torch.as_tensor(self.coef_, dtype=dt, device=dev)])
                else:
                    ones = xp.ones((n, 1), dtype=X_orig.dtype)
                    X_inf = xp.concatenate([ones, X_orig], axis=1)
                    params_inf = xp.concatenate([xp.asarray([self.intercept_], dtype=X_orig.dtype), xp.asarray(self.coef_, dtype=X_orig.dtype)])
            else:
    ...
    def _compute_inference(self):
        ...
        is_gpu = backend != 'numpy'
        result = m_estimation_inference(self._loss, self._X_design, self._y_inf, self._params, cov_type=self._cov_type, penalty_curvature_diag=curv, sample_weight=self._sample_weight_inf)
    ...
    def fit(self, X=None, y=None, sample_weight=None, formula=None, data=None):
        ...
        if formula is not None:
            ...
            y_arr, X_arr, design_info = _parse_formula_if_provided(formula, data, None, None)
        ...
        if _solver_lower == 'auto':
            ...
            if _pname in ('l1', 'scad', 'mcp', 'adaptive_l1', 'adaptive_lasso', 'group_lasso', 'group_mcp', 'group_scad'):
                solver_name = 'fista'
            else:
                solver_name = 'irls'
        else:
        ...
        if solver_name == 'irls':
            ...
            self._fit_smooth_solver(X_arr, y_arr, sample_weight, solver_name, backend_name)
        else:
            raise ValueError("solver must be one of: 'auto', 'irls', 'fista', 'newton', 'lbfgs'")
        from statgpu.backends import _to_numpy, _resolve_backend
        from statgpu.backends._utils import _get_xp
        ...
        if is_gpu:
            ...
            self._X_design, self._params, self._intercept_idx = self._aligned_inference_design_glm(X_arr)
        else:
        ...
        if self._compute_inference_enabled:
            if sample_weight is not None:
                ...
                if is_gpu:
                    self._sample_weight_inf = self._to_array(sw, backend=inf_backend)
                else:
                    self._sample_weight_inf = sw
            else:
                self._sample_weight_inf = None
            self._fit_metadata = {'solver_used': solver_name, 'objective_scale': 'mean_loss_plus_penalty', 'ridge_alpha_avg': None, 'penalty_curvature_diag': None}
            if solver_name == 'irls' and self.C > 0:
                lam = self._get_penalty_alpha()
                if is_gpu:
                    from statgpu.backends._utils import xp_zeros
                    curv = xp_zeros(self._params.shape[0], self._params.dtype, inf_xp, ref_arr=self._params)
                else:
    ...
    def _fit_irls(self, X, y, sample_weight, family, backend_name='numpy'):
        ...
        params, n_iter = solver.fit(X_design, y, sample_weight=sample_weight, ridge_alpha=ridge_alpha, ridge_penalize_intercept=not self._effective_intercept, backend=backend_name)
        self.n_iter_ = n_iter
        self._params = params
    ...
    def _fit_fista(self, X, y, sample_weight, family, backend_name='numpy'):
        ...
        if not self._effective_intercept:
            ...
            if self.family == 'gamma' and loss_kwargs.get('link') == 'inverse_power':
                eta_lo = float(getattr(loss, '_ETA_LO', 0.0001))
                if backend_name == 'cupy':
                    import cupy as cp
                    ...
                    eta_raw = 1.0 / cp.clip(y_cp, 1e-06, None)
                    ...
                    coef_dtype = X_centered.dtype if cp.issubdtype(X_centered.dtype, cp.floating) else cp.float64
                    ...
                    eta_raw = 1.0 / torch.clamp(y_t, min=1e-06)
                    ...
                    eta_raw = 1.0 / np.clip(y_np, 1e-06, None)
        ...
        if loss.name != 'squared_error':
            ...
            y_mean = max(float(_xp_mod.mean(y)), 0.001)
            init = np.zeros(p + 1, dtype=np.float64)
            if self.family == 'binomial':
                p_mean = np.clip(y_mean, 0.001, 1.0 - 0.001)
                init[-1] = np.log(p_mean / (1.0 - p_mean))
            elif self.family == 'gamma' and loss_kwargs.get('link') == 'inverse_power':
                init[-1] = 1.0 / y_mean
            elif self.family in ('poisson', 'gamma', 'inverse_gaussian', 'negative_binomial', 'tweedie'):
    ...
    def _fit_smooth_solver(self, X, y, sample_weight, solver_name, backend_name):
        ...
        if sample_weight is not None:
            raise ValueError(f"solver='{solver_name}' does not support sample_weight yet; use solver='irls' or solver='fista'.")
        loss_kwargs = self._get_loss_kwargs()
        loss = get_glm_loss(self.family_to_loss(), **loss_kwargs)
        ...
        if self._effective_intercept:
            ...
            if backend_name == 'cupy':
                ...
                X_float = X.to(dtype=x_dtype)
                y = y.to(X.device).to(x_dtype)
                X_work = torch.column_stack([X_float, torch.ones(X.shape[0], dtype=x_dtype, device=X.device)])
            else:
        ...
        if solver_name == 'newton':
            params, n_iter = newton_solver(loss, None, X_work, y, max_iter=self._max_iter, tol=self._tol)
        else:
            params, n_iter = lbfgs_solver(loss, None, X_work, y, max_iter=self._max_iter, tol=self._tol)
        params_np = _to_numpy(params)
        self.n_iter_ = n_iter
        if self._effective_intercept:
            ...
            self.coef_ = params_np.copy()
            self.intercept_ = 0.0
        self._params = np.concatenate([[self.intercept_], self.coef_]) if self._effective_intercept else self.coef_.copy()
        self._df_resid = self._nobs - (X.shape[1] + (1 if self._effective_intercept else 0))
...
class OrderedGeneralizedLinearModel(GeneralizedLinearModel):
    ...
    def __init__(self, n_categories: int=3, family: str='binomial', fit_intercept: bool=True, max_iter: int=100, tol: float=0.0001, C: float=1.0, device: Union[str, Device]=Device.AUTO, n_jobs: Optional[int]=None, solver: str='auto', compute_inference: bool=False, cov_type: str='nonrobust', gpu_memory_cleanup: bool=False):
        super().__init__(family=family, fit_intercept=fit_intercept, max_iter=max_iter, tol=tol, C=C, device=device, n_jobs=n_jobs, solver=solver, compute_inference=compute_inference, cov_type=cov_type, gpu_memory_cleanup=gpu_memory_cleanup)
        if n_categories < 2:
            raise ValueError(f'n_categories must be >= 2, got {n_categories}. Ordered models require at least 2 ordinal categories.')
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        """
        if sample_weight is not None:
            raise ValueError('OrderedGeneralizedLinearModel does not support sample_weight yet.')
        backend = self._get_backend(backend='auto')
        backend_name = backend.name
        ...
        if y_min < 0 or y_max >= K:
            raise ValueError(f'Ordered model labels must be integers in [0, {K - 1}], got range [{y_min}, {y_max}]. n_categories={K}.')
        if xp.any(y_flat != xp.floor(y_flat)):
            raise ValueError('Ordered model labels must be integer-coded categories, not continuous values. Found non-integer labels.')
        family = self._get_family()
        n = X.shape[0]
        p = X.shape[1]
        try:
    ...
    def _fit_ordered_newton_impl(self, X, y, family, K, n, p, xp, is_torch, is_cupy, dev=None):
        ...
        theta[p:] = xp.arange(0.5, K - 0.5, dtype=xp.float64, device=dev) if is_torch else xp.arange(0.5, K - 0.5, dtype=xp.float64)
        idx = xp.arange(n, device=dev) if is_torch else xp.arange(n)
        d = len(theta)
        nll_old = xp.inf
        ridge = 0.0001
        if self._max_iter <= 0:
            raise ValueError(f'max_iter must be > 0, got {self._max_iter}. Newton-Raphson requires at least 1 iteration.')
        ...
        def _enforce_thresh_gaps(thresh_arr):
            ...
            if len(t) > 1:
                gaps = xp.diff(t)
                if is_torch:
                    gaps = xp.clamp(gaps, min=1e-06)
                    t = xp.cat([t[:1], t[:1] + xp.cumsum(gaps, dim=0)])
                else:
                    gaps = xp.maximum(gaps, 1e-06)
        ...
        for iteration in range(self._max_iter):
            ...
            if is_torch:
                grad = self._ordered_gradient_torch(Xs, y, beta, thresh, prob, prob_c, family, K, n, eta=eta)
            else:
                grad = self._ordered_gradient(Xs, y, beta, thresh, prob, prob_c, family, K, n, eta=eta)
            if not xp.isfinite(nll):
                raise RuntimeError(f'NLL became non-finite ({float(nll):.4g}) at iteration {iteration}. Coefficients may have diverged.')
            ...
            if grad_inf < self._tol:
                break
            nll_old = nll
            H = self._ordered_hessian_analytical(Xs, y, beta, thresh, family, K, prob, prob_c, eta=eta)
            H_avg = H / n
    ...
    def _fit_scipy_ordered(self, X, y, family, K, n, p):
        ...
        self._fit_ordered_newton_impl(X, y, family, K, n, p, np, is_torch=False, is_cupy=False)
    ...
    def _fit_cupy_ordered(self, X, y, family, K, n, p):
        ...
        self._fit_ordered_newton_impl(X, y, family, K, n, p, cp, is_torch=False, is_cupy=True)
    ...
    def _fit_torch_ordered(self, X, y, family, K, n, p):
        ...
        if not isinstance(y, torch.Tensor):
            ...
            y = y.to(torch.int64)
        self._fit_ordered_newton_impl(X, y, family, K, n, p, torch, is_torch=True, is_cupy=False, dev=dev)

    def _ordered_category_probs(self, X, beta, thresh, family, K, eta=None):
        """Compute category probabilities P(y=j|X), shape (K, n)."""
        if eta is None:
            eta = X @ beta
        pi = family.link.inverse(thresh[:, None] - eta[None, :])
    ...
    def _compute_ordered_inference(self, X_orig, y_orig):
        ...
        if cov_type not in ('nonrobust',):
            raise NotImplementedError(f"Ordered model inference only supports cov_type='nonrobust', got '{self._cov_type}'. HC0/HC1 sandwich and penalized inference are not yet available for ordered models.")
        import numpy as np
        from statgpu.backends import _to_numpy, _resolve_backend
        ...
        try:
            ...
            raise np.linalg.LinAlgError("Ordered model Hessian is singular — cannot compute standard errors. This may indicate quasi-complete separation or redundant thresholds. Consider using inference_method='bootstrap' or reducing n_categories.") from e
        ...
        conf_int = xp.column_stack([params - z_crit * bse, params + z_crit * bse])
        ...
        self._inference_result = ParameterInferenceResult(method='analytical_hessian', params=self._params.copy(), bse=self._bse.copy(), statistic=self._zvalues.copy(), statistic_name='z', pvalues=self._pvalues.copy(), conf_int=self._conf_int.copy(), distribution='normal', feature_names=feat_names, metadata={'method': 'analytical', 'n_thresholds': n_thresh, 'backend': backend})
        self._inference_result.apply_to(self)
    ...
    def _ordered_gradient(self, X, y, beta, thresh, prob, prob_clipped, family, K, n, eta=None):
        ...
        inv_prob = 1.0 / prob_clipped[y, xp.arange(n)]
        ...
        scalar[idx_mid] = deriv_all[y[idx_mid] - 1, idx_mid] - deriv_all[y[idx_mid], idx_mid]
    ...
    def predict_proba(self, X):
        ...
        if is_torch:
            proba = xp.diff(pi, dim=0).T
        else:
            proba = xp.diff(pi, axis=0).T
