...
def _default_logistic_c_grid(X, y, n_Cs: int=100, C_min_ratio: float=0.001):
    """
    ...
    Cs = np.logspace(np.log10(C_min), np.log10(C_max), num=n_Cs, dtype=np.float64)
    return Cs
...
def _batch_log_loss(y_val, probs_desc, sample_weight=None):
    ...
    ll = -(y_val.reshape(1, -1) * np.log(probs_clipped) + (1 - y_val.reshape(1, -1)) * np.log(1 - probs_clipped))
...
def _batch_log_loss_backend(y_val, probs_desc, backend, sample_weight=None):
    ...
    ll = -(y_val.reshape(1, -1) * xp.log(probs_clipped) + (1 - y_val.reshape(1, -1)) * xp.log(1 - probs_clipped))
    if sample_weight is not None:
...
def _solve_logistic_path_gpu_from_batch(X_batch, y_batch, n_train_vec, Cs, backend, fit_intercept=True, max_iter=100, tol=0.0001, sw_batch=None):
    """
    ...
    coefs_desc = np.stack(all_coefs, axis=1)
    intercepts_desc = np.stack(all_intercepts, axis=1)
    return (coefs_desc, intercepts_desc)

def _select_logistic_c_cv(X, y, *, Cs=None, n_Cs: int=100, C_min_ratio: float=0.001, cv_folds: int=5, cv_splits=None, random_state: Optional[int]=None, sample_weight=None, fit_intercept: bool=True, max_iter: int=100, tol: float=0.0001, device: Union[str, Device]=Device.CPU, return_details: bool=False, cache_key: Optional[Tuple[Any, ...]]=None, gpu_cv_mixed_precision: bool=True):
    """
    ...
    if int(n_samples) < 4 or int(C_grid.size) == 1 or int(cv_folds) < 2:
        ...
        if not return_details:
            return C0
        return {'C': C0, 'Cs': C_grid.astype(np.float64, copy=False), 'loss_path': np.full((int(C_grid.size), 1), np.nan, dtype=np.float64), 'mean_loss': np.full(int(C_grid.size), np.nan, dtype=np.float64)}
    ...
    if use_gpu:
        try:
            ...
            coefs_batch, intercepts_batch = _solve_logistic_path_gpu_from_batch(X_batch, y_batch, n_train_vec, C_grid, backend, fit_intercept=bool(fit_intercept), max_iter=max_iter, tol=tol, sw_batch=sw_batch)
            for fold_idx in range(n_folds):
                X_val, y_val, sw_val = fold_eval_payload[fold_idx]
                n_val = int(X_val.shape[0])
                coefs_all = backend.asarray(coefs_batch[:, fold_idx, :])
                intercepts_all = backend.asarray(intercepts_batch[:, fold_idx])
                xp = backend.xp
            ...
            raise RuntimeError("GPU path failed in _select_logistic_c_cv with device='cuda'; CPU fallback is disabled for strict CUDA execution.") from exc
    if not use_gpu:
        if gpu_requested:
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(folds):
            ...
            fold_losses = []
            for C in C_grid:
                model = LogisticRegression(C=C, fit_intercept=fit_intercept, max_iter=max_iter, tol=tol, device='cpu', compute_inference=False)
    ...
    details = {'C': best_C, 'Cs': C_grid, 'loss_path': loss_path, 'mean_loss': mean_loss}
...
class LogisticRegressionCV(CVEstimatorBase):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if not np.all(np.isin(unique_y, [0.0, 1.0])):
            raise ValueError(f'LogisticRegressionCV requires binary y (0 or 1), got unique values: {unique_y[:10]}')
        device_name = self._get_compute_device().value
        details = _select_logistic_c_cv(X, y, Cs=self.Cs, n_Cs=self._n_Cs, C_min_ratio=self._C_min_ratio, cv_folds=self._cv, cv_splits=self.cv_splits, random_state=self.random_state, sample_weight=sample_weight, fit_intercept=self._fit_intercept, max_iter=self._max_iter, tol=self._tol, device=device_name, gpu_cv_mixed_precision=self._gpu_cv_mixed_precision, return_details=True)
        ...
        estimator = LogisticRegression(C=self.C_, fit_intercept=self._fit_intercept, max_iter=self._max_iter, tol=self._tol, device=self._device, n_jobs=self.n_jobs, compute_inference=self._compute_inference_enabled, cov_type=self._cov_type, gpu_memory_cleanup=self._gpu_memory_cleanup)
