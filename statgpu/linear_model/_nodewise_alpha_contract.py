"""Public/runtime contract for node-wise tuning in sparse Gaussian inference.

This is the single compatibility installer for the node-wise tuning migration.
It is intentionally installed after the existing #135/#138 compatibility
contracts because those contracts capture the maintained debiased-inference
originals at import time. Statistical/numerical work lives in normal helper
modules under ``linear_model.penalized``; this layer only joins that work to the
already-installed public runtime without adding separate cache/GPU installers.
"""
...
from statgpu.linear_model.penalized._nodewise_precision_cache import (
...
def _gpu_effective_n(n_rows: int, sample_weight, backend_name: str, ref_arr) -> float:
    """Compute Kish effective n on the concrete device with scalar sync only."""
    n = int(n_rows)
...
def _install_cv_resolved_property(cls):
    def get_resolved(self):
        ...
        return None if estimator is None else getattr(estimator, "nodewise_alpha_", None)
...
def _gpu_scope_applies(model) -> bool:
    method = str(
...
def _install_gpu_context(cls):
    ...
    def wrapped(self, X, y, sample_weight=None, backend_name="cupy"):
        if not _gpu_scope_applies(self):
            return current(self, X, y, sample_weight, backend_name=backend_name)
        n = int(X.shape[0])
...
def install_nodewise_alpha_contract() -> None:
    """Install the reviewed node-wise public/runtime migration once."""
    for cls in (
    ...
    # #138 centered/weighted wrappers invoke these captured originals.
    ...
    _install_gpu_context(PenalizedGeneralizedLinearModel)
    _install_gpu_context(PenalizedLinearRegression)


__all__ = ["install_nodewise_alpha_contract"]
