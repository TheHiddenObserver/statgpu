...
_NUMBA_CD_DISABLED = str(os.getenv('STATGPU_DISABLE_NUMBA_CD', '0')).strip().lower() in ('1', 'true', 'yes', 'on')
...
def _debiased_m_key_from_numpy_design(X: np.ndarray, *, n: int, p: int, lam_nw: float, tol: float):
...
def _debiased_m_key_from_sample(*, n: int, p: int, dtype_name: str, sample_block: np.ndarray, lam_nw: float, tol: float):
...
class Lasso(BaseEstimator):
    ...
    def __init__(self, alpha: float=1.0, fit_intercept: bool=True, max_iter: int=1000, tol: float=0.0001, stopping: str='coef_delta', inference_method: str='cpu_ols_inference', n_bootstrap: int=200, bootstrap_random_state: Optional[int]=None, enable_simultaneous_inference: bool=False, simultaneous_method: str='maxz_bootstrap', simultaneous_alpha: float=0.05, simultaneous_n_bootstrap: int=1000, simultaneous_random_state: Optional[int]=None, simultaneous_include_intercept: bool=False, device: Union[str, Device]=Device.AUTO, n_jobs: Optional[int]=None, compute_inference: bool=True, solver: str='fista', cpu_solver: str='coordinate_descent', lipschitz_L: Optional[float]=None, admm_rho: float=1.0, gpu_memory_cleanup: bool=False):
        ...
        alias_map = {'naive_ols': 'cpu_ols_inference', 'gpu_naive_ols': 'gpu_ols_inference'}
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if device == Device.CPU and self.inference_method == 'gpu_ols_inference':
            raise ValueError("inference_method='gpu_ols_inference' requires device='cuda' or device='torch'. Use inference_method='cpu_ols_inference' on CPU.")
        ...
        elif not self.compute_inference or self.inference_method in ('gpu_ols_inference', 'debiased'):
            self._y = None
        else:
            self._y = self._to_numpy(y)
        if self.compute_inference and device in (Device.CUDA, Device.TORCH) and (self.inference_method not in ('gpu_ols_inference', 'debiased')):
            raise NotImplementedError(f"Lasso inference_method='{self.inference_method}' is not implemented for device='{device.value}' without CPU fallback.")
    ...
    def _validate_simultaneous_config(self):
        ...
        if not self.compute_inference:
            raise ValueError('enable_simultaneous_inference=True requires compute_inference=True.')
        ...
        if self.simultaneous_method != 'maxz_bootstrap':
            raise ValueError("simultaneous_method must be 'maxz_bootstrap'.")
    ...
    def _build_inference_cautions(self):
        ...
        if self.inference_method in ('cpu_ols_inference', 'gpu_ols_inference'):
            cautions.append('Lasso OLS-style post-selection intervals are heuristic and do not provide valid selective-inference confidence coverage.')
        if self.inference_method == 'debiased':
            cautions.append('Debiased Lasso currently reports per-coefficient (marginal) confidence intervals only; joint/multiple-testing coverage is not guaranteed.')
        if self._simultaneous_enabled:
            target_txt = 'including intercept' if self.fit_intercept and self.simultaneous_include_intercept else 'excluding intercept'
            cautions.append(f'Simultaneous inference enabled via maxz_bootstrap with joint coverage target set {target_txt}.')
            if self.fit_intercept and self.simultaneous_include_intercept:
                cautions.append('Intercept is included using the same max-|Z| critical value calibrated on feature coefficients.')
        return cautions
    ...
    def _get_cupy_fused_kernels():
        ...
        def _kkt_violation_fused(grad, alpha):
            ...
            diff = abs_grad - alpha
            return (diff > 0) * diff
        SOFT_THRESHOLD_KERNEL = cp.ElementwiseKernel('float64 x, float64 gamma', 'float64 y', '\n            double abs_x = abs(x);\n            if (abs_x > gamma) {\n                y = (x > 0 ? 1.0 : -1.0) * (abs_x - gamma);\n            } else {\n                y = 0.0;\n            }\n            ', 'lasso_soft_threshold')
        ABS_DELTA_KERNEL = cp.ElementwiseKernel('float64 a, float64 b', 'float64 y', '\n            double diff = a - b;\n            y = (diff > 0 ? diff : -diff);\n            ', 'lasso_abs_delta')
        Lasso._cupy_fused_kernels = {'soft_threshold': _soft_threshold_fused, 'fista_momentum': _fista_momentum_fused, 'kkt_violation': _kkt_violation_fused, 'elementwise_kernel': SOFT_THRESHOLD_KERNEL, 'abs_delta_kernel': ABS_DELTA_KERNEL}
        return Lasso._cupy_fused_kernels
                ...
                for iteration in range(self.max_iter):
                    ...
                    if self.stopping == 'kkt':
                        ...
                        if violation < self.tol:
                            ...
                            break
                    elif np.sum(np.abs(coef - coef_old)) < self.tol:
                        self.n_iter_ = iteration + 1
                        break
                else:
    ...
    def _solve_lasso_path_torch_fista_multi_fold_from_gram(self, XtX_batch, Xty_batch, *, n_samples_vec, alphas_desc, max_iter, tol, stopping, lipschitz_L=None, check_every=8):
    ...
    def _compute_inference_debiased_torch(self, X_torch, y_torch, coef_torch):
        ...
        m_cache_key = _debiased_m_key_from_sample(n=n, p=p, dtype_name=str(dtype), sample_block=X_sample, lam_nw=lam_nw, tol=float(self.tol))
        ...
        if M_cached is not None:
            ...
            for j0 in range(0, p, chunk_size):
                ...
                XtX_batch = XtX_full[cols_batch[:, :, None], cols_batch[:, None, :]]
                Xty_batch = XtX_full[cols_batch, j_batch.reshape(-1, 1)].reshape(bsz, p - 1)
                coefs_batch_desc, _ = self._solve_lasso_path_torch_fista_multi_fold_from_gram(XtX_batch, Xty_batch, n_samples_vec=np.full((bsz,), float(n), dtype=np.float64), alphas_desc=alpha_nw, max_iter=500, tol=1e-05, stopping='coef_delta', lipschitz_L=None, check_every=8)
        ...
        if self.fit_intercept:
            ...
            ci_intercept = torch.stack([intercept_torch - z_crit * se_intercept, intercept_torch + z_crit * se_intercept]).reshape(1, 2)
        ...
        if self.enable_simultaneous_inference:
            self._compute_simultaneous_inference_torch(params_torch, bse_torch, se, M, X_torch, resid_full, n)

    def _compute_simultaneous_inference_torch(self, params_torch, bse_torch, se_feat_torch, M_torch, X_torch, resid_full_torch, n):
    ...
    def _fit_torch(self, X, y, sample_weight=None):
            ...
            for iteration in range(self.max_iter):
                ...
                if self.stopping == 'kkt':
                    ...
                    if violation < self.tol:
                        self.n_iter_ = iteration + 1
                        break
                elif torch.sum(torch.abs(coef - coef_old)) < self.tol:
                    self.n_iter_ = iteration + 1
                    break
            else:
                self.n_iter_ = self.max_iter
        ...
        if self.compute_inference:
            ...
            if self.inference_method == 'gpu_ols_inference':
                ...
                raise NotImplementedError(f"Lasso inference_method='{self.inference_method}' is not implemented for Torch without CPU fallback.")
        else:
            self._scale = np.nan
            self._resid = None
            self._X_design = None
    ...
    def _fit_gpu_admm(self, X, y, sample_weight=None):
        ...
        if self.fit_intercept:
            ...
            y_centered = y
        coef = cp.zeros(n_features, dtype=X.dtype)
        z = cp.zeros(n_features, dtype=X.dtype)
        u = cp.zeros(n_features, dtype=X.dtype)
        XtX = X_centered.T @ X_centered
        Xty = X_centered.T @ y_centered
        ...
        for iteration in range(self.max_iter):
            ...
            if self.stopping == 'kkt':
                ...
                if violation < self.tol:
                    self.n_iter_ = iteration + 1
                    break
            elif cp.sum(cp.abs(coef - coef_old)) < self.tol:
                self.n_iter_ = iteration + 1
                break
            z = z
        else:
        ...
        if self.compute_inference:
            ...
            if self.inference_method == 'gpu_ols_inference':
                ...
                raise NotImplementedError(f"Lasso inference_method='{self.inference_method}' is not implemented for CuPy without CPU fallback.")
        else:
            self._scale = np.nan
            self._resid = None
            self._X_design = None
    ...
    def _compute_inference_bootstrap(self) -> None:
        ...
        for b in range(self.n_bootstrap):
            ...
            refit = Lasso(alpha=self.alpha, fit_intercept=self.fit_intercept, max_iter=self.max_iter, tol=self.tol, stopping=self.stopping, inference_method='cpu_ols_inference', n_bootstrap=0, bootstrap_random_state=None, device='cpu', compute_inference=False, solver=self.solver, cpu_solver=self.cpu_solver, lipschitz_L=lipschitz_L, admm_rho=self.admm_rho)
            if self.fit_intercept:
                X_refit = X[:, 1:]
            else:
                X_refit = X
        ...
        self._conf_int = np.column_stack([np.quantile(boot_params, lower_q, axis=0), np.quantile(boot_params, upper_q, axis=0)])
    ...
    def _compute_inference_debiased(self) -> None:
        ...
        m_cache_key = _debiased_m_key_from_numpy_design(X, n=n, p=p, lam_nw=lam_nw, tol=float(self.tol))
        M_cached = _debiased_m_cache_get(m_cache_key)
        if M_cached is not None:
            ...
            for j in range(p):
                ...
                X_minus_j = X[:, cols]
                x_j = X[:, j]
                nw = Lasso(alpha=lam_nw, fit_intercept=False, max_iter=500, tol=1e-05, device='cpu', cpu_solver='fista', compute_inference=False)
        ...
        if self.fit_intercept:
            ...
            ci_intercept = np.array([self.intercept_ - z_crit * se_intercept, self.intercept_ + z_crit * se_intercept])
    ...
    def _compute_inference_debiased_gpu(self, X_gpu, y_gpu, coef_gpu):
        ...
        if M_cached is not None:
            ...
            for j0 in range(0, p, chunk_size):
                ...
                cols_batch = base + (base >= j_batch.reshape(-1, 1))
                XtX_batch = XtX_full[cols_batch[:, :, cp.newaxis], cols_batch[:, cp.newaxis, :]]
                Xty_batch = XtX_full[cols_batch, j_batch.reshape(-1, 1)].reshape(bsz, p - 1)
                coefs_batch_desc, _ = _solve_lasso_path_gpu_fista_multi_fold_from_gram(XtX_batch, Xty_batch, n_samples_vec=np.full((bsz,), float(n), dtype=n_samp_vec_dtype), alphas_desc=alpha_nw, max_iter=500, tol=1e-05, stopping='coef_delta', lipschitz_L=None, check_every=8)
        ...
        if self.fit_intercept:
            ...
            ci_intercept = cp.stack([intercept_gpu - z_crit * se_intercept, intercept_gpu + z_crit * se_intercept]).reshape(1, 2)
            ...
            params_gpu = theta_db
        if self.enable_simultaneous_inference:
            param_target_idx_np = self._get_simultaneous_target_indices(int(params_gpu.shape[0]))
            ...
            try:
                ...
                while filled < B:
                    ...
                    score_target = weighted @ X_gpu @ M_target.T / float(max(n, 1))
                    z_star_target = score_target / (se_target_gpu.reshape(1, -1) + 1e-30)
                    max_stats_gpu[filled:filled + bsz] = cp.max(cp.abs(z_star_target), axis=1)
                    filled += bsz
            critical_gpu = cp.quantile(max_stats_gpu, 1.0 - float(self.simultaneous_alpha))
            conf_sim_gpu = cp.array(conf_int_gpu, copy=True)
            lower_gpu = cp.take(params_gpu, param_target_idx_gpu) - critical_gpu * cp.take(bse_gpu, param_target_idx_gpu)
            upper_gpu = cp.take(params_gpu, param_target_idx_gpu) + critical_gpu * cp.take(bse_gpu, param_target_idx_gpu)
    ...
    def _compute_simultaneous_inference(self):
        ...
        if self._params is None or self._bse is None or self._conf_int is None:
            return
        if self._X_design is None or self._resid is None:
            raise RuntimeError('Simultaneous debiased inference requires accessible design/residual state; re-fit with compute_inference=True.')
    ...
    def compute_debiased_inference_(self):
        ...
        warnings.warn('compute_debiased_inference_ is deprecated and will be removed in a future release; use compute_debiased_inference instead.', DeprecationWarning, stacklevel=2)
    ...
    def compute_simultaneous_inference(self):
        ...
        self._check_is_fitted()
        if not self.enable_simultaneous_inference:
            raise ValueError('compute_simultaneous_inference requires enable_simultaneous_inference=True.')
        self._compute_simultaneous_inference()
        return self

    def compute_simultaneous_inference_(self):
        """Deprecated alias for :meth:`compute_simultaneous_inference`."""
        warnings.warn('compute_simultaneous_inference_ is deprecated and will be removed in a future release; use compute_simultaneous_inference instead.', DeprecationWarning, stacklevel=2)
    ...
    def _compute_simultaneous_ci_maxz_bootstrap(self):
        ...
        while filled < B:
            ...
            max_stats[filled:filled + bsz] = np.max(np.abs(z_star[:, feature_target_idx]), axis=1)
            filled += bsz
    ...
    def summary(self):
        ...
        if self._bse is None or self._pvalues is None or self._conf_int is None:
            raise RuntimeError('compute_inference=False: inference statistics are not available. Re-fit with compute_inference=True (default) to use summary().')
        if self.fit_intercept:
        ...
        if self._simultaneous_enabled and self._conf_int_simultaneous is not None:
            target_txt = 'include_intercept=True' if self.fit_intercept and self.simultaneous_include_intercept else 'include_intercept=False'
            print('-' * 80)
...
def _lasso_alpha_heuristic(y_centered: np.ndarray, n_features: int) -> float:
    ...
    sigma_hat = max(sigma_hat, 1e-08)
    penalty_scale = np.sqrt(2.0 * np.log(max(2, int(n_features))) / max(1, n_samples))
    return float(sigma_hat * penalty_scale)

def _default_lasso_alpha_grid(X: np.ndarray, y: np.ndarray, n_alphas: int=12, alpha_min_ratio: float=0.001) -> np.ndarray:
    ...
    alpha_max = max(alpha_max, 1e-06)
    if int(n_alphas) <= 1:
        return np.asarray([alpha_max], dtype=np.float64)
    alpha_min = max(float(alpha_min_ratio) * alpha_max, 1e-06)
    return np.geomspace(alpha_max, alpha_min, num=int(n_alphas)).astype(np.float64)

def _default_lasso_alpha_grid_backend(X, y, backend, n_alphas: int=12, alpha_min_ratio: float=0.001) -> np.ndarray:
    ...
    alpha_max = max(alpha_max, float(sigma_hat * penalty_scale), 1e-06)
    if int(n_alphas) <= 1:
        return np.asarray([alpha_max], dtype=np.float64)
    alpha_min = max(float(alpha_min_ratio) * alpha_max, 1e-06)
    return np.geomspace(alpha_max, alpha_min, num=int(n_alphas)).astype(np.float64)

def _default_lasso_alpha_grid_cupy(X, y, n_alphas: int=12, alpha_min_ratio: float=0.001) -> np.ndarray:
    import cupy as cp
...
def _normalize_cv_splits(cv_splits, n_samples: int):
    ...
    for split in cv_splits:
        ...
        if train_idx.size == 0 or val_idx.size == 0:
            continue
        if bool(np.any(train_idx < 0)) or bool(np.any(train_idx >= n)) or bool(np.any(val_idx < 0)) or bool(np.any(val_idx >= n)):
...
def _make_lasso_cv_auto_cache_key(*, X, y, sample_weight, alpha_grid: np.ndarray, folds, fit_intercept: bool, use_gpu: bool, max_iter: int, tol: float, cpu_solver: str, cv_method: str, cd_kkt_check_every: Optional[int], gpu_cv_mixed_precision: bool) -> Tuple[Any, ...]:
    return ('lasso_cv_auto_v1', _array_identity_token(X), _array_identity_token(y), _array_identity_token(sample_weight), _alphas_signature(alpha_grid), _folds_signature(folds), bool(fit_intercept), bool(use_gpu), int(max_iter), float(tol), str(cpu_solver).lower(), str(cv_method).lower(), None if cd_kkt_check_every is None else int(cd_kkt_check_every), bool(gpu_cv_mixed_precision))

def _clone_lasso_cv_cache_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    return {'alpha': float(payload['alpha']), 'alphas': np.asarray(payload['alphas'], dtype=np.float64).copy(), 'mse_path': np.asarray(payload['mse_path'], dtype=np.float64).copy(), 'mean_mse': np.asarray(payload['mean_mse'], dtype=np.float64).copy()}
...
def _adaptive_gpu_check_every(*, base_check_every: int, iteration: int, max_iter: int, active_ratio: float) -> int:
...
if _NUMBA_AVAILABLE:
    ...
    @njit(cache=True)
    def _solve_lasso_path_cpu_cd_numba_impl(XtX: np.ndarray, Xty: np.ndarray, n_samples: int, alphas_desc: np.ndarray, max_iter: int, tol: float, stopping_is_kkt: bool, cd_kkt_check_every: int) -> tuple[np.ndarray, np.ndarray]:
        ...
        for alpha_idx in range(n_alphas):
            ...
            for iteration in range(int(max_iter)):
                ...
                should_kkt_scan = (iteration + 1) % check_every == 0 or coef_delta_l1 < float(tol) or iteration + 1 == int(max_iter)
                violation = 0.0
                ...
                if stopping_is_kkt:
                    if should_kkt_scan and violation < float(tol):
                        ...
                        converged = True
                        break
                elif coef_delta_l1 < float(tol) and (not has_inactive_violation):
                    n_iters[alpha_idx] = int(iteration) + 1
                    converged = True
                    break
            if not converged:
...
def _solve_lasso_path_cpu_cd_numba(XtX: np.ndarray, Xty: np.ndarray, *, n_samples: int, alphas_desc: np.ndarray, max_iter: int, tol: float, stopping: str, cd_kkt_check_every: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    return _solve_lasso_path_cpu_cd_numba_impl(XtX_c, Xty_c, int(n_samples), alphas_c, int(max_iter), float(tol), bool(stopping_is_kkt), int(cd_kkt_check_every))

def _normalize_lassocv_method(method: str) -> str:
    """Normalize CV optimization profile name."""
    key = str(method).strip().lower()
    alias_map = {'default': 'standard', 'classic': 'standard', 'glmnet_cv': 'glmnet', 'glmnet.cv': 'glmnet'}
...
def _solve_lasso_path_cpu_fista_batched_from_gram(XtX: np.ndarray, Xty: np.ndarray, *, n_samples: int, alphas_desc: np.ndarray, max_iter: int, tol: float, stopping: str, lipschitz_L: Optional[float]=None, check_every: int=2) -> tuple[np.ndarray, np.ndarray]:
    ...
    for iteration in range(int(max_iter)):
        ...
        if stopping_name == 'kkt':
            ...
            viol = np.max(np.maximum(np.abs(grad_sse) - alphas_desc[active].reshape(1, -1), 0.0), axis=0)
...
def _solve_lasso_path_gpu_fista_batched_from_gram(XtX, Xty, *, n_samples: int, alphas_desc: np.ndarray, max_iter: int, tol: float, stopping: str, lipschitz_L: Optional[float]=None, check_every: int=8):
    ...
    for iteration in range(int(max_iter)):
        ...
        check_every_eff = _adaptive_gpu_check_every(base_check_every=check_every, iteration=iteration, max_iter=int(max_iter), active_ratio=active_ratio)
        should_check = (iteration + 1) % check_every_eff == 0 or iteration + 1 == int(max_iter)
        if not should_check:
            continue
        if stopping_name == 'kkt':
            grad_sse = (XtX @ coef_new - Xty.reshape(-1, 1)) / n_samp
            viol = cp.max(cp.maximum(cp.abs(grad_sse) - alpha_gpu[active_gpu].reshape(1, -1), 0.0), axis=0)
...
def _solve_lasso_path_gpu_fista_multi_fold_from_gram(XtX_batch, Xty_batch, *, n_samples_vec, alphas_desc, max_iter: int, tol: float, stopping: str, lipschitz_L: Optional[float]=None, check_every: int=8):
...
def _solve_lasso_path_cpu_from_gram(XtX: np.ndarray, Xty: np.ndarray, *, n_samples: int, alphas_desc: np.ndarray, max_iter: int, tol: float, stopping: str, cpu_solver: str, lipschitz_L: Optional[float]=None, cd_kkt_check_every: int=1) -> tuple[np.ndarray, np.ndarray]:
    ...
    if solver_name == 'fista':
        return _solve_lasso_path_cpu_fista_batched_from_gram(XtX, Xty, n_samples=n_samples, alphas_desc=alphas_desc, max_iter=max_iter, tol=tol, stopping=stopping, lipschitz_L=lipschitz_L, check_every=2)
    global _NUMBA_CD_DISABLED
    use_numba_cd = _NUMBA_AVAILABLE and (not _NUMBA_CD_DISABLED) and (solver_name == 'coordinate_descent')
    if use_numba_cd:
        try:
            return _solve_lasso_path_cpu_cd_numba(XtX, Xty, n_samples=n_samples, alphas_desc=alphas_desc, max_iter=max_iter, tol=tol, stopping=stopping, cd_kkt_check_every=cd_kkt_check_every)
        except Exception:
        ...
        for iteration in range(int(max_iter)):
            ...
            for j in active_idx:
                ...
                if abs(delta) > 0.0:
                    ...
                    grad += XtX[:, j] * delta
                    coef_delta_l1 += abs(delta)
            should_kkt_scan = (iteration + 1) % cd_kkt_check_every == 0 or coef_delta_l1 < float(tol) or iteration + 1 == int(max_iter)
            violation = float('inf')
            inactive_violation_idx = np.empty((0,), dtype=np.int64)
            if should_kkt_scan:
                violation_vec = np.maximum(np.abs(grad / float(max(1, n_samples))) - float(alpha), 0.0)
            ...
            if stopping_name == 'kkt':
                if should_kkt_scan and violation < float(tol):
                    ...
                    converged = True
                    break
            elif coef_delta_l1 < float(tol) and inactive_violation_idx.size == 0:
                n_iters[alpha_idx] = iteration + 1
                converged = True
                break
        if not converged:
...
def _solve_lasso_path_gpu_from_gram(XtX, Xty, *, n_samples: int, alphas_desc: np.ndarray, max_iter: int, tol: float, stopping: str, lipschitz_L: Optional[float]=None, check_every: int=8):
    """Solve a descending-alpha Lasso path on GPU using one precomputed Gram matrix."""
    return _solve_lasso_path_gpu_fista_batched_from_gram(XtX, Xty, n_samples=n_samples, alphas_desc=alphas_desc, max_iter=max_iter, tol=tol, stopping=stopping, lipschitz_L=lipschitz_L, check_every=check_every)

def _batch_mse_numpy(X_val: np.ndarray, y_val: np.ndarray, coefs_path: np.ndarray, intercepts_path: np.ndarray, sample_weight_val: Optional[np.ndarray]) -> np.ndarray:
...
def _batch_mse(X_val, y_val, coefs_path, intercepts_path, backend, sample_weight_val) -> np.ndarray:
    """
...
def _fit_lasso_single_alpha_fast(X, y, *, alpha: float, fit_intercept: bool, max_iter: int, tol: float, stopping: str, device: str, cpu_solver: str, cd_kkt_check_every: int=1, sample_weight=None) -> Dict[str, object]:
    ...
    if device_name == Device.CUDA.value and (not is_torch_gpu):
        ...
        XtX = X_centered.T @ X_centered
        Xty = X_centered.T @ y_centered
        coefs_desc, n_iters = _solve_lasso_path_gpu_from_gram(XtX, Xty, n_samples=int(X_arr.shape[0]), alphas_desc=alpha_vec, max_iter=int(max_iter), tol=float(tol), stopping=str(stopping), lipschitz_L=None, check_every=8)
        ...
        return {'coef': coef, 'intercept': float(intercept), 'n_iter': int(n_iters[0]), 'n_samples': int(X_arr.shape[0]), 'n_features': int(X_arr.shape[1])}
    elif is_torch_gpu:
        import torch
        X_arr = X
        y_arr = y.reshape(-1) if isinstance(y, torch.Tensor) else torch.as_tensor(y, dtype=X_arr.dtype, device=X_arr.device).reshape(-1)
        if sample_weight is not None:
            sw = sample_weight if isinstance(sample_weight, torch.Tensor) else torch.as_tensor(sample_weight, dtype=X_arr.dtype, device=X_arr.device)
        ...
        for iteration in range(int(max_iter)):
            ...
            if str(stopping).lower() == 'kkt':
                ...
                if violation < float(tol):
                    break
            elif torch.sum(torch.abs(coef - coef_old)) < float(tol):
                break
        if bool(fit_intercept):
            intercept_torch = y_mean - X_mean @ coef
        ...
        return {'coef': coef_np, 'intercept': float(intercept), 'n_iter': int(iteration + 1), 'n_samples': n_samples, 'n_features': n_features}
    X_arr = np.asarray(X)
    ...
    XtX = X_centered.T @ X_centered
    Xty = X_centered.T @ y_centered
    coefs_desc, n_iters = _solve_lasso_path_cpu_from_gram(XtX, Xty, n_samples=int(X_arr.shape[0]), alphas_desc=alpha_vec, max_iter=int(max_iter), tol=float(tol), stopping=str(stopping), cpu_solver=str(cpu_solver), lipschitz_L=None, cd_kkt_check_every=int(cd_kkt_check_every))
...
def _select_lasso_alpha_cv(X, y, *, alphas=None, n_alphas: int=12, alpha_min_ratio: float=0.001, cv_folds: int=5, cv_splits=None, random_state: Optional[int]=None, sample_weight=None, fit_intercept: bool=False, device: Union[str, Device]=Device.CPU, max_iter: int=3000, tol: float=0.0001, cpu_solver: str='coordinate_descent', method: str='standard', cd_kkt_check_every: Optional[int]=None, gpu_cv_mixed_precision: bool=True, return_details: bool=False, cache_key: Optional[Tuple[Any, ...]]=None):
    """
    ...
    if alphas is None:
        if gpu_input_cupy or gpu_input_torch:
            ...
            alpha_grid = _default_lasso_alpha_grid_backend(X, y, backend, n_alphas=n_alphas, alpha_min_ratio=alpha_min_ratio)
        else:
            alpha_grid = _default_lasso_alpha_grid(X_np, y_np, n_alphas=n_alphas, alpha_min_ratio=alpha_min_ratio)
    else:
    ...
    if use_gpu:
        try:
            ...
            if hasattr(xp, '__name__') and 'torch' in xp.__name__.lower():
                ...
                coefs_batch_desc, _ = _solve_lasso_path_gpu_fista_multi_fold_from_gram_torch(XtX_batch, Xty_batch, n_samples_vec=n_samples_vec_torch, alphas_desc=alpha_desc, max_iter=int(max_iter), tol=float(tol), stopping='coef_delta', lipschitz_L=None, check_every=8)
                for fold_idx in range(int(len(folds))):
                    coefs_desc_np = coefs_batch_desc[fold_idx]
                    if bool(fit_intercept):
                ...
                coefs_batch_desc, _ = _solve_lasso_path_gpu_fista_multi_fold_from_gram(XtX_batch, Xty_batch, n_samples_vec=n_samples_vec_cp, alphas_desc=alpha_desc, max_iter=int(max_iter), tol=float(tol), stopping='coef_delta', lipschitz_L=None, check_every=8)
                for fold_idx in range(int(len(folds))):
            ...
            raise RuntimeError("GPU path failed in _select_lasso_alpha_cv with device='cuda'; CPU fallback is disabled for strict CUDA execution.") from exc
    if not use_gpu:
        if gpu_requested:
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(folds):
            ...
            if fast_fold_stats:
                ...
                Xty = X_centered.T @ y_centered
                n_train = int(X_train.shape[0])
            coefs_desc, _ = _solve_lasso_path_cpu_from_gram(XtX, Xty, n_samples=int(n_train), alphas_desc=alpha_desc, max_iter=int(max_iter), tol=float(tol), stopping='coef_delta', cpu_solver=cpu_solver_name, lipschitz_L=None, cd_kkt_check_every=cd_kkt_check_every_effective)
            if bool(fit_intercept):
    ...
    details = {'alpha': float(best_alpha), 'alphas': alpha_grid.astype(np.float64, copy=False), 'mse_path': mse_path, 'mean_mse': mean_mse_vec}
...
class LassoCV(CVEstimatorBase):
    ...
    def __init__(self, alphas=None, n_alphas: int=12, alpha_min_ratio: float=0.001, cv: int=5, cv_splits=None, fit_intercept: bool=True, max_iter: int=3000, tol: float=0.0001, stopping: str='coef_delta', inference_method: str='cpu_ols_inference', n_bootstrap: int=200, bootstrap_random_state: Optional[int]=None, device: Union[str, Device]=Device.AUTO, n_jobs: Optional[int]=None, compute_inference: bool=True, solver: str='fista', cpu_solver: str='coordinate_descent', method: str='standard', cd_kkt_check_every: Optional[int]=None, lipschitz_L: Optional[float]=None, admm_rho: float=1.0, gpu_memory_cleanup: bool=False, gpu_cv_mixed_precision: bool=True, random_state: Optional[int]=None):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        effective_cpu_solver = 'coordinate_descent' if str(self.method).lower() == 'glmnet' else str(self.cpu_solver)
        details = _select_lasso_alpha_cv(X, y, alphas=self.alphas, n_alphas=self.n_alphas, alpha_min_ratio=self.alpha_min_ratio, cv_folds=self.cv, cv_splits=self.cv_splits, random_state=self.random_state, sample_weight=sample_weight, fit_intercept=self.fit_intercept, device=device_name, max_iter=self.max_iter, tol=self.tol, cpu_solver=effective_cpu_solver, method=self.method, cd_kkt_check_every=self.cd_kkt_check_every, gpu_cv_mixed_precision=self.gpu_cv_mixed_precision, return_details=True)
        ...
        if fast_refit_enabled:
            ...
            estimator._df_resid = int(fast['n_samples']) - (int(fast['n_features']) + (1 if bool(self.fit_intercept) else 0))
            if bool(self.fit_intercept):
                estimator._params = np.concatenate([[estimator.intercept_], estimator.coef_])
            else:
...
def _solve_lasso_path_gpu_fista_batched_from_gram_torch(XtX, Xty, *, n_samples: int, alphas_desc: np.ndarray, max_iter: int, tol: float, stopping: str, lipschitz_L: Optional[float]=None, check_every: int=8):
    ...
    for iteration in range(int(max_iter)):
        ...
        if stopping_name == 'kkt':
            grad_sse = (XtX @ coef_new - Xty.reshape(-1, 1)) / n_samp
            viol = torch.max(torch.maximum(torch.abs(grad_sse) - alpha_gpu[active_gpu].reshape(1, -1), torch.tensor(0.0, dtype=XtX.dtype, device=XtX.device)), dim=0).values
            converged_local_gpu = viol < float(tol)
        else:
...
def _solve_lasso_path_gpu_fista_multi_fold_from_gram_torch(XtX_batch, Xty_batch, *, n_samples_vec: np.ndarray, alphas_desc: np.ndarray, max_iter: int, tol: float, stopping: str, lipschitz_L: Optional[float]=None, check_every: int=8):
    """Solve descending-alpha Lasso paths for all folds together on Torch GPU."""
    import torch
    n_folds = int(XtX_batch.shape[0])
    n_features = int(XtX_batch.shape[1])
...
class Lasso(_PenalizedLinearRegression):
    ...
    def __init__(self, alpha: float=1.0, fit_intercept: bool=True, max_iter: int=1000, tol: float=0.0001, stopping: str='coef_delta', inference_method: str='cpu_ols_inference', n_bootstrap: int=200, bootstrap_random_state: Optional[int]=None, enable_simultaneous_inference: bool=False, simultaneous_method: str='maxz_bootstrap', simultaneous_alpha: float=0.05, simultaneous_n_bootstrap: int=1000, simultaneous_random_state: Optional[int]=None, simultaneous_include_intercept: bool=False, device: Union[str, Device]=Device.AUTO, n_jobs: Optional[int]=None, compute_inference: bool=True, solver: str='fista', cpu_solver: str='coordinate_descent', lipschitz_L: Optional[float]=None, admm_rho: float=1.0, gpu_memory_cleanup: bool=False, **kwargs):
        ...
        super().__init__(penalty='l1', alpha=alpha, fit_intercept=fit_intercept, max_iter=max_iter, tol=tol, device=device, n_jobs=n_jobs, cpu_solver=cpu_solver, solver=solver, lipschitz_L=lipschitz_L, gpu_memory_cleanup=gpu_memory_cleanup)
