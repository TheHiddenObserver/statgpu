...
def _get_irls_step_compiled():
    ...
    def _irls_weighted_gemm(X, W, z):
        """Weighted X'WX and X'Wz 鈥?elementwise ops fused by torch.compile."""
        W_col = W.unsqueeze(1)
        XtWX = X.T @ (X * W_col)
        Xtz = X.T @ (W * z)
        return XtWX, Xtz
...
def irls_solver(
    ...
    for iteration in range(max_iter):
        ...
        # For identity link (squared_error), skip clipping 鈥?mu = eta = X@params
        ...
        # For identity link (squared_error), skip clipping 鈥?mu = eta.
        ...
        def _dev_val(mu_arr):
            ...
            Returns device-side value (no GPU鈫扖PU sync) for torch/cupy.
        ...
        # Current loss 鈥?use only eta clipping (prevent exp overflow),
        ...
        if iteration % 5 == 4 or iteration == max_iter - 1:
            try:
                ...
                # No gradient method available 鈥?fall back to param change
