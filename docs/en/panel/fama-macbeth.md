...
A period is retained when it satisfies `min_obs_per_period` and the implementation's minimum count rule $n_t\ge k+1$, where $k$ is the intercept-augmented design width. The full-rank check is then applied to each retained period. The NumPy reference path keeps the long-standing serial rank-revealing SVD policy. CuPy and Torch use the mathematically identical SVD cutoff in exact-size batches: retained periods having the same $n_t$ are solved together, so a balanced panel uses one batched SVD while an unbalanced panel is bucketed by its exact period sizes. No zero padding is used, so each period keeps the same cutoff $\max(n_t,k)\epsilon s_{\max,t}$ as the serial path. The SVD factors provide both numerical rank and the least-squares coefficient vector, and the GPU path transfers one rank vector per size bucket instead of synchronizing one scalar rank per retained period.
...
At least two valid periods must remain after filtering; otherwise `.fit()` raises an error because the variability of the coefficient series cannot be estimated from fewer than two periods. Every retained period must also have full column rank under the shared panel SVD cutoff; a rank-deficient retained period fails closed before inference. Rank checking and coefficient solving share the same SVD factors. CuPy/Torch batching changes only how equal-shaped period problems are scheduled: it does not pad periods, alter the rank threshold, or change the chronological order of the resulting `betas_`. If several buckets contain deficient periods, the public error still identifies the earliest deficient retained period in chronological order.

The original physical performance fixture is deliberately retained as a **micro/launch-overhead regime**: 64 periods × 128 observations per period × 4 regressors (8,192 rows, with a 5-column design after adding the intercept). Even with batching, very small cross-sectional regressions can remain dominated by fixed GPU launch, synchronization, validation, and reporting costs, so `device="cuda"` or `device="torch"` is **not** a universal speedup guarantee. Performance should be judged across the actual panel scale rather than from one small fixture.

`dev/benchmarks/benchmark_fama_macbeth_scaling_gpu.py` therefore records a separate resident-array scaling matrix while retaining the micro case. The maintained matrix includes micro (64×128×4), medium (128×1,024×8), and large (128×4,096×16) workloads. It reports synchronized NumPy/CuPy/Torch median time, rows per second, GPU/NumPy ratio, speedup, solver mode/batch count, numerical parity, backend versions, and thread-environment provenance. GPU input arrays are transferred before warmup/timing, so this benchmark measures resident-array `fit()` performance and explicitly excludes host-to-device input-transfer time. A ratio below 1 means the GPU fit is faster than the serial NumPy reference on that scale; a ratio above 1 is reported as such rather than being treated as a correctness failure.
...
Maintained internal regressions additionally cover formula intercept behavior, ordered-categorical versus numeric chronology on both array and formula paths, missing-row formula alignment, retained-period rank rejection, failed-refit invalidation, the single-factorization solve contract, backend-native rank-cutoff behavior, both covariance modes on Torch CPU, backend-native distribution routing, exact df=1/df=2 small-degree inference boundaries, balanced batched solving, shuffled unbalanced exact-size bucketing, chronological rank-error reporting, and the packed reporting snapshot.

GPU consistency for the standard full-rank numeric-time `fama_macbeth_newey_west` case is tested by `dev/benchmarks/validate_panel_stage_a_gpu.py`. The focused exact-head gate `dev/benchmarks/validate_fama_macbeth_review_fix_gpu.py` additionally checks ordered-categorical chronology, formula/missing-row alignment, rank rejection, both no-intercept spellings, both covariance modes, the standard inference container/private aliases, requested/executed fit and inference backend identity, actual public-output array backend, and full parity for `betas_`, coefficients, covariance, standard errors, test statistics, p-values, confidence intervals, and predictions. Fresh optimized-source performance/crossover evidence is recorded separately by `dev/benchmarks/benchmark_fama_macbeth_scaling_gpu.py`; no universal GPU speedup is claimed from the focused micro fixture alone. Fama-MacBeth remains outside the Stage-C residual-covariance case matrix because its covariance is defined from the coefficient series rather than observation-level residual scores.
