...
A period is retained when it satisfies `min_obs_per_period` and the implementation's minimum count rule $n_t\ge k+1$, where $k$ is the intercept-augmented design width. The full-rank check is then applied to each retained period. The NumPy reference path keeps the long-standing serial rank-revealing SVD policy. Torch uses its documented stacked-SVD support to solve equal-size retained periods in exact-size batches: a balanced panel uses one batched SVD, while an unbalanced panel is bucketed by exact period size. CuPy's maintained public `cupy.linalg.svd` contract documents a two-dimensional matrix input rather than a stacked batch, so statgpu deliberately keeps supported two-dimensional CuPy SVDs. It groups rows once in chronological order, slices each period contiguously, keeps every rank scalar on device, and transfers one packed rank vector after the period-SVD loop instead of synchronizing once per period. Neither GPU path uses zero padding, so every period keeps the same cutoff $\max(n_t,k)\epsilon s_{\max,t}$ as the serial reference. In all paths the SVD factors provide both numerical rank information and the least-squares coefficient vector.
...
At least two valid periods must remain after filtering; otherwise `.fit()` raises an error because the variability of the coefficient series cannot be estimated from fewer than two periods. Every retained period must also have full column rank under the shared panel SVD cutoff; a rank-deficient retained period fails closed before inference. Rank checking and coefficient solving share the same SVD factors. Backend-specific GPU scheduling changes execution only: Torch exact-size batching does not pad periods or alter the rank threshold, while CuPy keeps the same supported two-dimensional period SVDs and merely defers rank synchronization. Both preserve the chronological order of `betas_`; if several periods are deficient, the public error still identifies the earliest deficient retained period in chronological order.

The original physical performance fixture is deliberately retained as a **micro/launch-overhead regime**: 64 periods × 128 observations per period × 4 regressors (8,192 rows, with a 5-column design after adding the intercept). Even after backend-specific scheduling optimizations, very small cross-sectional regressions can remain dominated by fixed GPU launch, synchronization, validation, and reporting costs, so `device="cuda"` or `device="torch"` is **not** a universal speedup guarantee. Performance should be judged across the actual panel scale rather than from one small fixture.

`dev/benchmarks/benchmark_fama_macbeth_scaling_gpu.py` therefore records a separate resident-array scaling matrix while retaining the micro case. The maintained matrix includes micro (64×128×4), medium (128×1,024×8), and large (128×4,096×16) workloads. It reports synchronized NumPy/CuPy/Torch median time, rows per second, GPU/NumPy ratio, speedup, solver mode/call count, rank-synchronization count, numerical parity, backend versions, and thread-environment provenance. GPU input arrays are transferred before warmup/timing, so this benchmark measures resident-array `fit()` performance and explicitly excludes host-to-device input-transfer time. A ratio below 1 means the GPU fit is faster than the serial NumPy reference on that scale; a ratio above 1 is reported as such rather than being treated as a correctness failure.
...
Maintained internal regressions additionally cover formula intercept behavior, ordered-categorical versus numeric chronology on both array and formula paths, missing-row formula alignment, retained-period rank rejection, failed-refit invalidation, the single-factorization solve contract, backend-native rank-cutoff behavior, both covariance modes on Torch CPU, backend-native distribution routing, exact df=1/df=2 small-degree inference boundaries, Torch balanced batched solving, shuffled unbalanced exact-size bucketing, chronological rank-error reporting, the CuPy stacked-SVD guard, and the packed reporting snapshot.

GPU consistency for the standard full-rank numeric-time `fama_macbeth_newey_west` case is tested by `dev/benchmarks/validate_panel_stage_a_gpu.py`. The historical focused gate `dev/benchmarks/validate_fama_macbeth_review_fix_gpu.py` remains the detailed chronology/formula/rank/inference correctness oracle. Final optimized-source physical acceptance should use `dev/benchmarks/validate_fama_macbeth_optimized_gpu.py`, which executes that focused oracle and additionally verifies backend-specific solver/rank-synchronization provenance before emitting the accepted artifact. Performance/crossover evidence is recorded separately by `dev/benchmarks/benchmark_fama_macbeth_scaling_gpu.py`; no universal GPU speedup is claimed from the micro fixture alone. Fama-MacBeth remains outside the Stage-C residual-covariance case matrix because its covariance is defined from the coefficient series rather than observation-level residual scores.
