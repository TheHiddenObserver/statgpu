...
- **Smooth Quantile FISTA backend parity**: NumPy, CuPy, and Torch now use the same every-iteration best-accepted/objective-stability convergence semantics on the backtracking L2/no-penalty Quantile route. This removes backend-specific stopping at different pinball kinks without changing the deferred-check asynchronous non-smooth CV path.
