...
Physical validation now has an explicit evidence chain rather than a stale single-head claim. Historical Stage-C v5 acceptance passed **47/47** correctness checks on each of CuPy and Torch plus a synchronized **60-row** performance matrix. The later Fama-MacBeth correctness source `464b587e83b234d78b5449666488d7f2f8ad367c` was then revalidated on Tesla P100: the Stage-C matrix passed 35 estimator integrations + 12 covariance primitives per backend, and focused CuPy/Torch checks passed ordered-categorical chronology, formula/missing-row alignment, the lexical-order negative control, retained-period rank rejection, and both explicit no-intercept spellings. Those raw artifacts are retained under `results/pr126_p100_fama_fix/`. A subsequent `.claude/skills/code-review.md` rerun found additional completion-gate gaps and reopened exact-head physical acceptance. `FamaMacBeth` now uses the shared rank-revealing SVD factors directly for each retained-period least-squares solve, keeps the singular-value cutoff on the active backend until only the final integer rank is extracted, and avoids materializing the residual-OLS covariance bread that this estimator does not consume. It also publishes the standard `ParameterInferenceResult`/private inference aliases while keeping public CuPy/Torch result arrays backend-native, invalidates prior fitted/inference state transactionally before every refit, and covers both Newey-West (`z`/normal) and nonrobust (`t`, df=`T-1`) inference modes. The focused physical runner now checks full coefficient/inference/prediction parity, both covariance modes, requested/executed backend identity, rank and formula edge behavior, and same-workload synchronized NumPy/CuPy/Torch timings. It records raw samples, median GPU/NumPy ratios, and explicit `optimization_notes`; a ratio above 1 is reported as the GPU being slower on that fixture, not hidden as a correctness failure, and no universal speedup claim is made. Because these changes modify the numerical/inference source after `464b587e...`, exact-head P100 acceptance remains **reopened** until both the Stage-C physical runner and `dev/benchmarks/validate_fama_macbeth_review_fix_gpu.py` pass on the final clean head.
