...
- `dev/benchmarks/validate_penalized_glm_inference_gpu.py` schema v3 is the maintained exact-source physical CUDA gate for CuPy and Torch. Weighted cases require public `solver="auto"` to resolve to Newton on applicable rows; unweighted explicit-FISTA cases retain independent FISTA fit/inference coverage, including Torch→CuPy / CuPy→Torch container crossings. Hosted checks do not substitute for this physical gate, and no physical GPU pass is claimed until the exact-head runner is executed.
