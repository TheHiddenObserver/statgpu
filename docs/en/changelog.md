...
- **Direct/CV contract reconciliation**: Quantile response validation is backend-preserving and shared by generic/typed direct fits and `PenalizedGLM_CV`; malformed custom folds and zero fold weight mass fail before automatic alpha-grid numerical work; continuation scales and adaptive/group penalty ownership remain aligned with the fitted objective. Prediction and score reject malformed shapes instead of allowing NumPy broadcasting to return plausible but invalid results.
...
- **Standalone inference lifecycle and controls**: bootstrap requires at least two resamples; invalid inference method/kernel/bandwidth/stopping/boolean/quantile controls fail before backend work; Hall-Sheather/Bofinger/Chamberlain bandwidth evaluation fails closed when `q ± h` leaves `(0,1)` or the final bandwidth is non-finite/non-positive. Fit/inference publication is transactional, failed fits clear result-bearing state, and `gpu_memory_cleanup=True` now recognizes the recorded `cupy` backend on success and failure paths.
...
- Physical CUDA evidence committed earlier in PR166 fingerprints earlier source and is therefore historical only after the solver/inference numerical changes above. Current exact-head physical revalidation remains a separate acceptance step; hosted checks do not substitute for it.
