...
- **Smooth Quantile FISTA backend parity**: NumPy, CuPy, and Torch now use CPU-equivalent every-iteration best-accepted/objective-stability semantics on the backtracking L2/no-penalty Quantile route. Accelerator execution batches the accepted trial's objective/coefficient-change and L2-tracking scalars into the Armijo synchronization that already occurs for that trial, avoiding extra convergence or penalty-tracking synchronization. The asynchronous non-smooth CV path keeps deferred checks; its physical stress case uses the maintained public L1 CV stopping tolerance while retaining the separate CPU/GPU objective-parity threshold.
