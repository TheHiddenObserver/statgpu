...
Physical validation is recorded as an evidence chain rather than a single mutable head claim. Historical Stage-C v5 acceptance passed **47/47** correctness checks on each of CuPy and Torch plus a synchronized **60-row** performance matrix. The later Fama-MacBeth correctness source `464b587e83b234d78b5449666488d7f2f8ad367c` was revalidated on Tesla P100, and after the subsequent Fama-MacBeth inference/solver repairs the clean source `0222ff8a338cc319ddf43b3295db63b69f37f40f` again passed the broad Stage-C and focused Fama-MacBeth physical gates. Those artifacts remain valid historical evidence for their exact numerical sources. A still-later independent `.claude/skills/code-review.md` audit found that legacy `PooledOLS(cov_type="hac")` could discard an ordered pandas categorical's declared chronology and sort materialized labels lexically, changing HAC lag pairs and inference. The repair now routes legacy-HAC time ordering through the shared chronology factorizer, adds ordered-categorical/numeric equivalence, lexical negative-control, formula-alignment, and missing-label regressions, and provides a dedicated `dev/benchmarks/validate_panel_hac_chronology_gpu.py` CuPy/Torch gate. Panel fits are also transactionally fail-closed across the maintained estimator family: a failed refit clears both stale prior inference and partially written new outputs. `FamaMacBeth` no longer retains the complete training design merely as a prediction device anchor; it keeps a zero-length backend/device reference instead, while its focused physical runner verifies that anchor, prediction parity, both covariance modes, requested/executed backend identity, and records the actual CuPy package version. Its synchronized timing evidence remains a workload caveat rather than a universal speedup claim: small serial per-period SVDs can be slower on GPU. Because these latest repairs change production numerical/lifecycle behavior after `0222ff8a...`, exact-head physical acceptance is **reopened** until the broad Stage-C runner, `dev/benchmarks/validate_fama_macbeth_review_fix_gpu.py`, and `dev/benchmarks/validate_panel_hac_chronology_gpu.py` all pass on one final clean source.
...
- Added the classical pooling F test for fixed effects, the one-way entity error-components Breusch-Pagan LM test including the Baltagi-Li unbalanced-panel formula, and the classical one-way classical FE-vs-RE Hausman with explicit applicability diagnostics.
