...
- `GammaRegression(link="inverse_power")` now gives explicit Newton/L-BFGS a loss-owned smooth-domain contract instead of a blanket no-intercept rejection. On the positive-weight rows, statgpu constructs a numerically certified direction with strictly positive linear predictor, scales it into the maintained inverse-link training band, and limits accepted steps to that band. Feasible no-intercept designs therefore fit normally; exact zero-weight rows do not constrain feasibility; genuinely infeasible or numerically uncertifiable active designs fail before the first objective evaluation rather than relying on clipping across the inverse-link boundary.
- The same inverse-Gamma domain contract is closed through penalized L2 Newton/L-BFGS and `PenalizedGLM_CV`: framework warm starts are reused only when domain-feasible, validation scoring uses the declared `inverse_power` objective rather than the log-link evaluator, and selected full-data refits preserve the link. Typed `PenalizedGammaRegression` also preserves historical `loss_kwargs["link"]` precedence while remaining sklearn-clone safe.
...
- Exact hosted validation at numerical/docs source `9ef5b34ffc8abf133bc6262e98a855d5efe37d3c` is green across all seven PR workflows. The full CPU suite reports **3370 passed / 826 skipped / 0 failed**; Python 3.9/3.10/3.11/3.12 regression matrices, static/documentation contracts, Torch-2.0 CPU regression, maintenance compatibility, release-note/package validation, Gaussian-inference, node-wise-inference, and Benchmark Frontend gates all passed. Coverage includes feasible and infeasible no-intercept inverse-Gamma designs, negative separator directions, zero-weight inactive rows, weighted Newton/L-BFGS, formula equivalence, typed-wrapper clone compatibility, fixed-penalty M-estimation, CV selected-final-refit inference, failure transactions, finite-difference objective derivatives, and statsmodels alignment.
- The earlier exact-source Tesla P100 schema-v3 run at `c6781cb6a2e1fe500f325e832d23cdc80a99b564` remains valid **historical evidence for that earlier implementation only**. It covered 48 ordinary NumPy/CuPy/Torch Newton/L-BFGS routes, 4 Torch↔CuPy cross-container routes, and 9 penalized/CV L-BFGS consumer routes, with maximum ordinary GPU-vs-NumPy absolute error `4.44e-16`. Its unchanged raw JSON is retained at `dev/reviews/pr151_glm_weighted_explicit_solvers_gpu.json`.
- Exact-source schema-v4 physical CUDA validation **passed** on clean source `9ef5b34ffc8abf133bc6262e98a855d5efe37d3c` using Tesla P100-SXM2-16GB, CuPy 13.6.0, Torch 2.0.0+cu117, NumPy 1.24.2, and Python 3.9.16. It reran the embedded schema-v3 contract unchanged, then passed 6 ordinary no-intercept Newton/L-BFGS NumPy/CuPy/Torch routes (maximum backend error `2.50e-16`; active `eta≈0.7375..1.0803`), 4 Torch↔CuPy cross-container routes, 6 penalized-L2 no-intercept routes, 3 smooth-L2 CV routes with identical `selected_alpha=0.03` and `link="inverse_power"`, plus CuPy/Torch contradictory-domain fail-closed and zero-weight-row-rescue checks. Frozen tolerances stayed at coefficient/intercept `2e-5`, weight-rescale `2e-6`, solver `1e-8`. The original raw schema-v4 JSON is retained unchanged at `dev/reviews/pr151_inverse_gamma_domain_gpu_v4.json`; it records `schema_version=4`, `source_sha=9ef5b34f...`, `source_clean=true`, and `status=success`. Evidence commit `314b2960...` is directly on top of the validated source; later relocation from `results/` is a 0-addition/0-deletion rename of the same blob. Issue #152 is completed.
