...
- The PR166 smooth-FISTA physical validator is now schema v2: it retains the penalized explicit-FISTA/CV matrix and adds CuPy/Torch standalone `tau=0.20` batched-bootstrap direction cases. Physical CUDA evidence committed earlier in PR166 fingerprints earlier source/schema and is historical only; current exact-head schema-v2 revalidation remains a separate acceptance step, and hosted checks do not substitute for it.
