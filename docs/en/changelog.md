...
- The PR166 smooth-FISTA physical validator is now schema v3: it retains the penalized explicit-FISTA/CV matrix and private CuPy/Torch `tau=0.20` batched-bootstrap direction cases, and adds full public standalone `fit(..., compute_inference=True, inference_method="bootstrap")` cases that verify point-fit convergence, inference publication, concrete device provenance, schedule identity, and finite reporting outputs. Physical CUDA evidence committed earlier in PR166 fingerprints earlier source/schema and is historical only; current exact-head schema-v3 revalidation remains a separate acceptance step, and hosted checks do not substitute for it.
