...
- `dev/benchmarks/validate_pr151_final_gpu_v7.py` is the corrected physical CUDA gate: it keeps design/response float64 while overflow weights remain float32, reuses the v6 analytic-weight inference matrix, reruns the accepted v5→v4→v3 chain, and records the v6 disposition. Exact clean source `e1cbf3756d269a6073461de8331da11c6235fb4e` passed schema v7 on Tesla P100-SXM2-16GB with `status: success` and `source_clean: true`; the retained artifact is `dev/reviews/pr151_final_gpu_v7.json`. A subsequent fresh review changed ordinary explicit-smooth refit transaction behavior, so that artifact is now immutable **historical exact-source evidence**, not acceptance of the final reviewed source. PR #151 remains **not merge-ready** until the latest review is clean, all hosted workflows are green on the final head, and schema v7 passes again on the final exact clean production source.
