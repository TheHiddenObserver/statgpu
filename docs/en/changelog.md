...
- Current inverse-Gamma domain/no-intercept behavior was implemented after the schema-v3 run. `dev/benchmarks/validate_pr151_inverse_gamma_domain_gpu_v4.py` is the frozen schema-v4 exact-source physical CUDA gate for the current capability. A fresh clean-source CuPy/Torch physical run is therefore still required before current-head GPU acceptance or merge-readiness can be claimed; hosted CI and the historical v3 artifact do not substitute for that gate.
