...
- The PR166 smooth-FISTA physical validator is now schema v4: it retains the penalized explicit-FISTA matrix and private/public CuPy/Torch standalone bootstrap cases, and adds non-uniform weighted L1 strict-CV parity against the CPU reference. That case directly exercises the shared weighted-FISTA spectral Gram step scale on the GPU async CV path. Physical CUDA evidence committed earlier in PR166 fingerprints earlier source/schema and is historical only; current exact-head schema-v3 revalidation remains a separate acceptance step, and hosted checks do not substitute for it.
