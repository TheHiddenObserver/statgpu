# Regression and statistical models

> Switch: [简体中文](../../cn/models/)

Open a model page directly from this catalog. For a package-level overview, see
the [full model inventory](README.md).

## Regression and generalized linear models

- [Linear regression](linear-regression.md)
- [Generalized linear models](generalized-linear-model.md)
- [Logistic regression](logistic-regression.md)
- [Poisson regression](poisson-regression.md)
- [Ordered response models](ordered.md)
- [Quantile regression](quantile.md)
- [Robust regression](robust.md)

## Regularization and feature selection

- [Feature selection](feature-selection.md)
- [Ridge regression](ridge.md)
- [Lasso](lasso.md)
- [Elastic net](elastic-net.md)
- [Adaptive lasso](adaptive-lasso.md)
- [SCAD](scad.md)
- [MCP](mcp.md)
- [Knockoff filters](knockoff.md)

## Inference and statistical utilities

- [Analysis of variance](anova.md)
- [Covariance estimators](covariance.md)
- [Loss functions](losses.md)
- [Multiple testing](multiple-testing.md)

## Specialized model families

- [Cox proportional hazards](coxph.md)
- [Nonparametric methods](nonparametric.md)
- [Semiparametric methods](semiparametric.md)
- [Splines](splines.md)
- [Kernel methods](kernel-methods.md)
- [Panel model overview](panel.md)
- [Unsupervised learning overview](unsupervised.md)

## Detailed catalogs

- [Panel estimators and diagnostics](../panel/)
- [Unsupervised learning algorithms](../unsupervised/)
