...
Formula input follows Patsy/R intercept semantics: without fixed effects, the default formula intercept is retained, while `0 +` / `-1` requests a no-intercept level regression; with entity/time fixed effects, the common formula intercept is absorbed by the fixed-effect space. Formula input can request fixed effects either with linearmodels-style additive tokens (`EntityEffects`, `TimeEffects`, `FixedEffects`) or through the existing pipe syntax, for example `"y ~ x1 + x2 | entity"`; these two fixed-effect syntaxes are alternatives and cannot be combined in one formula. Magic tokens are recognized only as complete top-level identifiers, so regressor names such as `EntityEffectsScore` remain ordinary Patsy variables. Formulas requesting more than two fixed-effect variables fail closed rather than silently dropping additional effects, and effects-only fixed-effect formulas without a non-intercept regressor are explicitly unsupported. Formula row filtering aligns side arrays to the retained estimation sample, and formula-enabled effects apply only to that fit rather than leaking into a later refit of the same estimator. For two-way fixed effects, `demean_max_iter` and `demean_tol` control the fail-closed backend-native projection solver; convergence is checked against residual entity and time means rather than only adjacent-iterate changes. Group codes are factorized once and reused across iterations. Fitted one-way/two-way level predictions restore the common grand-mean component whenever a stored fixed effect is used, so known-label fitted predictions match the corresponding fixed-effect least-squares projection. Fitted two-way effects are recovered jointly on unbalanced panels. For any fitted two-way fixed-effect model, a prediction that uses stored fixed effects is identified only when both entity and time labels are known; the pair must also belong to the same incidence component. One-sided labels, known-plus-unknown pairs, and cross-component pairs fail closed rather than exposing arbitrary fixed-effect normalizations. If both fixed-effect labels are unseen, the historical zero-effect fallback remains the linear-only prediction.
