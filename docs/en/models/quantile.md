...
- Group Proximal IRLS-LLA treats stopping controls as strict inputs rather than silently clamping them: `max_iter` must be a positive integer and `tol` a finite positive number. Direct estimators additionally require positive-integer `max_lla_iters` and finite-positive `lla_tol`; `PenalizedGLM_CV` exposes `max_iter`/`tol`; candidate fits and the final refit use the route's built-in LLA defaults.
- On direct `PenalizedGeneralizedLinearModel` fits, `lla=False` opts out of the automatic Group Proximal IRLS-LLA specialization for Quantile Group SCAD/MCP. The auto-selected ordinary proximal FISTA route is then used instead. `PenalizedGLM_CV` does not expose an `lla` switch.
