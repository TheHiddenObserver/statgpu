...
- Strict Quantile cross-validation requires complete converged fold evidence for every penalty family: if a fold fit reports solver non-convergence or otherwise has no finite score, that alpha is ineligible and the whole candidate column is treated as missing rather than averaging the remaining folds. Two-stage screening remains intentionally relaxed; this complete-evidence rule applies to strict refinement/selection. The selected full-data refit follows the direct-estimator convergence-reporting contract.
