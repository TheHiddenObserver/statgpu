...
- Strict Quantile cross-validation requires complete finite fold evidence for every penalty family: if a fold fit fails and produces no finite score, that alpha is ineligible and the whole candidate column is treated as missing rather than averaging the remaining folds. Automatic scalar/Group non-convex routes additionally promote their explicit target-level convergence failure signal to an unscoreable fold. A generic solver `ConvergenceWarning` alone does not erase an otherwise finite candidate result. Two-stage screening remains intentionally relaxed; the complete-evidence rule applies to strict refinement/selection. The selected full-data refit follows the direct-estimator convergence-reporting contract.
