...
- The public low-level `QuantileLoss.irls()` and `proximal_irls_quantile_solver()` boundaries likewise reject non-boolean `fit_intercept` values and invalid stopping controls instead of relying on Python truth-value or numeric-string coercion. For direct `proximal_irls_quantile_solver()` calls, `alpha_path` must also be a non-empty one-dimensional sequence of finite positive values in non-increasing order from the continuation start to the target.
