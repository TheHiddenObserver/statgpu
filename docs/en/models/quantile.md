...
- The public low-level `QuantileLoss.irls()`, `proximal_irls_quantile_solver()`, and Quantile `fista_lla_path()` boundaries fail closed on malformed supervised shapes: `X` must be two-dimensional, `y` one-dimensional, and their row counts must agree. They also reject non-boolean intercept controls and invalid stopping/path/weight inputs instead of relying on broadcasting or implicit coercion. For direct continuation calls, `alpha_path` must be a non-empty one-dimensional sequence of finite positive values in non-increasing order from the continuation start to the target.
