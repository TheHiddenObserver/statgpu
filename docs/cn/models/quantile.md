...
- strict Quantile 交叉验证对所有惩罚家族都要求完整且已收敛的折级证据：如果某一折的求解器报告未收敛，或该折没有有限得分，则该 α 整列都视为无效，不能只对剩余有限折求均值。two-stage 的第一阶段筛选仍有意保持 relaxed；完整证据规则在 strict refinement / strict selection 中执行。选中 α 后的全数据最终重拟合沿用直接估计器的收敛报告语义。
