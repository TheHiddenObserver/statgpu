...
- FISTA-BB 与公开的直接 ADMM 不支持 Quantile；显式请求会在进入数值迭代前报错。 历史 `quantile_cd_solver` 名称仅作为 import 兼容的 fail-closed 符号保留：旧实现会忽略 `sample_weight`，且无法可靠表示不受惩罚的截距，因此维护中的标量 SCAD/MCP 拟合改用 Proximal IRLS-CD。
