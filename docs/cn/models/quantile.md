...
- 公开底层 `QuantileLoss.irls()`、`proximal_irls_quantile_solver()` 与 Quantile `fista_lla_path()` 会对监督输入形状 fail closed：`X` 必须为二维、`y` 必须为一维，且二者行数一致；同时拒绝非布尔截距控制及非法的停止、路径和权重输入，不依赖 broadcasting 或隐式类型转换。直接 continuation 调用的 `alpha_path` 还必须是一维非空、元素均为有限正数、并从起点到目标值保持非递增的序列。
