...
| FISTA | ✅ | 用于 L1/ElasticNet 等近端路径；L2/无惩罚下显式 `solver="fista"` 也会真实执行普通 FISTA，而 `auto` 仍优先 IRLS |
| FISTA-BB | ❌ | BB 步长通过 smooth-gradient difference 估计局部曲率；Quantile 的次梯度是阶梯函数，因此 estimator/CV 的显式请求与公开底层 `fista_bb_solver(QuantileLoss, ...)` 都会直接报错 |
| L-BFGS | ✅（底层无权重/均匀权重边界） | 公开 `PenalizedQuantileRegression` / `PenalizedGLM_CV` 的显式 L-BFGS 请求不受支持；底层 `lbfgs_solver(QuantileLoss, ...)` 保留历史无权重/均匀权重兼容面，真正非均匀权重仍会被拒绝 |
| ADMM | ❌ | 共享 ADMM 的 w-update 使用 accelerated gradient descent，因此要求光滑损失梯度。Quantile 的梯度是阶梯函数；estimator/CV 的显式请求与公开底层 `admm_solver(QuantileLoss, ...)` 都会在数值迭代前报错 |
| Newton | ❌ | 分位数损失没有 Hessian |
| Proximal Newton | ❌ | 分位数损失没有 Hessian |

对 L2/无惩罚 Quantile 目标，`PenalizedQuantileRegression(..., solver="auto")` 会解析到 IRLS，显式 `solver="irls"` 直接请求同一算法。显式普通 `solver="fista"` 也受支持：它会真正进入通用 FISTA engine，而不会再静默替换成 IRLS。IRLS 仍是自动/默认选择，因为 pinball loss 本身非光滑；这里的 Quantile FISTA 是一阶近端/次梯度路径，并不声称满足 textbook smooth-FISTA 的经典收敛假设。FISTA-BB 仍不支持，因为它的 BB 曲率更新明确依赖有意义的光滑梯度差分。
...
- Quantile IRLS / Proximal IRLS-CD 具有显式带权实现；
- 普通 FISTA（包括显式选择的 L2/无惩罚 FISTA）使用损失层的归一化带权目标；
- 通用 `LossBase` 的共享函数值和梯度可以计算归一化带权目标；
- FISTA-BB 与 ADMM 在任何权重设置下都不支持 Quantile，因为这些通用算法依赖 check loss 并不具备的 smooth-gradient 结构；
- 底层 Quantile L-BFGS 保留未传/均匀权重兼容面，但真正非均匀权重会报错；模型/CV 层显式 L-BFGS 仍不支持。
...
对 L2/无惩罚，`auto` 与显式 IRLS 使用同一 IRLS 路径。显式普通 FISTA 是算法控制选项，并且会真实执行 FISTA，而不是 IRLS 的别名或 fallback。ElasticNet 等非光滑惩罚本来就使用普通 FISTA。
...
Quantile/check loss 本身非光滑，因此这里不应解释为满足经典 smooth-gradient FISTA 的全部理论假设。statgpu 使用注册的 Quantile 次梯度、既有的一阶步长/Lipschitz 策略以及所请求惩罚的近端算子。该路径用于显式算法控制以及凸稀疏 Quantile 路径；L2/无惩罚的自动策略仍然是 IRLS。
...
- 显式普通 L2/无惩罚 Quantile FISTA 已受支持并保持权威：它会真正执行 FISTA，而不是静默替换成 IRLS。Quantile FISTA-BB/ADMM 不支持并会在数值迭代前报错。模型/CV 层 L-BFGS 继续不支持，但底层无权重/均匀权重 L-BFGS 的既有兼容边界保留。
- 不支持的显式带权求解器组合会在数值迭代前报错，而不是静默替换成其他 solver。
- 支持的 GPU 路径（`cuda`/`torch`）不会静默回退到 CPU。
