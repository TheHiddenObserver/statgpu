...
- strict Quantile 交叉验证对所有惩罚家族都要求完整且有限的折级证据：如果某一折拟合失败、没有产生有限得分，则该 α 整列都视为无效，不能只对剩余有限折求均值。自动标量/Group 非凸路径还会把其明确的 target-level 收敛失败信号升级为不可评分的折；但通用求解器单独发出 `ConvergenceWarning` 并不会自动抹去一个已经得到的有限候选结果。two-stage 的第一阶段筛选仍有意保持 relaxed；完整证据规则在 strict refinement / strict selection 中执行。选中 α 后的全数据最终重拟合沿用直接估计器的收敛报告语义。
