...
- 对自动标量 SCAD/MCP 和自动 Group SCAD/MCP 交叉验证，如果某一折在目标 alpha 上未建立收敛，则该折不会计分；strict selection 只接受所有折都有有限得分的 alpha。选中 alpha 后的全数据最终重拟合沿用直接估计器的报告语义：目标 IRLS/LLA 预算耗尽会发出 `ConvergenceWarning` 并返回最终迭代结果。
