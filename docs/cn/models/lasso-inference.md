...
因此得分（score）并不像未惩罚 OLS 那样等于 0。由惩罚引入的**收缩偏差（shrinkage bias）**，才是不能直接拿原始 Lasso 系数配上 OLS 风格标准误、并期待得到以 0 为中心的近似正态统计量的核心原因。在高维情形中，正则化尺度通常不会小到使这一偏差在 $n^{-1/2}$ 的推断尺度上自动消失。此外，Lasso 这类稀疏估计量具有非光滑性，其极限分布依赖未知参数，并不具有固定模型 OLS 那种简单、统一的渐近分布（van de Geer et al., 2014；Javanmard & Montanari, 2014）。
...
所以，对 `debiased` 路径来说，主要动机是**消除主要的正则化偏差，并重新构造一个近似正态的逐系数推断对象**；而“选择不确定性”的警告主要解释为什么活跃集 OLS 路径只能作较弱的选择后推断主张。两者都属于高维推断的难点，但并不是同一个问题。
...
|---|---|---|---|
| `debiased` | 去偏/去稀疏化系数、标准误、z 统计量、p 值和边际置信区间 | 在去偏 Lasso 的理论条件下进行逐系数高维推断 | 有效性依赖稀疏性、设计矩阵、噪声以及正则化/去偏构造 |
...
`post_selection_ols` 是与硬件无关的规范名称。统一接口中的旧别名 `cpu_ols` 与 `gpu_ols` 已同时进入弃用期；在一个兼容周期内仍可使用，但会发出 `FutureWarning`，并统一映射到 `post_selection_ols`。`LassoCV` 在兼容边界上还识别更早的 `cpu_ols_inference` / `gpu_ols_inference` 写法，同样映射到这一统计方法。
...
用于预测的惩罚系数仍保存在 `coef_`。推断报告使用去偏后的参数向量，因此惩罚系数与对应的去偏推断估计不必在数值上相同。
...
校正系数，其中 $M$ 是由数据构造的**去相关矩阵（decorrelation matrix）**，用于近似特征 Gram 矩阵/协方差矩阵的逆。这正是 Zhang & Zhang (2014)、van de Geer et al. (2014) 与 Javanmard & Montanari (2014) 等工作中去偏/去稀疏化 Lasso（de-biased / de-sparsified Lasso）的核心构造。
...
第一项是由噪声驱动、可近似正态的主导项；第二项则是剩余的正则化误差与干扰参数估计误差。如果 $M\widehat\Sigma$ 足够接近单位阵，并且稀疏性、设计矩阵等条件使余项足够小，那么 Lasso 的主要收缩偏差就会在推断尺度上被抵消。这也是为什么去偏估计量能够获得渐近正态的逐系数推断，而原始 Lasso 估计量一般不能直接这样处理。

这并不意味着原来的稀疏 Lasso 估计“变成了 OLS”。`coef_` 仍然是预测所使用的惩罚模型系数；去偏向量则是用于逐系数不确定性报告的推断参数。

## statgpu 如何构造去相关矩阵？

这里采用的是去稀疏化 Lasso 中通过**逐节点回归（node-wise regression）**构造近似精度矩阵（precision matrix）的思路；可参见 van de Geer et al. (2014)，以及 Zhang & Zhang (2014) 的相关低维投影（low-dimensional projection）/偏差校正构造。

对每个特征 $j$，statgpu 用逐节点 Lasso（node-wise Lasso）把 $x_j$ 对其余列 $X_{-j}$ 做稀疏回归：
...
这与“第 $j$ 个位置放 $1/C_j$、其他位置放 $-\hat\gamma_{j,k}/C_j$”完全是同一个实现规则，但矩阵表达更能直接看出统计含义：每一行都是一个经过归一化的**残差化方向**，目标是让 $M\widehat\Sigma$ 尽可能接近单位阵。van de Geer et al. (2014) 的常见记号也会把它写成由逐节点 Lasso 得到的、按行归一化的近似逆矩阵/精度矩阵；不同论文对归一化常数的记号会随目标函数尺度略有不同。

### CPU 与 GPU 的统计构造相同

上面的公式**不是 CPU 专属算法**。当前维护的 NumPy、CuPy 和 Torch 去偏路径使用相同的逐节点惩罚强度、相同的 $\hat\gamma_j$、$C_j$ 与 $M$ 定义，以及相同的后续去偏与方差公式。

差别主要在计算实现：

- CPU 路径逐个特征求解逐节点 Lasso；
- CuPy 与 Torch 路径在具体 GPU 设备上构造相应的 Gram 子问题，并把多个逐节点 FISTA 求解批量执行；
- 批处理改变的是执行方式与内存流量，不改变去相关矩阵的统计定义。

逐节点优化的数值收敛是必要条件，但不能替代去偏推断所要求的稀疏性、设计矩阵和噪声条件。
...
- 一组 95% 边际区间并不会自动对整个系数向量提供 95% 的同时覆盖；
- `simultaneous_alpha` 不改变这些边际区间，它只控制下面单独的同时推断程序。

拟合截距时，预测与推断有意采用不同的**参数归属**。公开 `coef_` / `intercept_` 仍属于惩罚预测拟合；推断斜率使用去偏后的向量，而原始坐标系中的推断截距为
...
它的标准误和影响函数表示因而与同一个中心化去偏参数化保持一致，而不是把截距当成另一个逐节点 Lasso 特征坐标。

## 从边际区间到同时覆盖

95% 的边际置信区间控制的是**单个系数**的覆盖误差。如果一次报告很多坐标，那么“至少有一个区间没有覆盖真值”的概率可能远高于 5%。例如在独立的理想情形下，$m$ 个各自 95% 的区间，其联合覆盖率是 $(0.95)^m$，而不是 0.95。Bonferroni 校正可以简单地恢复族错误率控制，但通常偏保守，因为它没有利用去偏系数统计量之间的相关结构。

高维同时推断的思路则是直接校准所关心目标参数集合上**最大标准化误差**的分布。Zhang & Cheng (2017) 以及 Dezeure、Bühlmann & Zhang (2017) 都研究了基于去稀疏化 Lasso 的自助法同时推断；这种最大统计量构造能够把不同系数之间的依赖关系纳入共同临界值，而不是把每个系数当成彼此独立的检验。
...
该方法抽取独立标准正态乘子 $\xi_i$，利用拟合残差构造自助得分扰动，并记录所请求目标参数族上最大的标准化绝对扰动。示意地，
...
因为这个临界值是针对整个目标参数族的最大值来校准的，所以这些区间的目标是提供全族同时覆盖，通常会比对应的边际区间更宽。和边际去偏推断一样，理论保证仍然依赖相应的高维统计假设；自助法并不会让这些假设自动消失。

### 同时推断控制参数

| 参数 | 默认值 | 含义 |
|---|---:|---|
| `enable_simultaneous_inference` | `False` | 成功完成去偏推断后，再执行同时覆盖校准。 |
...
| `simultaneous_include_intercept` | `False` | 是否把中心化去偏截距同时纳入 max-|Z| 目标参数族和最终联合区间。 |

`enable_simultaneous_inference=True` 要求同时满足 `compute_inference=True`、`inference_method="debiased"` 和 `simultaneous_method="maxz_bootstrap"`。`simultaneous_alpha` 必须严格位于 `(0, 1)`，`simultaneous_n_bootstrap` 必须为正；不支持的组合会明确失败，而不是静默返回普通边际区间。

### 包含截距的同时推断参数族

当 `simultaneous_include_intercept=True` 时，边际标准误所使用的同一个、基于中心化逐节点构造的原始坐标系截距影响量会直接进入自助最大统计量。也就是说，截距不再只是“额外加一行、套用仅由特征校准出的临界值”，而是真正属于被校准的联合目标参数族。

默认值仍为 `False`，因此只需要对特征系数向量进行同时覆盖的调用不会无故扩大目标参数族。
...
- `_conf_int_simultaneous` 使用共同的 max-|Z| 临界值保护一个置信区间族；
...
对于 `debiased`，当前维护的 CuPy/Torch 边际推断路径同样把数值系数推断留在实际执行的 GPU 后端。最终的结果报告层仍以 NumPy 为主：`_bse`、`_pvalues`、`_conf_int` 等数组以及结构化结果元数据，只有在数值推断全部完成后才会复制为主机端结果对象。

对于中心化且 `fit_intercept=True` 的 CuPy/Torch 同时推断，计算量较大的 $B\times n$ 乘子抽样、特征/截距得分、max-|Z| 最大值归约、分位数校准和联合置信区间计算，都会留在同一个具体 GPU 设备上，直到最后的结果报告快照。历史的 `fit_intercept=False` 同时推断路径仍使用已有的通用结果报告阶段辅助函数；PR #138 并没有把这条历史路径声明为 GPU 原生实现。

残差自助法 `bootstrap` 不同：当前残差重拟合实现是 CPU 原生路径，因此估计器使用 GPU `device` 并不意味着自助重采样和重拟合也会在 GPU 上执行。

## 样本权重与推断范围

`Lasso.fit(..., sample_weight=...)` 支持样本权重。当前维护的 NumPy/CuPy/Torch `debiased` 路径使用相同的**加权中心化平均损失问题**，因此把所有分析权重同时乘以同一个正数，不会改变惩罚拟合以及当前维护的去偏推断结果。
...
当前实现用自助样本之间的变异计算标准误，用符号变化概率构造双侧 p 值，并使用百分位数置信区间。它比一次去偏拟合昂贵得多，也没有被声明为完整的选择性推断程序。
...
| 属性 | 含义 |
|---|---|
| `_params` | 推断报告所使用的参数向量；`debiased` 时特征项是去偏估计；`post_selection_ols` 时活跃坐标属于未惩罚重拟合 |
| `_bse` | 标准误 |
| `_tvalues` | 某些路径沿用的历史统计量字段；去偏推断时具有 z 统计量语义 |
...
去偏推断通常显著比原始 Lasso 拟合昂贵，因为它需要求解许多逐节点稀疏回归。后端加速会改变计算成本，但不会改变统计假设。

## 当前实现没有声称什么？

- 普通选择后 OLS/WLS 区间不是一般的选择性推断区间；
- 残差自助法不是对模型选择不确定性的普适修正；
- 边际去偏区间未经同时校准时不提供联合覆盖；
- max-|Z| 同时置信区间不能替代以错误发现率（FDR）为目标的 Benjamini-Hochberg 等程序；
- 数值收敛、GPU 执行以及很小的 KKT 残差不能证明数据满足去偏推断所需的高维统计假设；
- 历史 `fit_intercept=False` 同时推断路径不会因为惩罚/去偏拟合使用了 GPU，就自动获得“GPU 原生同时推断”的统计或实现声明。
