# 回归与统计模型

> 切换：[English](../../en/models/)

可以从本目录直接进入每个模型页面。包级概览请参阅[完整模型目录](README.md)。

## 回归与广义线性模型

- [线性回归](linear-regression.md)
- [广义线性模型](generalized-linear-model.md)
- [逻辑回归](logistic-regression.md)
- [泊松回归](poisson-regression.md)
- [有序响应模型](ordered.md)
- [分位数回归](quantile.md)
- [稳健回归](robust.md)

## 正则化与特征选择

- [特征选择](feature-selection.md)
- [岭回归](ridge.md)
- [Lasso](lasso.md)
- [Elastic Net](elastic-net.md)
- [Adaptive Lasso](adaptive-lasso.md)
- [SCAD](scad.md)
- [MCP](mcp.md)
- [Knockoff 过滤器](knockoff.md)

## 推断与统计工具

- [方差分析](anova.md)
- [协方差估计](covariance.md)
- [损失函数](losses.md)
- [多重检验](multiple-testing.md)

## 专门模型族

- [Cox 比例风险模型](coxph.md)
- [非参数方法](nonparametric.md)
- [半参数方法](semiparametric.md)
- [样条方法](splines.md)
- [核方法](kernel-methods.md)
- [面板模型概览](panel.md)
- [无监督学习概览](unsupervised.md)

## 详细目录

- [面板估计与诊断](../panel/)
- [无监督学习算法](../unsupervised/)
