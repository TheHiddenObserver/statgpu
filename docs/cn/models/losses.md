...
- [稳健回归](robust.md) — Huber、Bisquare、Fair 损失与稳健估计
...
与绝对损失只差一个常数比例，对应中位数回归。它的次梯度是阶梯函数；在当前通用实现中，这会排除 Quantile 的 FISTA-BB 与共享 ADMM 路径。底层无权重/均匀权重 L-BFGS 则作为既有兼容面单独保留，不能与模型层的 solver 支持混为一谈。
...
均匀权重继续保持历史无权重 L-BFGS 行为，包括底层 Quantile 的兼容面。这个底层兼容面并不意味着 `PenalizedQuantileRegression` 或 `PenalizedGLM_CV` 支持 `solver="lbfgs"`。
...
这些例子只说明无权重 L-BFGS 的底层直接调用，不能据此推断 Quantile 或 Huber 已支持非均匀带权 L-BFGS；其中 Quantile 的直接调用也不意味着模型层支持 `solver="lbfgs"`。需要加权稳健回归或分位数回归时，请以对应模型文档为准。
