...
`statgpu.panel` 提供六类面板数据估计量。它们不应理解成六个彼此无关的数据生成过程：多种估计量可以从同一个基础面板模型出发，只是对未观测异质性的处理方式、用于识别系数的变异来源或目标参数有所不同。
...
- [面板模型架构](../panel/architecture.md) — 当前 `BasePanelModel`、各估计量的数据变换与回归问题、共享数值线性代数、协方差/推断、诊断以及拟合生命周期之间的职责边界。
...
每个模型页面都会把**统计模型与识别假设**和**数值估计方法**分开说明。前者回答“在什么条件下，所报告的系数具有通常的面板经济计量解释”；软件本身仍然可以机械地计算某个估计量，因此这些统计假设需要由用户结合实际问题判断，而不是由 `.fit()` 自动验证。

这六类模型都可以通过 `device` 使用 NumPy CPU、CuPy CUDA 或 Torch CUDA。每个模型页面都给出了 CPU/GPU 和公式接口示例。若显式指定 `device="cuda"` 或 `device="torch"`，但对应计算后端不可用，statgpu 会直接报错，而不是自动切换到 CPU。

若需要理解不同面板估计量之间共享了哪些实现、哪些统计步骤必须保留在具体模型实现中，以及当前面板模型为什么不属于通用的 `LossBase + Penalty + Solver` 拟合路径，请直接阅读 [面板模型架构](../panel/architecture.md)。