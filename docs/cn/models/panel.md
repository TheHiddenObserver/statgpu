...
formula 输入遵循 Patsy/R 的 intercept 语义：没有 fixed effect 时保留默认 formula intercept，`0 +` / `-1` 表示 no-intercept level regression；存在 entity/time fixed effect 时，公共 formula intercept 由 fixed-effect space 吸收。formula 输入可以使用 linearmodels-style additive tokens（`EntityEffects`、`TimeEffects`、`FixedEffects`）或已有 pipe syntax（例如 `"y ~ x1 + x2 | entity"`）请求 fixed effects；这两种 fixed-effect 语法互为替代方案，不能在同一 formula 中混用。magic token 只在顶层完整 identifier 位置识别，因此 `EntityEffectsScore` 等 regressor 名仍按普通 Patsy 变量处理。若请求超过两个 fixed-effect 变量会 fail closed，而不会静默丢弃后续 effect；仅包含 fixed effect、没有 non-intercept regressor 的 effects-only formula 也会明确报不支持。formula 的 missing-row filtering 会同步对齐 observation-level side arrays，并且 formula 临时启用的 effect 只作用于该次 fit，不会泄漏到同一 estimator 后续的 refit。对于 two-way fixed effects，`demean_max_iter` 与 `demean_tol` 控制 fail-closed、backend-native 的 projection solver；收敛判据直接检查残余 entity/time group mean，而不是只比较相邻 iterate。group code 只在迭代前 factorize 一次并反复复用。one-way/two-way 的 level prediction 在实际使用已拟合 fixed effect 时会恢复公共 grand-mean 分量，因此已知 label 的 fitted prediction 与对应 fixed-effect least-squares projection 一致。unbalanced panel 的 two-way fixed effects 会联合恢复；对于任何已拟合的 two-way fixed-effect model，只要 prediction 使用 stored fixed effects，就要求 entity/time 两侧 label 都已知，并且二者属于同一 incidence component。单边 label、“已知一侧 + 未知另一侧”以及跨 component 组合都会 fail closed，避免暴露任意 fixed-effect normalization。若两侧 fixed-effect label 都未见过，则仍保留历史 zero-effect fallback，仅返回 linear prediction。
