...
- **overall $R^2$（总体 $R^2$）**：模型对实际响应水平差异的解释程度；
- **between $R^2$（个体间 $R^2$）**：模型对不同个体平均水平差异的解释程度；
- **within $R^2$（个体内 $R^2$）**：模型对同一个体围绕自身平均水平变化的解释程度。

这些名称在面板数据文献中通常直接使用英文术语，但它们对应的数据变异来源不同，不能简单互换。

## $R^2$ 定义

对拟合系数 $\widehat\beta$，总体 $R^2$ 写为
...
对 `PanelOLS`，固定效应同样占用自由度。统一统计量使用
...
为保持既有 API 行为，历史公开字段与 `fit_statistics_` 中的统一统计量可能承担不同职责。解释具体字段时，应以对应模型页为准，而不要假设所有面板估计器共享完全相同的 $R^2$ 定义。

## 不同模型的可用性

提供个体信息后，`fit_statistics_` 可以在适用模型上给出个体内、个体间和总体 $R^2$。

对于普通残差 OLS 型估计器，还可以给出调整 $R^2$ 和经典模型 F 统计量。

`FamaMacBeth` 的结构不同：它先在每个时期进行横截面回归，再对时期系数取平均。因此它可以提供基于参数预测的个体内、个体间和总体 $R^2$，但不报告单个残差 OLS 回归意义下的调整 $R^2$ 或模型 F 统计量。
