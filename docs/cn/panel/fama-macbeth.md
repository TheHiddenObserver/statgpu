...
时期只有在满足 `min_obs_per_period` 且通过实现中的最小样本量规则 $n_t\ge k+1$ 时才会保留，其中 $k$ 是含 intercept 的 design width。随后每个 retained period 还必须通过 full-rank 检查。NumPy reference path 保留原有的 serial rank-revealing SVD policy。Torch 使用其有文档保证的 stacked-SVD 支持，对相同 $n_t$ 的 retained periods 做 exact-size batching：balanced panel 可以用一次 batched SVD 完成所有时期，unbalanced panel 则按真实 period size 分桶。CuPy 当前维护中的公开 `cupy.linalg.svd` contract 只明确给出二维矩阵输入，因此 statgpu 不依赖未文档化的 stacked-SVD 行为，而继续使用受支持的二维 CuPy SVD：先一次性按 chronology 将 rows 连续化，每个 period 直接使用 contiguous slice，所有 rank scalar 保持在 device 上，最后一次性回传 packed rank vector，而不是每个 period 同步一次 rank。两条 GPU 路径都**不使用 zero padding**，因此每个时期仍保留与 serial reference 相同的 $\max(n_t,k)\epsilon s_{\max,t}$ cutoff；SVD factors 同时提供 rank 信息和 least-squares coefficient。
...
过滤后至少需要两个有效 period，否则 `.fit()` 会报错，因为少于两个 period 无法估计 coefficient series 的波动。每个 retained period 还必须在共享 panel SVD cutoff 下 full column rank；若某个 retained period rank deficient，会在 inference 之前 fail closed。rank check 与 coefficient solve 共享同一组 SVD factors。GPU scheduling 只改变执行方式：Torch 的 exact-size batching 不会 padding period 或改变 rank threshold；CuPy 则保留相同的二维 period SVD，只延后 rank synchronization。两者都保持 `betas_` 的 chronology；若多个时期 rank deficient，公开错误仍定位 chronology 上最早的 deficient retained period。

原来的 physical performance fixture 被**刻意保留为 micro/launch-overhead regime**：64 periods × 每期 128 observations × 4 regressors，总计 8,192 rows；加入 intercept 后每期 design width 为 5。即使进行了 backend-specific scheduling 优化，如此小的 cross-sectional regression 仍可能受到 GPU launch、synchronization、validation 和 reporting 等固定成本支配，所以 `device="cuda"` 或 `device="torch"` **不代表普遍更快**。性能判断应覆盖实际 panel scale，而不能只看一个小 fixture。

因此新增 `dev/benchmarks/benchmark_fama_macbeth_scaling_gpu.py`，在保留 micro case 的同时记录 resident-array scaling matrix：micro（64×128×4）、medium（128×1,024×8）和 large（128×4,096×16）。runner 会记录同步后的 NumPy/CuPy/Torch median time、rows/sec、GPU/NumPy ratio、speedup、solver mode/call count、rank synchronization count、numerical parity、backend version 以及 thread-environment provenance。GPU input array 在 warmup/timing 之前已经转移到 device，因此这里测量的是 resident-array `fit()` 性能，并明确**不包含** host-to-device input-transfer time。ratio 小于 1 表示该规模上 GPU fit 快于 serial NumPy reference；ratio 大于 1 会如实记录，而不是被当成 correctness failure。
...
内部 maintained regression 还覆盖 formula intercept、array/formula 两条路径的 ordered-categorical 与 numeric chronology、formula missing-row alignment、retained-period rank rejection、failed-refit invalidation、single-factorization solve contract、backend-native rank-cutoff behavior、Torch CPU 上的两种 covariance mode、backend-native distribution routing、df=1/df=2 的精确小自由度 inference boundary、Torch balanced batched solve、shuffled unbalanced exact-size bucketing、chronological rank-error reporting、CuPy stacked-SVD guard，以及 packed reporting snapshot。

标准 full-rank、numeric-time 的 `fama_macbeth_newey_west` case 仍由 `dev/benchmarks/validate_panel_stage_a_gpu.py` 做 GPU consistency 验证。历史 focused gate `dev/benchmarks/validate_fama_macbeth_review_fix_gpu.py` 继续作为 chronology/formula/rank/inference 的详细 correctness oracle。final optimized-source physical acceptance 应使用 `dev/benchmarks/validate_fama_macbeth_optimized_gpu.py`：它会先执行该 focused oracle，再检查 backend-specific solver 与 rank-synchronization provenance 后生成最终 artifact。performance/crossover evidence 则由 `dev/benchmarks/benchmark_fama_macbeth_scaling_gpu.py` 单独记录；不会根据 micro fixture 宣称普遍 GPU speedup。Fama-MacBeth 不属于 Stage-C residual-covariance matrix，因为它的 covariance 来自 coefficient series，而不是 observation residual。
