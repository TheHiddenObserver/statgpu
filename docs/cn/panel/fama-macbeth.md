...
时期只有在满足 `min_obs_per_period` 且通过实现中的最小样本量规则 $n_t\ge k$ 时才会保留，其中 $k$ 是含 intercept 的 design width；随后每个 retained period 仍必须满足 full-rank contract。NumPy、CuPy 与 Torch 统一使用 conservative exact-size Gram-certificate dispatch。retained period 按真实 row count 分组，不做 zero padding。因为每个 period 都包含精确 intercept，full-rank period 可以先沿 constant direction 移除一个 range-safe 的公共 response anchor，再构造 Gram 右端项，solve 后只在 intercept coordinate 恢复该 anchor。这用于消除巨大公共 response level。除此之外，$X_t^\top y_t$ 的精确 intercept coordinate 会直接使用 magnitude-tiered response sum，而不是普通 BLAS reduction，因此 `[2**55, 1, -2**55]` 这类围绕 0 的 cancellation tail 仍能保留可表示的贡献 `1`，并且不需要 SVD fallback。也就是说，response anchoring 与 stable intercept RHS 分别处理“巨大公共 level”和“围绕 0 的 cancellation”两类不同风险。

只有当 backend-native spectrum 满足 $\lambda_{\min}(G_t)/\lambda_{\max}(G_t)>10^{-4}$，且 Gram matrix、centered right-hand side 与 candidate solution 都有限时，才允许使用 Gram solve。对 Fama-MacBeth 的 single-response path，一次 augmented solve 同时返回 candidate coefficient 与用于 coordinatewise RHS-roundoff bound 的 inverse-Gram 信息，因此新增精度证书不会引入第二次 Gram factorization。若某个非 intercept coefficient 低于可认证的 float64 resolution，该 period 会退出 Gram fast path 并进入维护中的 SVD fallback。若某个非 intercept RHS coordinate 已舍入成精确 0，但对应 absolute summand mass 非零，则采用更保守的 fail-closed 语义：float64 arithmetic 无法认证这个 0 究竟是真正的数学 0，还是一个仍可表示的 cancellation tail 已被擦除；而 rounded SVD basis 同样可能产生有限但错误的低阶 coefficient。因此这类情况直接抛出 `FloatingPointError`，而不是发布无法认证的 period coefficient。普通 unsafe subset 中，Torch 使用 documented stacked-SVD，NumPy/CuPy 保持二维 fallback。
...
**如果 period full rank，但某个 coefficient 低于可靠的 float64 resolution 呢？**  fit 会抛出带 coefficient-resolution 信息的 `FloatingPointError`。这与 rank deficiency 不同：design 可以 well-conditioned，但 rounded projection arithmetic 仍可能无法可靠区分一个远小于 dominant direction 的 coefficient。若 non-intercept RHS 已舍入为 0、但同时存在非零正负 summands，也会采用同样的 fail-closed 行为，因为普通 float64 arithmetic 无法认证该 0 是否精确。
...
内部 maintained regression 还覆盖 formula intercept、array/formula chronology、formula missing-row alignment、retained-period rank rejection、failed-refit invalidation、SVD rank-boundary policy、conservative Gram-certificate acceptance/rejection、backend-native distribution routing、df=1/df=2 boundary、balanced/shuffled-unbalanced exact-size grouping、chronological rank-error reporting、SVD fallback ownership、direct-fit finite-validation ownership、packed reporting snapshot、large-common-intercept Gram centering、stable exact-intercept Gram-RHS cancellation、ambiguous nonconstant zero-RHS rejection，以及显式 coefficient-resolution failure reporting。

标准 full-rank numeric-time case 由 `dev/benchmarks/validate_panel_stage_a_gpu.py` 做 GPU consistency；历史 focused/optimized/scaling runners继续保留。PR126历史 P100 evidence仍锚定旧 numerical source，不代表 current-head acceptance。`dev/benchmarks/validate_panel_intercept_cancellation_gpu.py` 继续验证更广的 shared coefficient-resolution 与 large-intercept panel contract；新增 focused gate `dev/benchmarks/validate_fama_macbeth_rhs_cancellation_gpu.py` 会在 CuPy CUDA 与 Torch CUDA 上额外验证 intercept cancellation tail 保持 `gram-certified`/zero fallback，以及 ambiguous nonconstant zero RHS 必须 fail closed。
