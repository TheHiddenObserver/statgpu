...
重新计算近端步，最多回溯 20 次。如果 20 次候选都不能通过这一保护条件，求解器会恢复到最近一次已经接受的迭代点，发出 `ConvergenceWarning` 并停止，而不会发布一个未经验证的候选点。受支持的异步 GPU 非光滑路径为了避免每次回溯都发生设备同步，会使用经过安全放大的固定 $L_k$；因此该路径的数学更新仍是同一个近端步，但不会逐次执行上述 CPU 式回溯。
