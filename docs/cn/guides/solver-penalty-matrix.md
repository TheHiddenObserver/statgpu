...
- FISTA-LLA、Proximal IRLS-CD 等内部解析后的标签可以出现在矩阵中，即使它们不是公开的 `solver=` 参数值。
...
- FISTA-LLA 表示标量 SCAD/MCP 使用的非凸延续路径；分组 FISTA-LLA 是对应的分组路径。
- Proximal IRLS-CD 是专用的内部解析路径，而不是公开的显式求解器关键字。
- Group Lasso 和 Adaptive Group Lasso 使用分组 FISTA；Group SCAD/MCP 使用分组 LLA 路径。
...
| `proximal_irls_cd` | 专用近端 IRLS 上界近似 | 专用非凸路径 | 内部解析标签；不是公开显式 `solver=` 关键字 |
| `proximal_newton` | 损失函数与惩罚项具备相容的 Newton/近端结构 | 依路径而定 | 行为取决于具体损失函数与惩罚项 |

不支持的显式估计器组合会在数值拟合前报错。本节只描述共享数值前提；具体模型的排除项与替代路径应查对应模型页。
...
| `admm` | 相容的变量分裂与光滑 w 更新 | 近端形式 | 依路径而定 | ✅ |
...
- [损失函数](../models/losses.md) — 损失函数层的数学定义与数值原语
- [广义线性模型](../models/generalized-linear-model.md) — GLM 分布族/链接函数、权重、CV 与推断
- [分位数回归](../models/quantile.md) — Quantile 专属求解器、惩罚、权重与推断行为
- [稳健回归](../models/robust.md) — Huber/Bisquare/Fair 模型行为
- [CoxPH](../models/coxph.md) — 生存模型与 Cox 专属 CV 行为
- [推断模式](inference-modes.md) — 系数推断方法的选择与解释
