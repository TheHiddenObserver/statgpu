...
## 未发布 — Quantile solver 与推断契约闭合（PR #166，目标 0.2.6）
...
- **独立 `QuantileRegression` 推断正确性**：非中位数 batched bootstrap 的梯度现在在非负 residual 上使用 `-tau`、负 residual 上使用 `1-tau`，修复了此前可能朝互补分位数方向求解的非对称性错误。standalone 非均匀权重推断在没有维护中的 weighted kernel/bootstrap 定义前明确 fail closed；均匀权重继续对应等价的未加权推断目标。当前维护的 bootstrap 明确是基于可交换残差的 i.i.d. residual bootstrap，不把它描述为对一般异方差稳健的 wild/multiplier bootstrap。
