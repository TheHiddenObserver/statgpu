...
物理验证现在按照 evidence chain 记录，而不是把某一个可变 head 当成永久状态。历史 Stage-C v5 acceptance 在 CuPy 与 Torch 上各通过 **47/47** correctness check，并通过同步的 **60-row** performance matrix。随后 Fama-MacBeth correctness source `464b587e83b234d78b5449666488d7f2f8ad367c` 在 Tesla P100 上重新验证；完成后续 Fama-MacBeth inference/solver 修复后，clean source `0222ff8a338cc319ddf43b3295db63b69f37f40f` 又通过 broad Stage-C 与 focused Fama-MacBeth 两个 physical gate。这些 artifact 对各自的 exact numerical source 继续有效，但只作为历史证据。之后再次独立按照 `.claude/skills/code-review.md` 审计时发现，legacy `PooledOLS(cov_type="hac")` 会在 ordered pandas categorical 上丢失用户声明的 category chronology，并可能按 materialized label 的词典序排序，从而改变 HAC lag pair 以及 inference。现在 legacy-HAC 的时间排序统一经过 shared chronology factorizer，并增加 ordered-categorical/numeric 等价、lexical negative control、formula alignment、missing-label regression，以及专用的 `dev/benchmarks/validate_panel_hac_chronology_gpu.py` CuPy/Torch gate。维护中的 panel estimator 也统一采用事务式 fail-closed refit：失败的新拟合会同时清除上一轮 stale inference 和本次已经部分写入的新输出。`FamaMacBeth` 不再为了 prediction device anchor 长期保留完整训练 design，而只保存 zero-length backend/device reference；focused physical runner 会同时验证该 anchor、prediction parity、两种 covariance mode、requested/executed backend identity，并直接记录实际 CuPy package version。已有同步 timing evidence 继续只作为 workload caveat：小规模、串行的 per-period SVD 在 GPU 上可能慢于 NumPy，不宣称通用 GPU speedup。由于这些最新修复在 `0222ff8a...` 之后再次改变 production numerical/lifecycle behavior，所以 exact-head physical acceptance 目前仍然**重新打开**；只有 broad Stage-C runner、`dev/benchmarks/validate_fama_macbeth_review_fix_gpu.py` 与 `dev/benchmarks/validate_panel_hac_chronology_gpu.py` 在同一个最终 clean source 上全部通过后才能关闭。
