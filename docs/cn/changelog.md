...
物理验证现在按照明确的 evidence chain 记录，而不是保留一个已经过期的单一 head 状态。历史 Stage-C v5 acceptance 在 CuPy 与 Torch 上各通过 **47/47** correctness check，并通过同步的 **60-row** performance matrix。随后 Fama-MacBeth correctness source `464b587e83b234d78b5449666488d7f2f8ad367c` 又在 Tesla P100 上重新验证：Stage-C matrix 在每个 backend 上均通过 35 个 estimator integration + 12 个 covariance primitive；focused CuPy/Torch 检查也通过 ordered-categorical chronology、formula/missing-row alignment、lexical-order negative control、retained-period rank rejection 以及两个显式 no-intercept spelling。原始证据保存在 `results/pr126_p100_fama_fix/`。随后再次按照 `.claude/skills/code-review.md` 复审时发现了额外 completion-gate 缺口，因此当前 exact-head physical acceptance 继续保持 reopened。现在 `FamaMacBeth` 会直接复用每个 retained period 的共享 rank-revealing SVD factor 计算 least-squares coefficient，在 active backend 上保留 singular-value cutoff，直到只提取最终 integer rank，并且不再构造该 estimator 不使用的 residual-OLS covariance bread。它同时发布标准 `ParameterInferenceResult` 与 private inference aliases，但继续保持公开 CuPy/Torch result array 为 backend-native；每次 refit 前会事务式失效旧的 fitted/inference state；Newey-West (`z`/normal) 与 nonrobust (`t`, df=`T-1`) 两种 inference mode 都进入维护中的 matrix coverage。focused physical runner 现在会检查完整 coefficient/inference/prediction parity、两种 covariance mode、requested/executed backend identity、rank/formula edge behavior，以及相同 workload 下同步后的 NumPy/CuPy/Torch timing。artifact 会记录 raw sample、GPU/NumPy median-time ratio 与明确的 `optimization_notes`；ratio 大于 1 会如实表示该 GPU backend 在此 fixture 上慢于 NumPy，而不会被隐藏成 correctness failure，也不宣称通用 GPU speedup。由于这些变更在 `464b587e...` 之后再次修改了 numerical/inference source，所以在最终 clean head 上重新执行 Stage-C physical runner 与 `dev/benchmarks/validate_fama_macbeth_review_fix_gpu.py` 之前，exact-head P100 acceptance 仍然**重新打开**。
